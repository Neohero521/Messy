...
`;
  ...
  // ===== 系统提示词（自由生成 + MVU变量系统 + 写卡预设注入） =====
  const SYS_PROMPT = '你是一位专业的 SillyTavern 角色卡创作助手，通过自然对话帮助用户创建完整的角色卡：世界书条目（World Info）、开场白（first_mes）、角色描述（description/personality/scenario）。你不受任何固定创作框架约束，完全按用户意愿自由生成内容，不套用八体系、不做质检、不做AI优化。\n\n' +
    '【世界书条目 JSON 字段规则】\n' +
    '- entries 为对象，键是 uid 字符串（"0"、"1"...），每条目 uid 为数字且与外层键一致\n' +
    '- 字段名与默认值（不需要的字段保留默认值，不得省略字段名）：\n' +
    '  uid / key(触发关键词数组) / keysecondary(过滤关键词数组) / comment(条目备注，仅UI显示) / content(条目内容，触发后注入提示词)\n' +
    '  constant(false=需关键词触发，true=常驻) / vectorized(false=关键词匹配，true=向量匹配) / selective(true=启用过滤逻辑) / selectiveLogic(0=与任意，1=非全部，2=非任意，3=与全部)\n' +
    '  addMemo(false) / order(100，越大越靠后影响越强) / position(0=角色定义前，1=角色定义后，2=作者注顶部，3=作者注底部，4=@深度，5=示例消息前，6=示例消息后，7=输出口)\n' +
    '  disable(false=启用) / ignoreBudget(false=不忽略token预算) / excludeRecursion(false) / preventRecursion(false) / delayUntilRecursion(0)\n' +
    '  probability(100) / useProbability(true) / depth(4，仅position=4时生效) / role(null，仅position=4时生效：0=系统，1=用户，2=助手)\n' +
    '  outletName(""，仅position=7时生效) / group("") / groupOverride(false) / groupWeight(100)\n' +
    '  scanDepth(null=继承全局) / caseSensitive(null=继承全局) / matchWholeWords(null=继承全局，中文建议false) / useGroupScoring(null=继承全局)\n' +
    '  automationId("") / sticky(null=无黏性) / cooldown(null=无冷却) / delay(null=无延迟) / triggers([]，空=所有生成场景触发；可选normal/continue/impersonate/swipe/regenerate/quiet)\n' +
    '  matchCharacterDescription(false) / matchCharacterPersonality(false) / matchScenario(false) / matchPersonaDescription(false) / matchCharacterDepthPrompt(false) / matchCreatorNotes(false)\n' +
    '  characterFilter(角色过滤对象：{"names":[],"tags":[],"isExclude":false}，无角色绑定可省略此字段)\n\n' +
    '【自由生成规则】\n' +
    '1. 世界书条目：按用户需求自由设计，keywords精准、content自包含完整信息（不依赖上下文）\n' +
    '2. 开场白（first_mes）：完整文本，场景描写→动作驱动→自然对话→结尾留钩\n' +
    '3. 角色描述（description/personality/scenario）：自由组织，四处身份描述保持一致\n' +
    '4. 条目命名、分组、权重、位置完全按用户意愿自由安排，不套用任何固定标签体系\n\n' +
    '【:::操作块协议（写卡器解析用，必须遵守）】\n' +
    '· 每次回复先输出1-2句自然语言说明，再输出:::操作块，操作块后不再解释\n' +
    '· 5种动作：\n' +
    '  ::: set 字段名\\n新值\\n:::  修改顶层字段（name/description/personality/scenario/first_mes/creator_notes等）\n' +
    '  ::: upsert 条目comment\\n完整content\\n:::  新增或覆盖世界书条目（覆盖时必须输出完整旧内容+改动部分）\n' +
    '  ::: update 条目comment\\n完整content\\n:::  仅修改已存在条目（输出完整content）\n' +
    '  ::: delete 条目comment\\n:::  删除条目（comment必须与精确清单字符级一致）\n' +
    '  ::: rename 旧comment → 新comment\\n:::  重命名条目\n' +
    '· ⚠️修改已存在条目时：必须先读取上方「当前角色卡已有内容」中该条目的完整content，:::upsert里输出完整旧内容+改动，严禁只输出变化字段（会导致原信息清空）\n' +
    '· ⚠️用户说"补充/细化/扩展XXX"时，用 ⟦<标签>条目名·补充维度⟧ 生成新条目，不覆盖原条目\n' +
    '· ⚠️增删改可混合在同一次回复，操作块之间空一行；只处理用户最新一条消息的指令\n' +
    '· 没有需要修改的内容就回复"本次无修改"\n\n' +
    '【MVU 变量系统（进阶可选）】\n' +
    '- MVU 相关条目（[InitVar]初始变量、变量列表、变量更新规则、变量输出格式、<状态栏>占位符等8条工作流）只在「MVU变量状态栏」Tab生成\n' +
    '- 本Tab（角色卡生成）禁止生成任何MVU相关条目/正则/状态栏代码；用户需要时请提示切换到MVU Tab\n';
        ...
        if (String(path).indexOf(entryPrefix) === 0) {
          ...
          if (cd.character_book && cd.character_book.entries) {
            ...
            if (!isNaN(idx) && String(idx) === entryKey && idx >= 0 && idx < beforeLen) {
              // 🐛修复"条目被清空"：禁止按数字索引删除。AI 只被教过用精确 comment 删除，
  ...
  async function callAI(prompt) {
    ...
    // system prompt：完全自由创作，不注入任何固定写作风格约束（写卡预设已按用户要求清除）
    const sysPrompt =
      '你是一位专业的 SillyTavern 角色卡创作助手，完全按用户意愿自由生成世界书条目、开场白、角色描述。';
    try {
  ...
  function buildPrompt(cardData, cardGenerated, messages) {
    ...
    const antiMvuBlock = '\n\n' +
      ...
      '5. ✅除上述MVU相关条目外，按用户意愿自由生成所有角色卡/世界书条目（不限体系、不限前缀，完全自由组织）\n' +
  ...
  function buildMvuTabPrompt(cardData, messages) {
    ...
    const mvuSystemPrompt = '' +
      ...
      '   · 世界书条目 / 开场白 / 角色描述 / 任何角色卡JSON 等全部与你无关\n' +
    ...
    try {
      ...
      function renderWelcome() {
        doc.body.innerHTML =
          ...
          '<p>基于SillyTavern原生机制，通过AI对话自由生成你的角色卡：世界书条目、开场白、角色描述。<br>和AI聊天就能生成符合ST规范的角色卡！</p>' +
          '<div class="welcome-features">' +
          '<div class="wf-item"><div class="wf-icon">' + svgIcon('chat', 18) + '</div><div class="wf-copy"><div class="wf-title">对话式创作</div><div class="wf-desc">像聊天一样自然，AI按你的意愿自由生成</div></div></div>' +
          '<div class="wf-item"><div class="wf-icon">' + svgIcon('chart', 18) + '</div><div class="wf-copy"><div class="wf-title">自由生成</div><div class="wf-desc">世界书条目、开场白、角色描述完全按你的想法组织</div></div></div>' +
          '<div class="wf-item"><div class="wf-icon">' + svgIcon('sliders', 18) + '</div><div class="wf-copy"><div class="wf-title">MVU变量系统</div><div class="wf-desc">独立Tab管理变量/脚本/正则，与角色卡完全隔离</div></div></div>' +
          '</div>' +
          ...
          '</div>' +
          '<p style="font-size:.7em;color:var(--muted);margin-top:18px">创作流程：告诉我你的世界构想 → AI自由生成世界书条目 → 生成开场白与角色描述</p>' +
          '</div>' +
          '</div>';
        ...
        doc.getElementById('startBtn').addEventListener('click', function() {
          ...
          renderChatUI();
          addAssistantMsg('你好！我是你的世界模式角色卡创作助手 🎭\n\n我会按照你的意愿自由生成世界书条目、开场白、角色描述，不受任何固定体系约束。\n\n请直接告诉我你的构想：\n   • 世界观（如"修仙宗门""末世生存""日式校园恋爱"）\n   • 核心角色或关键设定\n   • 任何你想加入的规则、机制、风格\n\n越具体越好，我随时按你的要求增删改！');
        });
      ...
      function renderChatUI() {
        doc.body.innerHTML =
          ...
          '<button class="icon-btn" id="wsMenuBtn" aria-label="工作区" title="工作区（字体大小/导入导出）">' + svgIcon('menu', 15) + ' 工作区</button>' +
      ...
      function renderWorkspaceMenuItems() {
        ...
        // 工具（生成开场白 —— 质检/优化/权重/分组/进度总览已按用户要求清除）
        items += '<div class="ws-dropdown-divider"></div>';
      ...
      function _wsSetContent(node, val) {
        if (!node) return;
        if (node.type === 'field') {
          if (Array.isArray(cardData[node.key]) && node.index != null) {
            cardData[node.key][node.index] = val;
          } else {
      ...
      function _wsSetMeta(node, meta) {
        ...
        if (meta.keys != null) e.keys = meta.keys.split(',').map(function(s) {
          return s.trim();
        }).filter(Boolean);
      ...
      function openWorkspacePanel() {
        ...
        container.querySelectorAll('.ws-panel-tab').forEach(function(tab) {
          tab.addEventListener('click', function() {
            ...
            container.querySelectorAll('.ws-panel-tab').forEach(function(t) {
      ...
      function buildWorkspaceTree() {
        ...
        tagOrder.forEach(function(t) {
          ...
          groups.push({
            name: '📚 <' + t + '> (' + entryBuckets[t].length + ')',
            nodes: entryBuckets[t],
            bucketTag: t
          });
        });
        ...
        if (mvuNodes.length) groups.push({
          name: '🔧 MVU变量 (' + mvuNodes.length + ')',
          nodes: mvuNodes,
          isMVU: true
        });
        // 3. 酒馆助手脚本
        ...
        if (thScripts.length > 0) {
          const thNodes = thScripts.map(function(s, i) {
            return {
              ...
              index: i,
              label: (s.name || ('脚本' + (i + 1))),
              path: 'tavern_helper.script_lib[' + i + ']'
            };
          });
          groups.push({
            name: '🧪 酒馆助手脚本 (' + thScripts.length + ')',
            nodes: thNodes
          });
        }
        // 4. 正则脚本
        ...
        if (rxScripts.length > 0) {
          const rxNodes = rxScripts.map(function(r, i) {
            return {
              ...
              key: 'regex_scripts',
              index: i,
              label: r.scriptName || ('正则' + (i + 1)),
              path: 'regex_scripts[' + i + ']'
            };
          });
          groups.push({
            name: '🔍 正则脚本 (' + rxScripts.length + ')',
            nodes: rxNodes
          });
        }
        // 渲染
      ...
      function bindWsEditorEvents(container) {
        ...
        if (constChk) constChk.addEventListener('change', function() {
          if (wsSelectedNode) _wsSetMeta(wsSelectedNode, {
            constant: this.checked
          });
        });
        if (selChk) selChk.addEventListener('change', function() {
          if (wsSelectedNode) _wsSetMeta(wsSelectedNode, {
            selective: this.checked
          });
        });
        if (vecChk) vecChk.addEventListener('change', function() {
          if (wsSelectedNode) _wsSetMeta(wsSelectedNode, {
            vectorized: this.checked
          });
        });
        ...
        if (depthInp) depthInp.addEventListener('change', function() {
          if (wsSelectedNode) _wsSetMeta(wsSelectedNode, {
            depth: parseInt(this.value) || 0
          });
        });
        if (orderInp) orderInp.addEventListener('change', function() {
          if (wsSelectedNode) _wsSetMeta(wsSelectedNode, {
            order: parseInt(this.value) || 0
          });
        });
        if (groupInp) groupInp.addEventListener('change', function() {
          if (wsSelectedNode) _wsSetMeta(wsSelectedNode, {
            group: this.value
          });
        });
        if (groupWInp) groupWInp.addEventListener('change', function() {
          if (wsSelectedNode) _wsSetMeta(wsSelectedNode, {
            groupWeight: parseInt(this.value) || 0
          });
        });
        if (keysInp) keysInp.addEventListener('change', function() {
          if (wsSelectedNode) _wsSetMeta(wsSelectedNode, {
            keys: this.value
          });
        });
      ...
      function _wsDeleteNode(container, node) {
        ...
        if (!deleted) {
          showToast('⚠️ 删除失败：未找到该' + typeLabel, 'warning');
          return;
        }
      ...
      function importCardData(data) {
        ...
        const greeting = '你好！已成功导入角色卡「' + (cardData.name || '未命名') + '」🎭\n\n' +
          ...
          '• 想继续扩充？直接描述你想要的设定，我自由生成世界书条目/开场白/角色描述\n\n' +
      ...
      function updateCtxBar() {
        ...
        let h = '';
        if (__tab === 'card') {
          // 角色卡Tab：展示当前内容概况（八体系模块导航已按用户要求清除）
          const _entries = ((cardData.character_book || {}).entries || []).filter(function(e) {
            return !isMVUEntry(e.comment || '');
          });
          const _firstOk = cardData.first_mes && cardData.first_mes.length > 50;
          const _descOk = cardData.description && cardData.description.length > 50;
          const _nameOk = !!cardData.name;
          h += '<span class="ctx-chip ' + (_nameOk ? 'ok' : 'todo') + '" title="世界/角色名称">' + svgIcon(_nameOk ? 'checkCircle' : 'circle', 11) + ' 名称</span>';
          h += '<span class="ctx-chip ' + (_descOk ? 'ok' : 'todo') + '" title="世界观描述">' + svgIcon(_descOk ? 'checkCircle' : 'circle', 11) + ' 描述</span>';
          h += '<span class="ctx-chip ' + (_firstOk ? 'ok' : 'todo') + '" title="开场白">' + svgIcon(_firstOk ? 'checkCircle' : 'circle', 11) + ' 开场白</span>';
          h += '<span class="ctx-chip ' + (_entries.length > 0 ? 'ok' : 'todo') + '" title="世界书条目数（不含MVU条目）">' + svgIcon(_entries.length > 0 ? 'checkCircle' : 'circle', 11) + ' 条目×' + _entries.length + '</span>';
        } else {
      ...
      function updateQuickActions() {
        ...
        const actions = [];
        if (currentTab === 'card') {
          // 主操作：自由生成（八体系分期已按用户要求清除）
          if (!hasEntries) actions.push({
            action: 'axiom',
            icon: 'globe',
            label: '生成世界书条目',
            hl: true
          ...
          actions.push({
            ...
            icon: 'sparkle',
            label: '生成并写入酒馆',
            title: '通过写卡器装配角色卡（含MVU/正则/脚本），直接写入到酒馆当前角色卡',
            hl: p >= 95
          });
        } else {
      ...
      function handleQuickAction(action) {
        ...
        const cardOnlyActions = ['opening', 'generate', 'axiom', 'entity_interact'];
        ...
        // 八体系/质检/AI优化相关prompt已按用户要求清除，仅保留自由生成话术
        const cardPrompts = {
          next: '下一步我该做什么？请根据当前已有的角色卡内容，给出2-3条具体可执行的建议，并说明每条建议会丰富哪部分内容。',
          summary: '帮我梳理一下当前已收集的信息和进度：1) 已完成的核心设定 2) 现有世界书条目/开场白/角色描述概况 3) 还缺什么 4) 推荐的下一步。用简洁列表呈现。',
          opening: '请根据现有世界观设定生成一段500-800字的开场白（first_mes）。要求：场景描写→主角出场→冲突/悬念→结尾留钩。必须是完整文本，禁止占位符。',
          situation: '请帮我完善当前局势和主要势力关系，用:::upsert操作块输出新的世界书条目。',
          axiom: '请帮我完善世界基础设定：世界元数据、世界观公理、力量体系骨架。用:::upsert操作块输出新条目，每条content≥250字。',
          soft_rules: '请帮我设计互动规则：互动选项规则、叙事风格引导、剧情节奏控制。用:::upsert操作块输出新条目。',
          core_rules: '请帮我完善核心规则：绝对禁止项、输出格式要求、AI身份定位。详细规则用:::upsert新条目输出。',
          near_constraint: '请帮我设计近场约束：当前局势、即时状态、临时任务。用:::upsert操作块输出触发式新条目。',
          scene_mechanics: '请帮我完善场景机制：核心玩法、世界规则、战斗/修炼/谈判等机制。用:::upsert操作块输出新条目。',
          entity_interact: '请帮我设计实体内容：重要角色（NPC）、势力与组织、关键物品、地点场景。用:::upsert操作块输出新条目，prevent_recursion=true。',
          narrative_bg: '请帮我完善叙事背景：故事发展、文化与习俗、历史事件、主线剧情。用:::upsert操作块输出新条目，delay_until_recursion=true。',
          dynamic_adapt: '请帮我设计动态适配内容：新手引导、互动选项、depth_prompt渐进引导、分支开局（多开局请用条目+MVU initvar覆盖实现，禁止写入alternate_greetings字段）。用:::upsert操作块输出新条目。（状态栏和变量系统请去MVU Tab制作）'
        };
        const mvuPrompts = {
      ...
      async function doGenerate() {
        ...
        try {
          ...
          let genPrompt = filteredSysForGenerate + antiMvuBanGenerate +
            ...
            '- 多开局机制：自由设计多开局方式（可在开场白内嵌互动选项或分支出场）\n' +
            '- extensions.depth_prompt：新手引导（depth=0，可选）\n' +
            '- character_book.entries：不限数量、不限体系、完全自由组织，每条字数自由\n' +
      ...
      function getModuleProgress() {
        ...
        // 通用内容维度（八体系模块已按用户要求清除，改为按基础内容类型判断）
        const keywords = {
          world: ['世界', '世界观', '设定', '大陆', '星球', '世界规则'],
          character: ['角色', '人物', 'NPC', '性格', '主角'],
          scenario: ['场景', '地点', '局势', '地图'],
          faction: ['势力', '组织', '宗门', '家族'],
          item: ['物品', '道具', '装备', '宝物'],
          story: ['故事', '剧情', '事件', '历史'],
          culture: ['文化', '习俗', '宗教'],
          mechanic: ['规则', '机制', '系统', '玩法'],
      ...
      function _renderPreviewImpl() {
        ...
        // 模块进度（独立 pv-section，角色卡Tab：隐藏MVU模块；八体系模块已按用户要求清除）
        const mp = getModuleProgress();
        const modLabels = {
          world: {
            ic: 'globe',
        ...
        const multiOpenEntries = allEntriesForMulti.filter(function(e) {
          return (e.comment || '').indexOf('分支开局') >= 0 || (e.comment || '').indexOf('多开局') >= 0;
        }).length;
      ...
      function bindPreviewInteractions() {
        ...
        for (let mi = 0; mi < modItems.length; mi++) {
          ...
          modItems[mi].addEventListener('click', function(e) {
            e.stopPropagation();
            const mod = this.getAttribute('data-mod');
            if (mod) handleQuickAction('axiom'); // 自由生成入口（原模块快捷动作已按用户要求清除，统一走通用生成指令）
          });
        }
        // 状态栏按钮
