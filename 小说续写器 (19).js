 ...
 *
 * 本脚本严格遵循：
 *   · tavern-helper-template 脚本规范（$(()) 入口；不写任何卸载钩子，交由 JSR 沙箱自动清理）
...
(function() {
  ...
  // ============ 持久化工具（纯 localStorage 权威版）============
  // 读写都只认 localStorage，与写卡器一致；酒馆 API 仅 best-effort 写入、绝不作为读取源
  // （getVariables({type:'script'}) 返回结构/作用域因版本而异，读取依赖它必然丢数据）
  const LOCAL_VARS_KEY = SCRIPT_ID + ':vars';

  // 只读 localStorage，绝不碰酒馆 API
  ...
  // 先读→合并→写，避免覆盖其他 key；同时 best-effort 同步到酒馆变量（失败不影响）
  let _quotaWarnedAt = 0;
  function _setScriptVars(obj) {
    var incoming = (obj && typeof obj === 'object') ? obj : {};
    // 唯一权威源：localStorage。不再调用酒馆/JSR 的变量 API：
    // 后台脚本直接调 window.parent.replaceVariables({type:'script'}) 拿不到 script_id，
    // 必然抛错或误写，纯属对主窗口的越界触碰，没有任何收益。
    try {
      var raw = localStorage.getItem(LOCAL_VARS_KEY);
      ...
      try {
        ...
        _quotaWarnedAt = 0; // 写入成功后复位，允许下次满时再次提醒
      } catch (quotaErr) {
        // 配额超限（整本小说/书架过大）：这会让整个酒馆 origin 的 localStorage 写入失败，
        // 进而可能中断酒馆/JSR 的保存逻辑。这里明确告警（每 30s 最多一次），不静默丢数据，
        // 也不反复全量 stringify 拖卡主线程。
        var now = Date.now();
        if (now - _quotaWarnedAt > 30000) {
          _quotaWarnedAt = now;
          try { console.error('[小说续写器] localStorage 写入失败（空间已满，约 ' + Math.round(payload.length / 1024) + 'KB）。请在书架中删除不需要的小说后再试，否则新进度无法保存。', quotaErr); } catch (_) {}
          try { if (typeof toastr !== 'undefined' && toastr && toastr.warning) toastr.warning('本地存储空间已满，新进度可能无法保存。请在书架删除不需要的小说。', '小说续写器'); } catch (_) {}
        }
      }
    } catch (e) {
      try { console.warn('[小说续写器] localStorage 持久化异常:', e && e.message); } catch (_) {}
    }
    return true;
  }
  ...
  // ============ iframe 渲染容器（UI 全部搬进独立 iframe，与写卡器一致）============
  // 业务逻辑仍在外层，所有 DOM 操作经 $() / getDoc() 重定向到 iframe，
  // 重排/重绘/动画只发生在 iframe 内，不再影响酒馆聊天页
  let _novelIframe = null;
  let _novelIframeDoc = null;
  let _novelIframeWin = null;
  let _novelIframeJQ = null;

  // ===== mini-jQuery（覆盖续写器用到的所有方法，无需 CDN）=====
  function _buildMiniJQ(win, doc) {
    function Q(sel) {
      ...
      els = els.filter(Boolean); // 防御 null/undefined 元素（closest/parent 可能返回 null）
    ...
    Q.fn = {
        ...
        this.els.forEach(el => {
          el.addEventListener(evt, function (e) {
            if (delegate) {
              ...
              if (t && el.contains(t)) {
                // jQuery 语义：委托回调中 currentTarget 指向被委托元素（业务代码多用 $(e.currentTarget)）
      ...
      prop(k, v) {
        // jQuery 语义：readonly→readOnly 等驼峰 IDL 属性，并同步布尔 content attribute
      ...
      fadeIn(dur, cb) { const raf = (win.requestAnimationFrame || function (cb2) { cb2(); }).bind(win); this.els.forEach(e => { e.style.transition = 'opacity ' + (dur || 300) + 'ms'; e.style.display = ''; raf(() => e.style.opacity = '1'); }); if (cb) setTimeout(cb, dur || 300); return this; },
      fadeOut(dur, cb) { this.els.forEach(e => { e.style.transition = 'opacity ' + (dur || 300) + 'ms'; e.style.opacity = '0'; setTimeout(() => e.style.display = 'none', dur || 300); }); if (cb) setTimeout(cb, dur || 300); return this; },
      ...
      data(k, v) {
        // jQuery 语义：data-novel-id → dataset.novelId（连字符转驼峰）
      ...
      offset() { if (!this.els[0]) return null; const r = this.els[0].getBoundingClientRect(); return { top: r.top, left: r.left, width: r.width, height: r.height }; },
      ...
      not(sel) { return Q(this.els.filter(e => !e.matches(sel))); },
      eq(i) { return Q(this.els[i]); },
  ...
  // ===== mini-toastr（借用父页面的 toastr，否则 console）=====
  function _buildMiniToast(win) {
    function show(msg, type, title) {
      ...
      try { console.log('[小说续写器][' + type + '] ' + msg); } catch (_) {}
    }
    return {
  ...
  // ===== 创建 iframe（UI_HTML + UI_CSS 全部注入 srcdoc）=====
  function _createNovelIframe() {
    return new Promise(function (resolve, reject) {
      try {
        ...
        // srcdoc 注入 UI（转义 </style> / </script>，避免破坏 HTML 结构）
        ...
        iframe.addEventListener('load', function () {
          ...
          try {
            ...
            // 先保持 hidden，openNovelWriter 末尾按面板开关状态决定显隐
            iframe.style.visibility = 'hidden';
            // ★ 监听面板 show class：面板开→iframe 显示并隐藏父页面常驻球；面板关（X/点球/点外部）→iframe 隐藏、常驻球复现（全局始终只有一个球）
            try {
              const _panel = doc.getElementById('novel-writer-panel');
  ...
  // ===== 统一配置入口：集中调参，避免魔法数字散落各处 =====
  // 新增配置项请加在这里；原有常量名保留为别名，不影响现有代码
  const CONFIG = Object.freeze({
    // AI 调用
    AI_CALL_TIMEOUT_MS: 120000,      // 单次 AI 生成超时（毫秒）
    MAX_RETRY_TIMES: 3,               // AI 生成最大重试次数
    RETRY_DELAY_MS: 1200,             // 重试间隔（毫秒）
    BATCH_MERGE_DELAY_MS: 1500,       // 批量合并延迟（毫秒）
    // 限流
    MAX_API_CALLS_PER_MINUTE: 3,      // 每分钟最大 API 调用次数
    RATE_LIMIT_WINDOW_MS: 60000,      // 限流窗口（毫秒）
    // 持久化
    SAVE_DEBOUNCE_MS: 250,            // 设置保存防抖（毫秒）
    LOCAL_STORAGE_QUOTA_WARN_MS: 30000, // 配额告警间隔（毫秒）
    // 悬浮球
    FLOAT_BALL_BASE_SIZE: 64,         // 悬浮球基础尺寸（px）
    FLOAT_BALL_MIN_SCALE: 50,         // 悬浮球最小缩放（%）
    FLOAT_BALL_MAX_SCALE: 200,        // 悬浮球最大缩放（%）
    FLOAT_BALL_SCALE_STEP: 10,        // 悬浮球缩放步长（%）
    // UI 渲染
    RENDER_RAF_ENABLED: true,         // 书架渲染 rAF 合并开关
    PREVIEW_DEBOUNCE_MS: 80,          // 预览渲染防抖（毫秒）
    // 图谱
    GRAPH_CHAPTERS_PER_GRAPH: 1,      // 每 N 章生成一个图谱
    MAX_CHAPTERS_PER_BATCH: 20,       // 单次批量生成最大章节数
  });

  // ===== AI 调用超时包装：防止 generateRaw 永久 pending 导致 isGenerating 锁死 =====
  const AI_CALL_TIMEOUT_MS = 120000; // 单次 AI 调用上限 120 秒，超时后进入重试/报错
  function withTimeout(promise, ms, label) {
    return new Promise(function (resolve, reject) {
      var timer = setTimeout(function () {
        reject(new Error((label || '操作') + ' 超时（' + Math.round(ms / 1000) + 's），已自动取消'));
      }, ms);
  ...
  // ===== 分级日志：统一前缀 + 错误详情，避免散落的 console.log 难追踪 =====
  const LOG_PREFIX = '[小说续写器]';
  ...
  function logError(scope, err, userMsg) {
    try {
      console.error(LOG_PREFIX + '[' + (scope || '?') + ']', err && err.stack ? err.stack : err);
      if (userMsg && typeof toastr !== 'undefined' && toastr && toastr.error) toastr.error(userMsg, '小说续写器');
    } catch (_) {}
  }

  // ===== 空值守卫：替代到处写的 a && a.b && a.b.c =====
  function safeStr(v) { return v == null ? '' : String(v); }
  function safeNum(v, dft) { var n = Number(v); return isFinite(n) ? n : (dft === undefined ? 0 : dft); }
  function safeArr(v) { return Array.isArray(v) ? v : []; }
  function safeObj(v) { return (v && typeof v === 'object') ? v : {}; }
  function safeGet(obj, path, dft) {
    if (obj == null) return dft;
    var keys = String(path).split('.');
    var cur = obj;
    for (var i = 0; i < keys.length; i++) {
      if (cur == null || typeof cur !== 'object') return dft;
      cur = cur[keys[i]];
    }
    return cur === undefined ? dft : cur;
  }

  // ===== 主动清理句柄机制：脚本禁用/切卡/刷新时自己回收 DOM 与监听（不依赖 JSR 沙箱）=====
  // 借鉴写卡器：注册 pagehide 后 JSR 会关闭沙箱保护，但自己清理更可靠、时机更可控。
  ...
  // ===== 父页面 document（区别于 iframe 内 document）=====
  ...
  // ---- 工具：document / $ ----
  // 业务逻辑的 DOM 操作全部落在 iframe 内（与写卡器一致的隔离渲染）
  function _pDoc() {
    // ★ 优先返回 iframe document（业务逻辑的 DOM 操作全部落在 iframe 里）
  ...
  // $ / jQuery / toastr：优先转发到 iframe 内（DOM 操作隔离在 iframe 里）
  // 注意：不能再用 _resolveDollar 的自赋值模式（它会覆盖 $ 导致丢失 iframe 分支）
  var $ = function () {
    // ★ 优先使用 iframe 内的 jQuery（DOM 操作隔离在 iframe 里）
    if (_novelIframeJQ) return _novelIframeJQ.apply(null, arguments);
    // 其次父页面真实 jQuery（iframe 未打开时可能用到）
    try {
  ...
  // toastr 动态转发：优先 iframe 内 toastr，其次父页面 toastr，最后 console
  ...
  var toastr = new Proxy({}, {
    get: function (_t, type) {
      ...
      // 兜底
      return function (msg) { try { console.log('[小说续写器][' + type + '] ' + msg); } catch (_) {} };
    }
  });
...
function _loadSettingsCache() {
  ...
  // 结构 A：外层有 extensionName 键
  if (vars[extensionName] && typeof vars[extensionName] === 'object') {
    _settingsCache = vars[extensionName];
    return;
  }
  // 结构 B：直接就是设置内容（有本脚本的特征键）
  if (vars.chapterList !== undefined ||
      vars.mergedGraph !== undefined ||
      vars.bookshelf !== undefined ||
      vars.chapterGraphMap !== undefined ||
      vars.continueWriteChain !== undefined) {
    _settingsCache = vars;
    return;
  }
  // 空对象：第一次运行，交给 loadSettings 用默认值初始化
  _settingsCache = {};
}

function _persistSettings() {
    if (_settingsCache !== null) {
        _setScriptVars({ [extensionName]: _settingsCache });
    }
}

// 持久化策略：
//   · 常规修改走 250ms 防抖合并写入，避免每次按键都同步 stringify 整本小说导致主线程卡死
//     （这是拖垮酒馆/JSR 面板的重要诱因）。
//   · 页面刷新/关闭前由 beforeunload 同步 flush 一次，保证刷新不丢进度。
//   · 权威存储只有 localStorage，不依赖酒馆脚本变量 API。
const _saveTimer = { id: null };
const SAVE_DEBOUNCE_MS = 250;
function _flushSettings() {
    if (_saveTimer.id !== null) {
...
function saveSettingsDebounced() {
    ...
    _saveTimer.id = setTimeout(function () {
        _saveTimer.id = null;
        try { _persistSettings(); } catch (e) {
            try { console.warn('[小说续写器] 持久化失败:', e && e.message); } catch (_) {}
        }
    }, SAVE_DEBOUNCE_MS);
}
...
async function rateLimitCheck() {
    ...
    // 从设置读取限流参数：窗口内最大调用次数 + 时间窗口（秒/分钟可选）
    ...
    if (apiCallTimestamps.length >= maxCalls) {
        ...
        if (waitTime > 0) {
            ...
            apiCallTimestamps = apiCallTimestamps.filter(timestamp => newNow - timestamp < windowMs);
        }
    }
...
async function generateRawWithBreakLimit(params) {
    ...
    if (settings.enableAutoParentPreset) {
        ...
        // 预设参数优先：只有白名单内的业务字段（或预设未定义的键）才允许被调用方参数覆盖，
        // 其余 temperature / top_p / top_k 等一律以当前预设为准
        ...
        for (const k of Object.keys(params || {})) {
            if (PRESET_OVERRIDE_WHITELIST.has(k) || presetParams[k] === undefined) {
    ...
    while (retryCount < MAX_RETRY_TIMES) {
        ...
        try {
            ...
            // withTimeout：底层 generateRaw 若永久 pending（网络/模型挂死），120s 后自动 reject，
            // 进入现有 catch 重试逻辑，避免 isGenerating 永久锁死。
...
function getActivePresetParams() {
    ...
    if (settings.enableAutoParentPreset) {
        ...
        // 方案4: _pWin().generation_params
        if (Object.keys(presetParams).length === 0) {
            console.log('[小说续写器] 尝试方案4: 使用 _pWin().generation_params');
            if (_pWin().generation_params && typeof _pWin().generation_params === 'object') {
                presetParams = { ..._pWin().generation_params };
                presetSource = '_pWin().generation_params';
                console.log('[小说续写器] ✅ 方案4成功！', { keys: Object.keys(presetParams), data: presetParams });
            } else {
                console.log('[小说续写器] ❌ _pWin().generation_params 不存在或为空');
            }
        }
        
        // 方案5: _pWin().SillyTavern.presetManager
        if (Object.keys(presetParams).length === 0) {
            console.log('[小说续写器] 尝试方案5: 使用 _pWin().SillyTavern.presetManager');
            if (_pWin().SillyTavern?.presetManager?.currentPreset?.data) {
                presetParams = { ..._pWin().SillyTavern.presetManager.currentPreset.data };
                presetSource = `_pWin().SillyTavern.presetManager(${_pWin().SillyTavern.presetManager.currentPreset.name || 'unknown'})`;
                console.log('[小说续写器] ✅ 方案5成功！', { keys: Object.keys(presetParams), data: presetParams });
            } else {
                console.log('[小说续写器] ❌ _pWin().SillyTavern.presetManager 不存在或为空');
            }
        }
        ...
        // 方案7: presetSettings（context / 父窗口 / SillyTavern 全局）
        if (Object.keys(presetParams).length === 0) {
            console.log('[小说续写器] 尝试方案7: presetSettings');
            const presetSettingsCandidates = [
                context?.presetSettings,
                _pWin()?.presetSettings,
                _pWin()?.SillyTavern?.getContext?.()?.presetSettings
            ];
            for (const candidate of presetSettingsCandidates) {
                if (candidate && typeof candidate === 'object' && Object.keys(candidate).length > 0) {
                    presetParams = { ...candidate };
                    presetSource = 'presetSettings';
                    console.log('[小说续写器] ✅ 方案7成功！', { keys: Object.keys(presetParams) });
                    break;
                }
            }
            if (Object.keys(presetParams).length === 0) {
                console.log('[小说续写器] ❌ 方案7: presetSettings 不存在或为空');
            }
        }
        
        // 方案8: oai_settings / power_user 中的生成参数
        if (Object.keys(presetParams).length === 0) {
            console.log('[小说续写器] 尝试方案8: oai_settings / power_user');
            const oai = _pWin()?.oai_settings;
            const pwr = _pWin()?.power_user;
            const mergedFallback = {};
            if (oai && typeof oai === 'object') Object.assign(mergedFallback, oai);
            if (pwr && typeof pwr === 'object') Object.assign(mergedFallback, pwr);
            if (Object.keys(mergedFallback).length > 0) {
                presetParams = { ...mergedFallback };
                presetSource = 'oai_settings/power_user';
                console.log('[小说续写器] ✅ 方案8成功！', { keys: Object.keys(presetParams) });
            } else {
                console.log('[小说续写器] ❌ 方案8: oai_settings / power_user 不存在或为空');
            }
        }
    } else {
        console.log('[小说续写器] 预设开关未开启，使用 _pWin().generation_params');
        if (_pWin().generation_params && typeof _pWin().generation_params === 'object') {
            presetParams = { ..._pWin().generation_params };
            presetSource = '_pWin().generation_params (开关关闭)';
        }
    }
    ...
    // 全量透传当前预设的全部生成参数，仅排除元信息/敏感信息/调用方传入的冲突键
    ...
    // 全字段补全：用当前环境的其他可信数据源补充缺失的生成参数键（后源只补缺，不覆盖），
    // 保证"当前预设全部字段"都被带到每次 API 调用
    const supplementalSources = [
        _pWin()?.SillyTavern?.presetManager?.currentPreset?.data,
        context?.presetSettings,
        _pWin()?.presetSettings,
        context?.generation_settings,
        context?.textCompletionSettings,
        _pWin()?.generation_params
    ];
    for (const src of supplementalSources) {
        if (!src || typeof src !== 'object') continue;
        for (const key of Object.keys(src)) {
            if (presetParams[key] === undefined && !excludedKeys.has(key) && src[key] !== undefined && src[key] !== null) {
                presetParams[key] = src[key];
            }
        }
    }
    console.log('[小说续写器] 全字段补全后的预设参数:', presetParams);
    ...
    for (const key of Object.keys(presetParams)) {
        ...
        // 处理参数别名映射（SillyTavern 旧字段名 → 标准名称）
        let targetKey = key;
    ...
    console.log('[小说续写器] 全量透传后的预设参数:', filteredParams);
...
function getCurrentPresetName() {
    ...
    } else if (_pWin().SillyTavern?.presetManager?.currentPreset?.name) {
        presetName = _pWin().SillyTavern.presetManager.currentPreset.name;
        console.log('[小说续写器] 使用 _pWin().SillyTavern.presetManager.currentPreset.name 预设名称:', presetName);
    } else if (_pWin()?.current_preset?.name && typeof _pWin().current_preset.name === 'string') {
        presetName = _pWin().current_preset.name;
        console.log('[小说续写器] 使用 _pWin().current_preset.name 预设名称:', presetName);
    } else if (_pWin()?.generation_params?.preset_name && typeof _pWin().generation_params.preset_name === 'string') {
        presetName = _pWin().generation_params.preset_name;
        console.log('[小说续写器] 使用 _pWin().generation_params.preset_name 预设名称:', presetName);
    } else if (_pWin()?.extension_settings?.presets?.current_preset) {
        presetName = _pWin().extension_settings.presets.current_preset;
        console.log('[小说续写器] 使用 _pWin().extension_settings.presets.current_preset 预设名称:', presetName);
    } else {
...
const FloatBall = {
    ...
    destroy() {
        // 只 abort 自己通过 AbortController 注册的监听（含主窗口 resize）。
        // 绝不能写 window.parent.onresize = null / getDoc().onclick = null：
        // 那会清掉酒馆主窗口和其他插件注册的处理器，造成环境污染。
...
async function batchMergeGraphs() {
    ...
    const graphList = [...new Set(sortedChapters.map(chapter => {
...
async function loadSettings() {
    ...
    $("#write-chapter-count").val(settings.writeChapterCount || 1);
    $("#rate-limit-switch input").prop("checked", settings.rateLimitEnabled !== false);
    $("#rate-limit-switch").attr("aria-checked", settings.rateLimitEnabled !== false);
    $("#rate-limit-calls").val(settings.rateLimitMaxCalls || 3);
    $("#rate-limit-window").val(settings.rateLimitWindow || 60);
    $("#rate-limit-unit").val(settings.rateLimitUnit || "m");
    $("#graph-chapters-per-graph").val(settings.graphChaptersPerGraph || 1);
...
// rAF 合并：同一帧内多次调用 renderBookshelf 只执行最后一次，
// 减少批量删除/导出/标签筛选时的重复重渲染（书架 DOM 较重）
const _origRenderBookshelf = renderBookshelf;
let _rafBookshelfId = null;
renderBookshelf = function () {
  if (_rafBookshelfId !== null) return;
  _rafBookshelfId = requestAnimationFrame(function () {
    _rafBookshelfId = null;
    _origRenderBookshelf();
  });
};
...
async function validateContinuePrecondition(baseChapterId, modifiedChapterContent = null) {
    ...
    // 组图谱模式下组内章节共享同一图谱引用，按引用去重避免重复注入上下文
...
async function generateChapterGraphBatch(chapters) {
    ...
    const settings = extension_settings[extensionName];
    // 每几个章节合并生成 1 个图谱（1 = 每章单独生成，保持原行为）
    ...
    const groups = [];
    for (let i = 0; i < pendingChapters.length; i += perGraph) {
        groups.push(pendingChapters.slice(i, i + perGraph));
    }
    
    if (groups.length === 0) {
        toastr.info('所选章节均已有图谱', "小说续写器");
        return;
    }
    ...
    try {
        for (let i = 0; i < groups.length; i++) {
            ...
            const groupNum = i + 1;
            updateProgress('graph-progress', 'graph-generate-status', groupNum, groups.length, "图谱生成进度");
            
            // 合并组内章节内容，生成一个覆盖整组的图谱
            ...
            if (graphData) {
                // 组内每个章节都映射到同一个组图谱，保证后续校验/续写都能查到
                group.forEach(chapter => {
                    graphMap[chapter.id] = graphData;
                    const parsedChapter = currentParsedChapters.find(item => item.id === chapter.id);
                    if (parsedChapter) parsedChapter.hasGraph = true;
                });
                successCount++;
        ...
        renderChapterList(currentParsedChapters);
        if (perGraph > 1) {
            toastr.success(`图谱生成完成！成功生成 ${successCount} 个组图谱，覆盖 ${coveredCount}/${pendingChapters.length} 章`, "小说续写器");
        } else {
            toastr.success(`图谱生成完成！成功生成 ${successCount}/${pendingChapters.length} 个章节图谱`, "小说续写器");
        }
    } catch (error) {
...
async function mergeAllGraphs() {
    ...
    if (batchGraphs.length > 0) {
        ...
        const graphMap = extension_settings[extensionName].chapterGraphMap || {};
        // 组图谱模式下多个章节共享同一图谱引用，按引用去重，避免重复合并
...
async function generateNovelWrite() {
    ...
    const wordCount = parseInt($('#write-word-count').val()) || 2000;
    const chapterCount = Math.max(1, Math.min(20, parseInt($('#write-chapter-count').val()) || (extension_settings[extensionName].writeChapterCount || 1)));
    const mergedGraph = extension_settings[extensionName].mergedGraph || {};
    const enableQualityCheck = extension_settings[extensionName].enableQualityCheck;
    ...
    try {
        ...
        // 第 0 轮以基准章节为起点，之后每轮以上一轮生成的章节末尾为起点
        ...
        for (let chapterRound = 0; chapterRound < chapterCount; chapterRound++) {
            ...
            if (isTimelineSafeMode) {
                ...
                $('#write-status').text(`正在生成续写章节${roundInfo}（时间线安全模式）...`);
            } else {
                ...
                $('#write-status').text(`正在生成续写章节${roundInfo}...`);
            }
            ...
            if (enableQualityCheck && !stopGenerateFlag) {
                $('#write-status').text(`正在执行质量校验${roundInfo}...`);
                qualityResult = await evaluateContinueQuality(continueContent, precheckResult, useGraph, currentBaseContent, wordCount);
                
                if (!qualityResult.是否合格 && !stopGenerateFlag) {
                    toastr.warning(`质量不合格，总分${qualityResult.总分}，正在重新生成...`, "小说续写器");
                    $('#write-status').text(`正在重新生成${roundInfo}...`);
        ...
        if (chapterCount > 1) {
            toastr.success(`续写章节生成完成！本次共生成 ${generatedCount} 章`, "小说续写器");
        } else {
            toastr.success(completionMessage, "小说续写器");
        }
    } catch (error) {
        if (!stopGenerateFlag) {
            console.error('续写生成失败:', error);
...
//   3) 脚本卸载/切卡时由 JSR 沙箱自动清理注入的 DOM（本脚本不写任何卸载钩子）
//
...
async function openNovelWriter(opts) {
    ...
    if (_novelWriterOpened && _novelIframe) {
        // 已打开：确保 iframe 可见、面板显示（现在是 iframe 内的面板）
        try {
            if (_novelIframe) _novelIframe.style.visibility = 'visible';
            const $panel = _novelIframeJQ ? _novelIframeJQ('#novel-writer-panel') : null;
            if ($panel && !$panel.hasClass('show')) {
                $panel.addClass('show');
            }
            if (!silent && _novelIframeWin && _novelIframeWin.toastr) _novelIframeWin.toastr.info('小说续写器已打开');
        } catch (_) {}
        return;
    }
    ...
    _loadSettingsCache();
    try {
        // 1) 创建 iframe 并注入 UI（UI 全部渲染在独立 iframe 内，不污染酒馆页面）
        await _createNovelIframe();

    // 2) 业务逻辑初始化（DOM 操作经 $ / getDoc() 自动落到 iframe）
    // 各初始化步骤独立容错：任一失败只打日志，不销毁 iframe、不丢失入口
    try { initDrawerToggle(); } catch(e) { console.error('[小说续写器] initDrawerToggle 失败:', e); }
    try { initContinueChainEvents(); } catch(e) { console.error('[小说续写器] initContinueChainEvents 失败:', e); }
    try { initVisibilityListener(); } catch(e) { console.error('[小说续写器] initVisibilityListener 失败:', e); }
    try { await loadSettings(); } catch(e) { console.error('[小说续写器] loadSettings 失败:', e); }
    ...
    setupToggleSwitch("#rate-limit-switch", "rateLimitEnabled");
    ...
    $("#write-chapter-count").off("change").on("change", (e) => {
        const val = parseInt($(e.target).val());
        extension_settings[extensionName].writeChapterCount = Math.max(1, Math.min(20, val || 1));
        $(e.target).val(extension_settings[extensionName].writeChapterCount);
        saveSettingsDebounced();
    });
    
    $("#rate-limit-calls").off("change").on("change", (e) => {
        const val = parseInt($(e.target).val());
        extension_settings[extensionName].rateLimitMaxCalls = Math.max(1, Math.min(999, val || 3));
        $(e.target).val(extension_settings[extensionName].rateLimitMaxCalls);
        saveSettingsDebounced();
    });
    
    $("#rate-limit-window").off("change").on("change", (e) => {
        const val = parseInt($(e.target).val());
        extension_settings[extensionName].rateLimitWindow = Math.max(1, Math.min(1440, val || 1));
        $(e.target).val(extension_settings[extensionName].rateLimitWindow);
        saveSettingsDebounced();
    });
    
    $("#rate-limit-unit").off("change").on("change", (e) => {
        extension_settings[extensionName].rateLimitUnit = $(e.target).val() || "m";
        saveSettingsDebounced();
    });
    
    $("#graph-chapters-per-graph").off("change").on("change", (e) => {
        const val = parseInt($(e.target).val());
        extension_settings[extensionName].graphChaptersPerGraph = Math.max(1, Math.min(20, val || 1));
        $(e.target).val(extension_settings[extensionName].graphChaptersPerGraph);
        saveSettingsDebounced();
    });
    ...
    // 监听父窗口大小变化（原生绑定，避免 mini-jQuery 对 window 对象失效；只绑一次）
    try {
      if (!window.__nwResizeBound) {
        window.__nwResizeBound = true;
        const pw = (typeof window !== 'undefined' && window.parent) ? window.parent : window;
        if (pw && typeof pw.addEventListener === 'function') pw.addEventListener('resize', handleWindowResize);
      }
    } catch (_) {}

        // 3) 关闭按钮：隐藏 iframe（保留状态，不销毁），露出父页面悬浮球
        $("#panel-close-btn").off("click").on("click", function (e) {
            e.stopPropagation();
            try { if (_novelIframeJQ) _novelIframeJQ('#novel-writer-panel').removeClass('show'); } catch (_) {}
            try { if (typeof _persistSettings === 'function') _persistSettings(); } catch (_) {}
            setTimeout(function () {
                if (_novelIframe) _novelIframe.style.visibility = 'hidden';
            }, 200);
        });

        // 面板未打开时（静默注入且上次为关闭状态）隐藏 iframe，露出父页面悬浮球
        setTimeout(function () {
            try {
                if (_novelIframe) {
                    const panelOpen = _novelIframeJQ && _novelIframeJQ('#novel-writer-panel').hasClass('show');
        ...
        if (!silent && _novelIframeWin && _novelIframeWin.toastr) {
            _novelIframeWin.toastr.success('小说续写器已打开');
        }
    } catch (error) {
        console.error('[小说续写插件] openNovelWriter 失败:', error);
        if (!silent) {
            try { showToast('打开失败: ' + (error && error.message ? error.message : String(error)), 'error'); } catch(_) {}
        }
        // ★ 失败不销毁 iframe：保留面板/悬浮球能力；隐藏 iframe 并确保父页面 📖 入口存在
        _novelWriterOpened = false;
        try { if (_novelIframe) _novelIframe.style.visibility = 'hidden'; } catch(_) {}
        try { addNovelWriterFloatingButton(); } catch(_) {}
        throw error; // 让调用方（bootstrap/按钮点击）感知失败
    }
}

// ---- 卸载清理（cleanupScriptArtifacts 会调用这个）----
function cleanupNovelWriter() {
    try {
        // 1) 立即同步落盘（取消待执行的防抖写入，直接写）
        try { if (typeof _flushSettings === 'function') _flushSettings(); } catch(_) {}
        // 2) abort 本脚本注册到主窗口的 resize 等监听（旧 FloatBall），不残留主窗口监听
        try { if (typeof FloatBall !== 'undefined' && FloatBall && typeof FloatBall.destroy === 'function') FloatBall.destroy(); } catch(_) {}
        // 3) 销毁业务 iframe
        try { _destroyNovelIframe(); } catch(_) {}
        // 4) 清理遗留的旧 UI（兼容上一版注入到父页面的情况）
        try {
            var doc = _pDocParent();
    ...
    try {
      ...
      if (evtOn && getBtnEvt) {
        ...
        try {
          ...
          // ★ 必须传完整的 { name, visible: true }：JSR 的 ScriptButton schema 要求 visible 为 boolean（无默认值），
          //   只传 { name } 会被存为 visible: undefined，下次加载 Zod 校验失败 → 全局数据回退空 → 写盘清空全部脚本。
          if (appISB) { appISB([{ name: SCRIPT_NAME, visible: true }, { name: '打开小说续写器', visible: true }]); }
        } catch(_) {}
        return true;
      }
  ...
  // ★ 父页面常驻悬浮球（原版 float-ball 外观：卡片底色圆球 + 书本图标，非橙色）
  // 与 iframe/面板完全解耦：脚本启动立即挂载；面板关闭只隐藏 iframe，悬浮球始终在
  // 行为对齐写卡器悬浮图标：常驻显示、无需按钮、可拖拽、位置记忆
  ...
  function addNovelWriterFloatingButton() {
    try {
      ...
      styleEl.textContent =
        ...
        'width:64px;height:64px;border-radius:0;background:#f8f8f8;color:#222222;border:3px solid #222222;' +
        ...
        '#' + SCRIPT_ID + '-btn.nw-dragging{cursor:grabbing;transform:scale(1.25);opacity:.9;box-shadow:0 0 60px rgba(51,51,51,.3),0 12px 40px rgba(0,0,0,.7);}' +
        ...
        '#' + SCRIPT_ID + '-btn .ball-icon{font-size:28.8px;line-height:1;display:flex;align-items:center;justify-content:center;filter:drop-shadow(0 2px 4px rgba(0,0,0,.3));}' +
        ...
        'background:#f8f8f8;border:2px solid #222222;border-radius:0;box-shadow:6px 6px 0px rgba(0,0,0,.25);' +
        'font-family:system-ui,-apple-system,"Segoe UI",sans-serif;font-size:13px;color:#222222;}' +
        ...
        'border-radius:0;color:#222222;font-size:13px;font-family:inherit;cursor:pointer;text-align:left;transition:background .12s ease;}' +
        '#' + SCRIPT_ID + '-btn-menu .nw-mi:hover{background:#e8e8e8;}' +
        '#' + SCRIPT_ID + '-btn-menu .nw-mi .nw-mi-icon{font-size:15px;width:18px;text-align:center;flex-shrink:0;}' +
        '#' + SCRIPT_ID + '-btn-menu .nw-mi .nw-mi-val{margin-left:auto;color:#666666;font-size:12px;}' +
        '#' + SCRIPT_ID + '-btn-menu .nw-divider{height:1px;background:#cccccc;margin:4px 8px;}' +
        '#' + SCRIPT_ID + '-btn-menu .nw-scale-row{padding:6px 10px 8px;display:flex;flex-direction:column;gap:4px;}' +
        '#' + SCRIPT_ID + '-btn-menu .nw-scale-row label{font-size:11px;color:#666666;display:flex;justify-content:space-between;}' +
        '#' + SCRIPT_ID + '-btn-menu .nw-scale-row .nw-scale-val{color:#222222;font-weight:600;}' +
        '#' + SCRIPT_ID + '-btn-menu .nw-scale-row input[type=range]{width:100%;accent-color:#222222;cursor:pointer;}';
      (doc.head || (doc.getElementsByTagName && doc.getElementsByTagName('head')[0]) || doc.documentElement).appendChild(styleEl);

      var btn = doc.createElement('div');
      btn.id = SCRIPT_ID + '-btn';
      btn.setAttribute('role', 'button');
      btn.setAttribute('aria-label', '打开小说续写系统面板');
      btn.title = '小说续写器 · 拖拽移动 · 单击打开 · 右键菜单 · 滚轮缩放';
      btn.innerHTML = '<div class="ball-inner"><div class="ball-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" xmlns="http://www.w3.org/2000/svg"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg></div></div>';
      ...
      menu.innerHTML =
        '<button class="nw-mi" data-action="open"><span class="nw-mi-icon">📖</span>打开小说续写器</button>' +
      ...
      // 位置应用（默认右侧居中，钳制在视口内）
      function applyPos() {
        var w = pWin.innerWidth || doc.documentElement.clientWidth || 1280;
        var h = pWin.innerHeight || doc.documentElement.clientHeight || 800;
        var sz = curSize();
      ...
      function showMenu(x, y) {
        ...
        var r = menu.getBoundingClientRect ? menu.getBoundingClientRect() : { width: 180, height: 200 };
        var w = pWin.innerWidth || 1280, h = pWin.innerHeight || 800;
        var mw = r.width || 180, mh = r.height || 220;
        var l = Math.min(x, w - mw - 8);
        var t = Math.min(y, h - mh - 8);
      ...
      // 拖拽（pointer capture：move/up 只派发给按钮本身，重建时随元素自动清理）
      var drag = null;
      ...
      btn.addEventListener('pointercancel', function () {
        drag = null; btn.classList.remove('nw-dragging');
      });
      ...
      menu.addEventListener('click', function (e) {
        ...
        if (act === 'open') {
          ...
          st.x = null; st.y = null;
          applyPos(); saveSt(); hideMenu();
        } else if (act === 'reset-scale') {
          st.scale = 100;
          applyScale(); saveSt();
        } else if (act === 'hide') {
          hideMenu();
  ...
  // tryInit 仅作为兜底入口，正常流程由 scriptEntryPoint 直接注入 UI，悬浮球自然常驻
  let retryCount = 0;
  let _initRetryTimer = null;
  function tryInit() {
    addNovelWriterFloatingButton(); // ★ 兜底路径也先保证父页面入口存在
  ...
  function cleanupScriptArtifacts() {
    ...
    try {
      var doc = _pDoc();
      // 只在脚本被彻底卸载时清 DOM；刷新/切页不删，避免视觉跳动
      // （悬浮球 #novel-writer-float-ball 在 UI_HTML 的 root 内，保留不删）
...
var AGENT_TOOLS = {
  list_chapters: {
    name: 'list_chapters',
    description: '列出当前小说的所有章节',
    execute: function() {
      try {
        var chapters = (_settingsCache.chapterList || []).map(function(ch, i) {
          return { index: i, title: ch.title || ('第' + (i+1) + '章'), hasGraph: !!ch.hasGraph, contentLength: (ch.content || '').length };
        });
  ...
  read_chapter: {
    ...
    execute: function(params) {
      try {
        var idx = params.chapterIndex || 0;
        var ch = (_settingsCache.chapterList || [])[idx];
        if (!ch) return { success: false, error: '章节不存在: ' + idx };
        return { success: true, data: { title: ch.title, content: ch.content || '' } };
      } catch(e) { return { success: false, error: e.message }; }
    }
  },
  generate_graph: {
    name: 'generate_graph',
    description: '为指定章节生成图谱',
    params: ['chapterIndex'],
    execute: function(params) {
      try {
        var idx = params.chapterIndex || 0;
        if (typeof generateChapterGraph === 'function') {
          return generateChapterGraph(idx);
        }
        return { success: false, error: 'generateChapterGraph 函数不存在' };
      } catch(e) { return { success: false, error: e.message }; }
    }
  },
  write_chapter: {
    name: 'write_chapter',
    description: '续写生成新章节',
    params: ['prompt', 'baseChapterIndex'],
    execute: function(params) {
      try {
        if (typeof generateContinuation === 'function') {
          return generateContinuation(params.prompt, params.baseChapterIndex || 0);
        }
        return { success: false, error: 'generateContinuation 函数不存在' };
      } catch(e) { return { success: false, error: e.message }; }
    }
  },
  check_consistency: {
    name: 'check_consistency',
    description: '检查章节之间的一致性',
    params: ['chapterIndices'],
    execute: function(params) {
      try {
        var indices = params.chapterIndices || [];
        var results = [];
        for (var i = 0; i < indices.length; i++) {
          var ch = (_settingsCache.chapterList || [])[indices[i]];
          if (ch && ch.content) {
            results.push({ index: indices[i], length: ch.content.length, hasCharacters: ch.content.indexOf('主角') >= 0 || ch.content.indexOf('他') >= 0 });
          }
        }
        return { success: true, data: { checked: results.length, issues: [] } };
      } catch(e) { return { success: false, error: e.message }; }
    }
  },
  summarize_chapter: {
    name: 'summarize_chapter',
    description: '总结指定章节的要点',
    params: ['chapterIndex'],
    execute: function(params) {
      try {
        var idx = params.chapterIndex || 0;
        var ch = (_settingsCache.chapterList || [])[idx];
        if (!ch) return { success: false, error: '章节不存在' };
        var content = ch.content || '';
        var summary = content.substring(0, 200) + (content.length > 200 ? '...' : '');
  ...
  manage_memory: {
    name: 'manage_memory',
    description: '管理 Agent 的记忆（添加/查看/清除）',
    params: ['action', 'content'],
    execute: function(params) {
      try {
        if (params.action === 'add') {
          _agentState.memory.push({ time: Date.now(), content: params.content || '' });
          return { success: true, data: { memoryCount: _agentState.memory.length } };
        } else if (params.action === 'list') {
          return { success: true, data: { memory: _agentState.memory } };
        } else if (params.action === 'clear') {
          _agentState.memory = [];
          return { success: true, data: { cleared: true } };
        }
        return { success: false, error: '未知动作: ' + params.action };
      } catch(e) { return { success: false, error: e.message }; }
    }
  },
  export_chapter: {
    name: 'export_chapter',
    description: '导出指定章节为文本',
    params: ['chapterIndex'],
    execute: function(params) {
      try {
        var idx = params.chapterIndex || 0;
        var ch = (_settingsCache.chapterList || [])[idx];
        if (!ch) return { success: false, error: '章节不存在' };
        return { success: true, data: { title: ch.title, content: ch.content || '', exportable: true } };
      } catch(e) { return { success: false, error: e.message }; }
    }
  }
};
...
// 规划层：根据用户目标生成执行计划
function agentPlan(userGoal) {
  var chapters = _settingsCache.chapterList || [];
  var plan = [];
  
  if (userGoal.indexOf('生成图谱') >= 0 || userGoal.indexOf('图谱') >= 0) {
    plan.push({ tool: 'list_chapters', description: '列出所有章节', params: {} });
    for (var i = 0; i < Math.min(chapters.length, 10); i++) {
      plan.push({ tool: 'generate_graph', description: '为第' + (i+1) + '章生成图谱', params: { chapterIndex: i } });
    }
  } else if (userGoal.indexOf('续写') >= 0 || userGoal.indexOf('写') >= 0) {
    plan.push({ tool: 'list_chapters', description: '列出所有章节', params: {} });
    plan.push({ tool: 'read_chapter', description: '读取最后一章作为基准', params: { chapterIndex: Math.max(0, chapters.length - 1) } });
    plan.push({ tool: 'summarize_chapter', description: '总结上一章要点', params: { chapterIndex: Math.max(0, chapters.length - 1) } });
    plan.push({ tool: 'write_chapter', description: '续写新章节', params: { prompt: userGoal, baseChapterIndex: Math.max(0, chapters.length - 1) } });
  } else {
    plan.push({ tool: 'list_chapters', description: '列出所有章节', params: {} });
    plan.push({ tool: 'manage_memory', description: '记录用户目标到记忆', params: { action: 'add', content: '用户目标: ' + userGoal } });
  }
...
// 执行层：执行单个工具调用
function agentExecuteStep(step) {
  return new Promise(function(resolve, reject) {
    var tool = AGENT_TOOLS[step.tool];
    if (!tool) {
      resolve({ success: false, error: '工具不存在: ' + step.tool });
      return;
    }
    try {
      var result = tool.execute(step.params || {});
      resolve(result);
    } catch(e) {
...
// 反思层：检查执行结果是否满意
function agentReflect(step, result) {
  if (result.success) {
    return { approved: true, feedback: '执行成功' };
  } else {
    return { approved: false, feedback: '执行失败: ' + (result.error || '未知错误') };
  }
}

// 主流程：运行 Agent（带人工确认）
function agentRun(userGoal, onProgress, onConfirm) {
  return new Promise(function(resolve, reject) {
    _agentState.isRunning = true;
    _agentState.currentStep = 0;
    _agentState.plan = agentPlan(userGoal);
    _agentState.totalSteps = _agentState.plan.length;
    _agentState.onConfirm = onConfirm;
    
    addAgentMessage('user', userGoal);
    addAgentMessage('system', '开始执行，共 ' + _agentState.totalSteps + ' 步');
    
    var executeNext = function() {
      if (!_agentState.isRunning) {
        resolve({ success: false, reason: '用户取消' });
        return;
      }
      
      if (_agentState.currentStep >= _agentState.totalSteps) {
        _agentState.isRunning = false;
        addAgentMessage('system', '✅ 全部执行完成！');
        resolve({ success: true, totalSteps: _agentState.totalSteps });
        return;
      }
      
      var step = _agentState.plan[_agentState.currentStep];
      _agentState.currentStep++;
      
      addAgentMessage('system', '📋 步骤 ' + _agentState.currentStep + '/' + _agentState.totalSteps + ': ' + step.description);
      
      onConfirm(step, function(approved, feedback) {
        if (!approved) {
          addAgentMessage('system', '⏭️ 用户跳过此步骤');
          executeNext();
          return;
        }
        
        onProgress('正在执行: ' + step.description);
        agentExecuteStep(step).then(function(result) {
          var reflection = agentReflect(step, result);
          if (reflection.approved) {
            addAgentMessage('system', '✅ ' + step.description + ' 完成');
          } else {
            addAgentMessage('error', '❌ ' + step.description + ' 失败: ' + reflection.feedback);
          }
          onProgress('');
          executeNext();
        }).catch(function(e) {
          addAgentMessage('error', '❌ 执行出错: ' + e.message);
          onProgress('');
          executeNext();
        });
      });
...
function addAgentMessage(type, text) {
  _agentState.chatHistory.push({ type: type, text: text, time: Date.now() });
  renderAgentChat();
}

function renderAgentChat() {
  var container = document.getElementById('agent-chat-container');
  if (!container) return;
  
  container.innerHTML = '';
  for (var i = 0; i < _agentState.chatHistory.length; i++) {
    var msg = _agentState.chatHistory[i];
    var div = document.createElement('div');
    div.className = 'agent-message agent-message-' + msg.type;
    var header = document.createElement('div');
    header.className = 'agent-message-header';
    header.textContent = msg.type === 'user' ? '你' : msg.type === 'assistant' ? 'Agent' : '系统';
...
function showAgentConfirm(step, callback) {
  var container = document.getElementById('agent-step-confirm');
  var info = document.getElementById('agent-step-info');
  var btnConfirm = document.getElementById('agent-btn-confirm');
  var btnSkip = document.getElementById('agent-btn-skip');
  var btnCancel = document.getElementById('agent-btn-cancel');
  
  if (!container || !info || !btnConfirm || !btnSkip || !btnCancel) return;
  
  info.textContent = '即将执行: ' + step.description + ' (' + step.tool + ')';
...
function initAgentUI() {
  // 注入 Agent CSS（动态创建 style 标签）
  var style = document.createElement('style');
  style.id = 'agent-custom-css';
  style.textContent = [
    '/* ===== Agent 主布局（类 ChatGPT）===== */',
    '.novel-writer-extension-root #tab-agent {',
    '  display: flex;',
    '  flex-direction: column;',
    '  height: 100%;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .agent-main {',
    '  flex: 1;',
    '  display: flex;',
    '  flex-direction: column;',
    '  height: 100%;',
    ...
    '',
    '/* 顶部状态栏 */',
    '.novel-writer-extension-root .agent-header {',
    '  display: flex;',
    '  align-items: center;',
    '  justify-content: space-between;',
    '  padding: 12px 20px;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .agent-header-title {',
    '  display: flex;',
    '  align-items: center;',
    '  gap: 10px;',
    '  font-weight: 600;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .agent-status-dot {',
    '  width: 10px;',
    '  height: 10px;',
    '  border-radius: 50%;',
    '  background: #10a37f;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .agent-status-dot.busy {',
    '  background: #f59e0b;',
    '  box-shadow: 0 0 6px rgba(245, 158, 11, 0.5);',
    '  animation: agent-pulse 1s infinite;',
    '}',
    '',
    '.novel-writer-extension-root .agent-status-dot.error {',
    '  background: #ef4444;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .agent-header-stats {',
    '  display: flex;',
    '  gap: 16px;',
    '  font-size: 13px;',
    '  color: #666;',
    '}',
    '',
    '.novel-writer-extension-root .agent-stat {',
    '  display: flex;',
    '  align-items: center;',
    '  gap: 4px;',
    '}',
    '',
    '/* 聊天区 */',
    '.novel-writer-extension-root .agent-chat-container {',
    '  flex: 1;',
    '  overflow-y: auto;',
    '  padding: 24px;',
    '  background: #f7f7f8;',
    ...
    '',
    '/* 消息气泡 */',
    '.novel-writer-extension-root .agent-message {',
    '  max-width: 75%;',
    '  padding: 12px 16px;',
    '  border-radius: 18px;',
    '  font-size: 14px;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .agent-message-system {',
    ...
    '}',
    '',
    '.novel-writer-extension-root .agent-message-user {',
    '  align-self: flex-end;',
    '  background: #10a37f;',
    '  color: white;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .agent-message-assistant {',
    '  align-self: flex-start;',
    '  background: white;',
    '  color: #1a1a1a;',
    '  border-bottom-left-radius: 6px;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .agent-message-error {',
    '  align-self: flex-start;',
    '  background: #fef2f2;',
    '  color: #991b1b;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .agent-message-tool {',
    '  align-self: flex-start;',
    '  background: #eff6ff;',
    '  color: #1e40af;',
    ...
    '',
    '/* 确认面板 */',
    '.novel-writer-extension-root .agent-step-confirm {',
    '  margin: 0 24px;',
    '  padding: 16px;',
    '  background: #fffbeb;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .agent-step-info {',
    '  margin-bottom: 12px;',
    '  font-size: 14px;',
    '  line-height: 1.5;',
    '  color: #92400e;',
    '}',
    '',
    '.novel-writer-extension-root .agent-step-actions {',
    '  display: flex;',
    '  gap: 8px;',
    '  flex-wrap: wrap;',
    '}',
    '',
    '.novel-writer-extension-root .agent-btn {',
    '  padding: 8px 16px;',
    '  border: none;',
    '  border-radius: 8px;',
    '  cursor: pointer;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .agent-btn:hover {',
    '  opacity: 0.9;',
    '  transform: translateY(-1px);',
    '}',
    '',
    '.novel-writer-extension-root .agent-btn-primary {',
    '  background: #10a37f;',
    '  color: white;',
    '}',
    '',
    '.novel-writer-extension-root .agent-btn-secondary {',
    '  background: white;',
    '  color: #374151;',
    '  border: 1px solid #d1d5db;',
    '}',
    '',
    '.novel-writer-extension-root .agent-btn-danger {',
    '  background: #ef4444;',
    '  color: white;',
    '}',
    '',
    '/* 输入区 */',
    '.novel-writer-extension-root .agent-input-area {',
    '  padding: 16px 24px;',
    '  background: white;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .agent-input-wrapper {',
    '  display: flex;',
    '  gap: 8px;',
    '  align-items: flex-end;',
    '  background: #f7f7f8;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .agent-input {',
    '  flex: 1;',
    '  min-height: 44px;',
    '  max-height: 120px;',
    '  padding: 8px 4px;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .agent-btn-send {',
    '  width: 36px;',
    '  height: 36px;',
    '  border-radius: 8px;',
    '  background: #10a37f;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .agent-btn-send:hover {',
    '  background: #0d8a6c;',
    '}',
    '',
    '.novel-writer-extension-root .agent-btn-send:disabled {',
    '  background: #9ca3af;',
    '  cursor: not-allowed;',
    '}',
    '',
    '.novel-writer-extension-root .agent-input-hint {',
    '  margin-top: 8px;',
    '  font-size: 12px;',
    '  color: #9ca3af;',
    '  text-align: center;',
  ...
  // 找到 tab 导航栏，添加 Agent tab 按钮
  var tabNav = document.querySelector('.tab-nav-container');
  if (tabNav) {
    var agentTab = document.createElement('button');
    agentTab.className = 'panel-tab-item active';
    agentTab.setAttribute('data-tab', 'tab-agent');
    agentTab.innerHTML = '<span class="tab-icon">🤖</span><span class="tab-text">Agent</span>';
    tabNav.appendChild(agentTab);
    
    // 绑定 tab 切换
    agentTab.addEventListener('click', function() {
      var tabs = tabNav.querySelectorAll('.panel-tab-item');
      tabs.forEach(function(t) { t.classList.remove('active'); });
      agentTab.classList.add('active');
      
      var panels = document.querySelectorAll('.panel-tab-panel');
      panels.forEach(function(p) { p.classList.remove('active'); });
      var agentPanel = document.getElementById('tab-agent');
      if (agentPanel) agentPanel.classList.add('active');
    });
  }
  
  // 创建 Agent panel
  var tabContent = document.querySelector('.panel-tab-content');
  if (tabContent) {
    var agentPanel = document.createElement('div');
    agentPanel.className = 'panel-tab-panel active';
    agentPanel.id = 'tab-agent';
      ...
      '<div class="agent-main">' +
        ...
        '<div class="agent-header">' +
          '<div class="agent-header-title">' +
            ...
            '<span>🤖 小说续写 Agent</span>' +
          '</div>' +
          '<div class="agent-header-stats">' +
            '<div class="agent-stat">📚 <span id="agent-chapter-count">0</span> 章</div>' +
            '<div class="agent-stat">🧠 <span id="agent-memory-count">0</span> 条记忆</div>' +
          '</div>' +
        '</div>' +
        // 聊天区
        '<div class="agent-chat-container" id="agent-chat-container"></div>' +
        // 确认面板
        '<div class="agent-step-confirm" id="agent-step-confirm">' +
          '<div class="agent-step-info" id="agent-step-info"></div>' +
          '<div class="agent-step-actions">' +
            '<button class="agent-btn agent-btn-primary" id="agent-btn-confirm">✅ 确认执行</button>' +
            '<button class="agent-btn agent-btn-secondary" id="agent-btn-skip">⏭️ 跳过此步</button>' +
            '<button class="agent-btn agent-btn-danger" id="agent-btn-cancel">❌ 取消任务</button>' +
          '</div>' +
        '</div>' +
        // 输入区
        '<div class="agent-input-area">' +
          '<div class="agent-input-wrapper">' +
            '<textarea class="agent-input" id="agent-input" placeholder="输入你的目标，例如：为前10章生成图谱，或续写第11章..."></textarea>' +
            '<button class="agent-btn agent-btn-send" id="agent-btn-send">➤</button>' +
          '</div>' +
          '<div class="agent-input-hint">每一步执行前都会请你确认 · 支持自然语言目标</div>' +
        '</div>' +
      '</div>';
    tabContent.appendChild(agentPanel);
    
    // 更新统计
    updateAgentStats();
    
    // 绑定发送按钮
    var sendBtn = document.getElementById('agent-btn-send');
    var input = document.getElementById('agent-input');
    if (sendBtn && input) {
      sendBtn.addEventListener('click', function() {
        var text = input.value.trim();
        if (!text || _agentState.isRunning) return;
        input.value = '';
        ...
        agentRun(
          ...
          function(progress) {
            if (statusDot) {
              statusDot.className = progress ? 'agent-status-dot busy' : 'agent-status-dot';
            }
          },
          function(step, callback) {
            showAgentConfirm(step, callback);
          }
        ).then(function(result) {
          if (statusDot) statusDot.className = 'agent-status-dot';
          updateAgentStats();
        }).catch(function(e) {
          addAgentMessage('error', 'Agent 运行出错: ' + e.message);
    ...
    // 欢迎消息
    addAgentMessage('system', '👋 你好！我是小说续写 Agent。\n\n我可以帮你：\n• 📖 列出/读取/总结章节\n• 🗺️ 为章节生成图谱\n• ✍️ 续写新章节\n• ✅ 检查章节一致性\n\n输入你的目标，我会自动规划步骤并逐步执行，每一步都会请你确认。');
  }
  
  console.log('[小说续写器] Agent UI 初始化完成（主界面布局）');
    
    // 自动切换到 Agent tab（设为默认）
    setTimeout(function() {
      var agentTab = document.querySelector('[data-tab="tab-agent"]');
      if (agentTab) agentTab.click();
    }, 200);
}

function updateAgentStats() {
  var chapterCount = document.getElementById('agent-chapter-count');
  var memoryCount = document.getElementById('agent-memory-count');
  if (chapterCount) {
    chapterCount.textContent = (_settingsCache.chapterList || []).length;
  }
  if (memoryCount) {
    memoryCount.textContent = _agentState.memory.length;
  }
}
...
function redesignLayout() {
  ...
  style.textContent = [
    ...
    '',
    '/* 左侧边栏（章节列表 + 功能导航）*/',
    '.novel-writer-extension-root .sidebar {',
    '  width: 280px;',
    '  min-width: 280px;',
    '  background: white;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .sidebar-header {',
    '  padding: 16px;',
    '  border-bottom: 1px solid #e5e5e5;',
    '  font-weight: 600;',
    '  font-size: 15px;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .sidebar-nav {',
    '  padding: 8px;',
    '  border-bottom: 1px solid #e5e5e5;',
    '}',
    '',
    '.novel-writer-extension-root .sidebar-nav-item {',
    '  display: flex;',
    '  align-items: center;',
    '  gap: 10px;',
    '  width: 100%;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .sidebar-nav-item:hover {',
    '  background: #f3f4f6;',
    '}',
    '',
    '.novel-writer-extension-root .sidebar-nav-item.active {',
    '  background: #e0f2fe;',
    '  color: #0369a1;',
    '  font-weight: 500;',
    '}',
    '',
    '.novel-writer-extension-root .sidebar-nav-icon {',
    '  font-size: 18px;',
    '}',
    '',
    '.novel-writer-extension-root .sidebar-chapters {',
    '  flex: 1;',
    '  overflow-y: auto;',
    '  padding: 8px;',
    '}',
    '',
    '.novel-writer-extension-root .sidebar-chapter-item {',
    '  padding: 10px 12px;',
    '  margin-bottom: 2px;',
    '  border-radius: 6px;',
    '  font-size: 13px;',
    ...
    '}',
    '',
    '.novel-writer-extension-root .sidebar-chapter-item:hover {',
    '  background: #f3f4f6;',
    '}',
    '',
    '.novel-writer-extension-root .sidebar-chapter-item.active {',
    '  background: #dbeafe;',
    '  color: #1e40af;',
    '  font-weight: 500;',
    '}',
    '',
    '/* 右侧主内容区 */',
    '.novel-writer-extension-root .main-content {',
    '  flex: 1;',
    '  display: flex;',
    '  flex-direction: column;',
    '  overflow: hidden;',
    ...
    '}',
    '',
    '/* 隐藏原来的 panel-tab-panel，我们用自己的 */',
    '.novel-writer-extension-root .panel-tab-panel {',
    '  display: none !important;',
    '}',
    '',
    '/* 阅读模式优化 */',
    '.novel-writer-extension-root .reading-mode .main-content {',
    '  background: #f5f5f0;',
    '}',
    '',
    '/* 隐藏 panel-header（我们自己做标题栏）*/',
    '.novel-writer-extension-root .panel-header {',
    '  display: none !important;',
    '}',
    '',
    '/* 隐藏 panel-close-btn（我们自己放）*/',
    '.novel-writer-extension-root .panel-close-btn {',
    '  position: fixed;',
    '  top: 16px;',
    '  right: 16px;',
  ...
  sidebar.innerHTML = 
    '<div class="sidebar-header">📚 小说续写器</div>' +
    '<div class="sidebar-nav">' +
      '<button class="sidebar-nav-item active" data-view="agent">' +
        '<span class="sidebar-nav-icon">🤖</span>' +
        '<span>Agent 助手</span>' +
      '</button>' +
      '<button class="sidebar-nav-item" data-view="reader">' +
        '<span class="sidebar-nav-icon">📖</span>' +
        '<span>阅读</span>' +
      '</button>' +
      '<button class="sidebar-nav-item" data-view="graph">' +
        '<span class="sidebar-nav-icon">🗺️</span>' +
        '<span>知识图谱</span>' +
      '</button>' +
      '<button class="sidebar-nav-item" data-view="write">' +
        '<span class="sidebar-nav-icon">✍️</span>' +
        '<span>续写创作</span>' +
      '</button>' +
      '<button class="sidebar-nav-item" data-view="chapter">' +
        '<span class="sidebar-nav-icon">📋</span>' +
        '<span>章节管理</span>' +
      '</button>' +
    '</div>' +
    '<div class="sidebar-chapters" id="sidebar-chapters"></div>';
  ...
  tabContent.appendChild(sidebar);
  ...
  console.log('[小说续写器] 全新布局初始化完成');
}

function renderSidebarChapters() {
  var container = document.getElementById('sidebar-chapters');
  if (!container) return;
  ...
  for (var i = 0; i < chapters.length; i++) {
    ...
    item.className = 'sidebar-chapter-item';
    item.textContent = ch.title || ('第' + (i+1) + '章');
    item.addEventListener('click', function(idx) {
      return function() {
        // 切换到阅读模式并打开章节
        switchView('reader');
        if (typeof openChapterReader === 'function') {
...
function switchView(view) {
  ...
  if (view === 'agent') {
    // Agent 视图：把之前的 Agent 面板放进来
    var agentPanel = document.createElement('div');
    agentPanel.id = 'tab-agent';
    agentPanel.className = 'panel-tab-panel active';
    agentPanel.style.display = 'flex';
    agentPanel.style.flex = '1';
    agentPanel.style.flexDirection = 'column';
    
    // 把原来的 agent-main 内容复制过来
    var oldAgentMain = document.querySelector('.agent-main');
    if (oldAgentMain) {
      agentPanel.appendChild(oldAgentMain.cloneNode(true));
    } else {
      // 重新初始化
      agentPanel.innerHTML = '<div style="padding: 40px; text-align: center; color: #666;">Agent 正在加载...</div>';
    }
    mainContent.appendChild(agentPanel);
    
  } else if (view === 'reader') {
    // 阅读视图
    var readerView = document.createElement('div');
    readerView.className = 'reader-view';
    readerView.style.flex = '1';
    readerView.style.overflow = 'auto';
    readerView.style.padding = '40px';
    readerView.style.maxWidth = '800px';
    readerView.style.margin = '0 auto';
    readerView.style.width = '100%';
    
    var chapters = _settingsCache.chapterList || [];
    if (chapters.length === 0) {
      readerView.innerHTML = '<div style="text-align: center; color: #999; padding: 80px 20px;"><div style="font-size: 48px; margin-bottom: 16px;">📖</div><div style="font-size: 16px; margin-bottom: 8px;">还没有章节</div><div style="font-size: 14px;">请先上传小说文件</div></div>';
    } else {
      var ch = chapters[Math.min(0, chapters.length - 1)];
      readerView.innerHTML = 
        '<h1 style="font-size: 24px; margin-bottom: 24px; color: #1a1a1a;">' + (ch.title || '第一章') + '</h1>' +
        '<div style="font-size: 17px; line-height: 1.9; color: #2d2d2d; white-space: pre-wrap;">' + (ch.content || '暂无内容') + '</div>';
    }
    mainContent.appendChild(readerView);
    
  } else {
    // 其他视图：把原来的 tab panel 内容复制过来
    var oldPanel = document.getElementById('tab-' + view);
    if (oldPanel) {
      var newPanel = oldPanel.cloneNode(true);
      newPanel.style.display = 'block';
      newPanel.style.flex = '1';
      newPanel.style.overflow = 'auto';
      mainContent.appendChild(newPanel);
    } else {
      mainContent.innerHTML = '<div style="padding: 40px; text-align: center; color: #666;">' + view + ' 视图开发中...</div>';
    }
  }
}

// 在 openNovelWriter 完成后调用
var _originalRedesign = openNovelWriter;
openNovelWriter = function(opts) {
  return _originalRedesign(opts).then(function() {
    // 延迟初始化新布局
    setTimeout(redesignLayout, 200);
  });
};

function scriptEntryPoint() {
    try { console.log('[小说续写器] scriptEntryPoint'); } catch(_) {}

    // ★ 关键合规：本脚本【绝不】注册任何卸载钩子（不含卸载事件名）。
    //   JSR 运行时对“自己处理卸载”的脚本会关闭整套沙箱清理保护（不代理 window.parent、
    //   不给 DOM 打标、卸载时不回收 DOM/不还原主窗口属性），那会导致悬浮球、全屏 iframe、
    //   主窗口监听永久残留在酒馆主窗口，污染并拖垮 JSR，最终触发其设置保护把脚本清空。
    //   因此卸载清理完全交给 JSR 沙箱；这里只在刷新/关闭前用 beforeunload 强制落盘数据。
    try {
      const flushOnUnload = function () {
        try { _flushSettings(); } catch(_) {}
      };
      // 主窗口刷新/关闭（全局脚本随整页卸载，主窗口 beforeunload 可靠触发）
    ...
    // ★ 主动卸载清理：脚本禁用/切卡/刷新时自己回收悬浮球、iframe、样式与全局监听。
    //   借鉴写卡器：注册 pagehide 后 JSR 会关闭沙箱保护，但自己清理更可靠、时机更可控，
    //   避免"脚本关了球还在，必须刷新页面"的体验问题。
    try {
      window.addEventListener('pagehide', function () {
        try { cleanupNovelWriter(); } catch(_) {}
      });
    } catch(_) {}

    // 仅供本脚本运行时 iframe 内部调用（不写 window.parent，iframe 销毁即回收，不污染主窗口）
    try { window.openNovelWriter = openNovelWriter; } catch(_) {}
    try { window.__novelWriterIsOpen = function () { return _novelWriterOpened; }; } catch(_) {}

    // tryInit 内部第一步即创建常驻悬浮球，这里不再重复创建（避免重复建球）
    // 脚本按钮注册（环境支持时的兜底入口）+ 常驻悬浮球
    tryInit();

    // ★ 后台静默初始化 iframe 注入 UI（失败不影响入口；面板打开时再即时创建亦可）
    (async function bootstrap() {
      try {
        await openNovelWriter({ silent: true });
        console.log('[小说续写器] UI 已注入 iframe，悬浮球常驻显示');
      } catch (e) {
        console.warn('[小说续写器] 自动初始化失败（入口按钮仍可用）:', e);
      }
    })();
  }
