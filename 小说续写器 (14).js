...
(function() {
  ...
  function _setScriptVars(obj) {
    ...
    // 1) localStorage（唯一权威源）
    try {
      var raw = localStorage.getItem(LOCAL_VARS_KEY);
      var stored = raw ? JSON.parse(raw) : {};
      var next = Object.assign({}, stored, incoming);
      localStorage.setItem(LOCAL_VARS_KEY, JSON.stringify(next));
    } catch (_) {}
    // 2) 酒馆变量：尽力写一次，失败也不管（不读它就行）
    try {
      var pWin = (typeof window !== 'undefined' && window.parent) ? window.parent : window;
      if (pWin && typeof pWin.replaceVariables === 'function') {
        pWin.replaceVariables(incoming, { type: 'script' });
      }
    } catch (_) {}
    return true;
  }
  ...
  function _buildMiniJQ(win, doc) {
    ...
    Q.fn = {
      ...
      prop(k, v) { if (v === undefined) return this.els[0] ? this.els[0][k] : undefined; this.els.forEach(e => e[k] = v); return this; },
...
// 同步持久化：每次修改立即落盘（与写卡器一致），避免"300ms 内刷新就丢"
const _saveTimer = { id: null };
function saveSettingsDebounced() {
    // 写卡器就是每次都同步写，我们照抄，避免"300ms 内刷新就丢"
...
function cleanupNovelWriter() {
  ...
  function addNovelWriterFloatingButton() {
    try {
      ...
      // ★ 精确复刻原版 #novel-writer-float-ball 外观：方形白底 + 深色描边 + 书本 SVG + 硬投影
      btn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:1.25em;height:1.25em;filter:drop-shadow(0 2px 4px rgba(0,0,0,.3));"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>';
      btn.style.cssText = 'position:fixed;z-index:2147483647;width:' + FLOAT_BALL_SIZE + 'px;height:' + FLOAT_BALL_SIZE + 'px;border-radius:0;display:flex;align-items:center;justify-content:center;background:#f8f8f8;color:#222222;border:3px solid #222222;cursor:grab;font-size:28.8px;line-height:1;box-shadow:6px 6px 0px rgba(0,0,0,.25);transition:all .15s ease;user-select:none;-webkit-user-select:none;touch-action:none;font-family:system-ui,-apple-system,"Segoe UI",sans-serif;';
      ...
      btn.addEventListener('pointercancel', function () {
        drag = null; btn.style.cursor = 'grab'; btn.style.transform = '';
      });
      btn.addEventListener('mouseenter', function () { btn.style.transform = 'scale(1.1)'; btn.style.boxShadow = '8px 8px 0px rgba(0,0,0,.3)'; });
      btn.addEventListener('mouseleave', function () { btn.style.transform = ''; btn.style.boxShadow = '6px 6px 0px rgba(0,0,0,.25)'; });
  ...
  function scriptEntryPoint() {
    ...
    // 页面卸载前：强制落盘一次（现在是同步写，此为双保险），只清兜底按钮，不动 UI 与悬浮球
    try {
      window.addEventListener('pagehide', function () {
        try {
          ...
          if (typeof _persistSettings === 'function') _persistSettings(); // 强制落盘
        } catch(_) {}
        // 只清按钮/模态，不动 #novel-writer-extension-root（让 UI 与悬浮球常驻）
        try {
          var doc = _pDoc();
          var btn = doc.getElementById(SCRIPT_ID + '-btn');
          if (btn) btn.remove();
    ...
    // ★ 入口优先：悬浮球立即创建（不依赖 iframe 初始化，刷新后第一时间可见可点）
