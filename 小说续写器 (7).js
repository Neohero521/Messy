...
async function openNovelWriter(opts) {
    ...
    if (_novelWriterOpened) {
        ...
        try {
            ...
            if (!silent) toastr.info('小说续写器已打开');
        } catch (_) {}
        return;
    }
    ...
    $(window.parent).off('resize.novelWriter').on('resize.novelWriter', handleWindowResize);
        ...
        if (!silent) toastr.success('小说续写器已打开');
    } catch (error) {
...
// 注意：pagehide 时只做强制落盘，不删除 UI DOM（悬浮球 #novel-writer-float-ball 在 UI_HTML 内），
// 否则刷新/切页时悬浮球会闪没；真正需要彻底清 DOM 的场景再单独放开下面注释
function cleanupNovelWriter() {
    try {
        ...
        // 清理注入的 UI DOM（走父页面 document）——刷新/切页不删，避免悬浮球消失
        // try {
        //     var doc = _pDoc();
        ...
        _novelWriterOpened = false;
        try { console.log('[小说续写器] 已卸载清理完成（仅落盘，保留 UI）'); } catch(_) {}
    } catch(e) {
  ...
  // 悬浮图标状态常量（位置/缩放/动画记忆，刷新后恢复）
  const FLOAT_ICON_STORAGE_KEY = SCRIPT_ID + ':float-icon';
  const FLOAT_ICON_BASE_SIZE = 64;
  ...
  function addNovelWriterFloatingButton() {
    try {
      ...
      // ---- 幂等：清掉旧实例 ----
      [ '-btn', '-float-style', '-float-menu' ].forEach(function (suffix) {
        const el = doc.getElementById(SCRIPT_ID + suffix);
        if (el) el.remove();
      });

      // ---- 状态（localStorage 持久化）----
      const st = { posX: null, posY: null, scale: 100, animEnabled: true };
      try {
        const raw = localStorage.getItem(FLOAT_ICON_STORAGE_KEY);
        if (raw) {
          const saved = JSON.parse(raw);
          if (saved) {
            if (typeof saved.scale === 'number') st.scale = Math.max(60, Math.min(200, saved.scale));
            if (saved.animEnabled !== undefined) st.animEnabled = !!saved.animEnabled;
            if (typeof saved.posX === 'number') st.posX = saved.posX;
            if (typeof saved.posY === 'number') st.posY = saved.posY;
          }
        }
      } catch (_) {}
      function saveSt() {
        try { localStorage.setItem(FLOAT_ICON_STORAGE_KEY, JSON.stringify(st)); } catch (_) {}
      }
      ...
      styleEl.textContent =
        '#' + SCRIPT_ID + '-btn{position:fixed;z-index:2147483647;display:flex;align-items:center;justify-content:center;width:64px;height:64px;border-radius:50%;background:linear-gradient(135deg,#f97316,#ea580c);color:#fff;border:2px solid #7c2d12;cursor:grab;font-size:32px;line-height:1;box-shadow:0 6px 24px rgba(249,115,22,.5),2px 2px 0 #7c2d12;transition:box-shadow .2s ease;user-select:none;-webkit-user-select:none;touch-action:none;font-family:system-ui,-apple-system,"Segoe UI",sans-serif;}' +
        '#' + SCRIPT_ID + '-btn:hover{box-shadow:0 10px 32px rgba(249,115,22,.7),2px 2px 0 #7c2d12;}' +
        '#' + SCRIPT_ID + '-btn.dragging{cursor:grabbing;}' +
        '#' + SCRIPT_ID + '-btn .nw-icon{display:block;pointer-events:none;transition:transform .2s ease;}' +
        '#' + SCRIPT_ID + '-btn.anim-on .nw-icon{animation:nw-breath 3s ease-in-out infinite;}' +
        '@keyframes nw-breath{0%,100%{transform:translateY(0) scale(1)}50%{transform:translateY(-3px) scale(1.08)}}' +
        '#' + SCRIPT_ID + '-float-menu{position:fixed;z-index:2147483646;display:none;flex-direction:column;gap:2px;min-width:184px;padding:8px 4px;background:rgba(28,20,16,.96);border:1px solid rgba(249,115,22,.32);border-radius:12px;box-shadow:0 16px 48px rgba(0,0,0,.65);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);font-family:system-ui,-apple-system,"Segoe UI",sans-serif;}' +
        '#' + SCRIPT_ID + '-float-menu .nw-fm-item{display:flex;align-items:center;gap:10px;width:100%;padding:8px 14px;border:none;background:transparent;border-radius:8px;color:#d8ccc2;font-size:12.5px;font-family:inherit;cursor:pointer;text-align:left;transition:background .15s ease,color .15s ease;}' +
        '#' + SCRIPT_ID + '-float-menu .nw-fm-item:hover{background:rgba(249,115,22,.18);color:#f5ded0;}' +
        '#' + SCRIPT_ID + '-float-menu .nw-fm-item.danger{color:#d08080;}' +
        '#' + SCRIPT_ID + '-float-menu .nw-fm-item.danger:hover{background:rgba(180,60,60,.25);color:#ffb0a0;}' +
        '#' + SCRIPT_ID + '-float-menu .nw-fm-divider{height:1px;background:rgba(255,255,255,.08);margin:4px 8px;}' +
        '#' + SCRIPT_ID + '-float-menu .nw-fm-scale{padding:6px 14px 10px;display:flex;flex-direction:column;gap:4px;}' +
        '#' + SCRIPT_ID + '-float-menu .nw-fm-scale label{font-size:11px;color:#a09088;display:flex;justify-content:space-between;align-items:center;}' +
        '#' + SCRIPT_ID + '-float-menu .nw-fm-scale .nw-fm-val{color:#f97316;font-weight:600;}' +
        '#' + SCRIPT_ID + '-float-menu .nw-fm-scale input[type=range]{width:100%;accent-color:#f97316;cursor:pointer;}';
      ...
      btn.title = '小说续写器 · 单击打开 / 拖拽移动 / 右键菜单 / 滚轮缩放';
      ...
      menu.innerHTML =
        '<button class="nw-fm-item" data-action="open"><span>📖</span> 打开小说续写器</button>' +
        '<button class="nw-fm-item" data-action="toggle-anim"><span>🎬</span> 动画 <span class="nw-fi-state">开</span></button>' +
        '<div class="nw-fm-divider"></div>' +
        '<div class="nw-fm-scale">' +
        '<label>大小 <span class="nw-fm-val">100%</span></label>' +
        '<input type="range" min="60" max="200" step="10" value="100">' +
        '</div>' +
        '<div class="nw-fm-divider"></div>' +
        '<button class="nw-fm-item" data-action="reset-pos"><span>📍</span> 重置位置</button>' +
        '<button class="nw-fm-item danger" data-action="hide"><span>✕</span> 隐藏图标（刷新后恢复）</button>';

      doc.body.appendChild(btn);
      doc.body.appendChild(menu);

      const slider = menu.querySelector('input[type=range]');
      const scaleVal = menu.querySelector('.nw-fm-val');
      const animStateLabel = menu.querySelector('.nw-fi-state');

      // ---- 位置 / 缩放 ----
      function applyPosition() {
        const w = pWin.innerWidth || doc.documentElement.clientWidth || 0;
        const h = pWin.innerHeight || doc.documentElement.clientHeight || 0;
        const bw = btn.offsetWidth || FLOAT_ICON_BASE_SIZE;
        const bh = btn.offsetHeight || FLOAT_ICON_BASE_SIZE;
        ...
        btn.style.left = Math.round(st.posX) + 'px';
        btn.style.top = Math.round(st.posY) + 'px';
      }
      function applyScale() {
        const s = st.scale / 100;
        const sz = Math.round(FLOAT_ICON_BASE_SIZE * s);
      ...
      function applyAnim() {
        if (st.animEnabled) btn.classList.add('anim-on');
        else btn.classList.remove('anim-on');
      ...
      function onMove(e) {
        if (!drag) return;
        if (Math.abs(e.clientX - drag.sx) > MOVE_THRESHOLD || Math.abs(e.clientY - drag.sy) > MOVE_THRESHOLD) drag.moved = true;
        const w = pWin.innerWidth, h = pWin.innerHeight;
        const bw = btn.offsetWidth || FLOAT_ICON_BASE_SIZE;
        const bh = btn.offsetHeight || FLOAT_ICON_BASE_SIZE;
      ...
      function onUp() {
        ...
        if (wasClick) {
          try { openNovelWriter(); } catch (err) { showToast('打开失败: ' + (err && err.message ? err.message : err), 'error'); }
        } else {
          saveSt();
        }
      }
      btn.addEventListener('pointerdown', function (e) { e.preventDefault(); onDown(e); });
      ...
      menu.addEventListener('click', function (e) {
        ...
        if (act === 'open') {
          ...
          applyPosition(); saveSt(); hideMenu();
        } else if (act === 'hide') {
          hideMenu(); btn.style.display = 'none';
        }
      });
      slider.addEventListener('input', function () {
        st.scale = Math.max(60, Math.min(200, parseFloat(slider.value) || 100));
        applyScale(); syncScaleUI(); saveSt();
      });

      // ---- 滚轮缩放 ----
      btn.addEventListener('wheel', function (e) {
        e.preventDefault(); e.stopPropagation();
        st.scale = Math.max(60, Math.min(200, st.scale + (e.deltaY > 0 ? -10 : 10)));
        applyScale(); syncScaleUI(); saveSt();
      }, { passive: false });
      ...
      pWin.addEventListener('resize', function () {
        ...
        _resizeTimer = setTimeout(function () { _resizeTimer = null; applyPosition(); }, 100);
      });
      ...
      try { console.warn('[小说续写器] addFloatingButton:', e && e.message); } catch (_) {}
      return false;
    }
  }
  ...
  function tryInit() {
    ...
    if (retryCount < 10) {
      ...
      addNovelWriterFloatingButton(); // 极端兜底：加个📖按钮
    }
  }
  ...
  function scriptEntryPoint() {
    ...
    // ★ 核心：脚本启动直接注入 UI，让悬浮球自然显示（不打开面板、不弹 toast）
    (async function bootstrap() {
      try {
        await openNovelWriter({ silent: true });
        console.log('[小说续写器] UI 已注入，悬浮球常驻显示');
      } catch (e) {
        console.warn('[小说续写器] 自动初始化失败，回退按钮入口:', e);
        tryInit(); // 仅失败时退回到"按钮"方式
      }
    })();
  }
