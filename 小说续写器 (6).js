...
(function() {
  ...
  function _getScriptVars() {
    ...
    try {
      ...
      if (pWin && typeof pWin.getVariables === 'function') {
        var vars = pWin.getVariables({ type: 'script' });
        if (vars && typeof vars === 'object' && Object.keys(vars).length > 0) {
          return vars;
        }
      }
    } catch(_) {}
    // 2) 酒馆变量为空/不可用 → 回退 localStorage（刷新不丢的关键）
    try {
  ...
  function _setScriptVars(obj) {
    ...
    // 1) 酒馆脚本变量：先读 → 合并 → 再写，绝不覆盖其他 key
    try {
      var pWin = (typeof window !== 'undefined' && window.parent) ? window.parent : window;
      if (pWin && typeof pWin.replaceVariables === 'function') {
        var merged = {};
        if (typeof pWin.getVariables === 'function') {
          try {
            var existing = pWin.getVariables({ type: 'script' }) || {};
            merged = Object.assign({}, existing);
          } catch(_) {}
        }
        Object.assign(merged, incoming);
        pWin.replaceVariables(merged, { type: 'script' });
        ok = true;
      }
    } catch(_) {}

    // 2) localStorage：同样先读 → 合并 → 再写，保证刷新后一定能恢复
    try {
      var raw = localStorage.getItem(SCRIPT_ID + ':vars');
      var stored = raw ? JSON.parse(raw) : {};
      var next = Object.assign({}, stored, incoming);
      localStorage.setItem(SCRIPT_ID + ':vars', JSON.stringify(next));
    } catch(_) {}
    return ok;
  }
...
function cleanupNovelWriter() {
  ...
  function scriptEntryPoint() {
    ...
    // 卸载前：先同步强制落盘（覆盖防抖 300ms 期间就刷新的极端情况），再走清理
    try {
