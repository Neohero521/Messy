...
(function() {
  ...
  function _buildMiniJQ(win, doc) {
    ...
    Q.fn = {
      ...
      addClass(c) { this.els.forEach(e => e.classList.add(c)); return this; },
      removeClass(c) { this.els.forEach(e => e.classList.remove(c)); return this; },
      toggleClass(c, force) { this.els.forEach(e => e.classList.toggle(c, force)); return this; },
      hasClass(c) { return this.els.some(e => e.classList.contains(c)); },
      css(k, v) {
...
function cleanupNovelWriter() {
  ...
  // 父页面常驻悬浮球（UI 已搬入 iframe，这里只需一个轻量入口按钮）
  function addNovelWriterFloatingButton() {
    try {
      ...
      btn.textContent = '📖';
      // ★ 简化样式：去掉动画，位置固定右侧
      btn.style.cssText = 'position:fixed;bottom:80px;right:20px;z-index:2147483647;width:52px;height:52px;border-radius:50%;display:flex;align-items:center;justify-content:center;padding:0;background:linear-gradient(135deg,#f97316,#ea580c);color:#fff;border:2px solid #7c2d12;cursor:pointer;font-size:24px;line-height:1;box-shadow:0 6px 24px rgba(249,115,22,.5),2px 2px 0 #7c2d12;font-family:system-ui,-apple-system,"Segoe UI",sans-serif;transition:box-shadow .15s ease;';
      btn.onmouseenter = function () { btn.style.boxShadow = '0 8px 28px rgba(249,115,22,.65),2px 2px 0 #7c2d12'; };
  ...
  function scriptEntryPoint() {
    ...
    // ★ 核心：脚本启动直接创建 iframe 注入 UI；父页面保留轻量 📖 入口（面板关闭时可见可点）
    (async function bootstrap() {
      try {
        await openNovelWriter({ silent: true });
        addNovelWriterFloatingButton(); // 父页面常驻入口
