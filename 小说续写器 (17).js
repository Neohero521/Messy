...
function cleanupNovelWriter() {
  ...
  function addNovelWriterFloatingButton() {
    try {
      ...
      // ★ 注入作用域样式：1:1 复刻原版 .float-ball / .ball-inner / .ball-icon（含 border-box，避免在酒馆父页面被撑大）
      ...
      styleEl.textContent =
        ...
        'cursor:grab;touch-action:none;user-select:none;-webkit-user-select:none;transition:all .15s ease;}' +
      ...
      btn.title = '小说续写器 · 拖拽移动 · 单击打开';
      // 与原版 float-ball 完全相同的 DOM 结构和书本 SVG
      ...
      function applyPos() {
        ...
        if (st.x == null || st.y == null) { st.x = w - FLOAT_BALL_SIZE - 20; st.y = Math.round(h / 2 - FLOAT_BALL_SIZE / 2); }
        st.x = Math.max(4, Math.min(w - FLOAT_BALL_SIZE - 4, st.x));
        st.y = Math.max(4, Math.min(h - FLOAT_BALL_SIZE - 4, st.y));
      ...
      btn.addEventListener('pointermove', function (e) {
        ...
        st.x = Math.max(4, Math.min(w - FLOAT_BALL_SIZE - 4, e.clientX - drag.ox));
        st.y = Math.max(4, Math.min(h - FLOAT_BALL_SIZE - 4, e.clientY - drag.oy));
        btn.style.left = Math.round(st.x) + 'px';
        btn.style.top = Math.round(st.y) + 'px';
      });
