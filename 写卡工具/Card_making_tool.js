...
`;
  ...
  // 配套正则1：prompt-only，从发送给AI的提示词中移除占位符（AI不需要看到它）
  // 配套正则2：display-only，将占位符替换为下方HTML（用 ```html 代码块包裹，ST会渲染）
  // 设计要点：监听 MVU VARIABLE_INITIALIZED/VARIABLE_UPDATE_ENDED 事件，动态读取 stat_data 填充
  var MVU_STATUS_BAR_HTML = '<!doctype html>\n<html lang="zh-CN">\n<head>\n  <meta charset="UTF-8">\n  <style>\n    :root { --mvu-s-cold: #325c9d; --mvu-s-sky: #4eb9e7; --mvu-s-soft: #f5b7c9; --mvu-s-hot: #bd2d3a; }\n    body { margin: 0; padding: 0; font-family: "Microsoft YaHei", "PingFang SC", system-ui, sans-serif; color: #18211f; }\n    .mvu-s-card-status { width: 100%; box-sizing: border-box; border: 1px solid rgba(31,45,42,.13); border-radius: 8px; background: linear-gradient(135deg, rgba(247,245,239,.96), rgba(230,236,232,.96)); padding: 10px; box-shadow: 0 12px 30px rgba(21,29,27,.12); }\n    .mvu-s-card-status-head { display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; margin-bottom: 10px; }\n    .mvu-s-card-status-head h3 { margin: 0; font-size: 15px; line-height: 1.3; font-weight: 700; }\n    .mvu-s-card-status-head p { margin: 3px 0 0; color: #66736d; font-size: 11px; line-height: 1.4; }\n    .mvu-s-card-status-scene { display: grid; gap: 3px; min-width: 96px; color: #66736d; font-size: 11px; text-align: right; }\n    .mvu-s-role-list { display: grid; gap: 8px; }\n    .mvu-s-status-role { position: relative; isolation: isolate; display: grid; grid-template-columns: 1fr; align-items: center; min-height: 82px; overflow: hidden; border: 1px solid rgba(255,255,255,.58); border-radius: 8px; background: linear-gradient(135deg, rgba(255,255,255,.72), rgba(238,242,239,.72)); box-shadow: inset 0 0 0 1px rgba(31,45,42,.05); }\n    .mvu-s-status-role-body { display: grid; gap: 9px; min-width: 0; padding: 12px; }\n    .mvu-s-role-name { margin: 0; overflow: hidden; color: currentColor; font-size: 16px; line-height: 1.25; text-overflow: ellipsis; white-space: nowrap; }\n    .mvu-s-affection-row { display: grid; grid-template-columns: 58px minmax(88px,1fr) 40px; align-items: center; gap: 9px; min-width: 0; }\n    .mvu-s-affection-label { font-size: 12px; color: #66736d; }\n    .mvu-s-affection-bar { height: 8px; border-radius: 4px; background: rgba(31,45,42,.08); overflow: hidden; }\n    .mvu-s-affection-fill { height: 100%; border-radius: 4px; background: var(--mvu-s-sky); transition: width .4s ease; }\n    .mvu-s-affection-value { font-size: 12px; color: #18211f; text-align: right; font-variant-numeric: tabular-nums; }\n  </style>\n</head>\n<body>\n  <section class="mvu-s-card-status">\n    <div class="mvu-s-card-status-head">\n      <div>\n        <h3>角色状态</h3>\n        <p>CURRENT STATUS</p>\n      </div>\n      <div class="mvu-s-card-status-scene">\n        <span id="mvu-s-status-time">开局</span>\n        <span id="mvu-s-status-place">待定</span>\n      </div>\n    </div>\n    <div id="mvu-s-role-list" class="mvu-s-role-list"></div>\n  </section>\n  <script>\n    function errorCatched(fn){ return function(){ try { fn.apply(this, arguments); } catch(e){ console.error("[mvu-s-status]", e); } }; }\n    function getStatData(){ try { return Mvu.getVar("stat_data") || {}; } catch(e){ return {}; } }\n    function clamp(v, lo, hi){ return Math.max(lo, Math.min(hi, v)); }\n    function affColor(v){ if (v <= 30) return "var(--mvu-s-cold)"; if (v <= 70) return "var(--mvu-s-sky)"; return "var(--mvu-s-hot)"; }\n    function renderRole(name, info){\n      var aff = clamp(Number(info && info["好感度"] != null ? info["好感度"] : 0), 0, 100);\n      return \'<div class="mvu-s-status-role"><div class="mvu-s-status-role-body">\'+\n        \'<h4 class="mvu-s-role-name">\' + name + \'</h4>\'+\n        \'<div class="mvu-s-affection-row">\'+\n          \'<span class="mvu-s-affection-label">好感度</span>\'+\n          \'<div class="mvu-s-affection-bar"><div class="mvu-s-affection-fill" style="width:\' + aff + \'%;background:\' + affColor(aff) + \'"></div></div>\'+\n          \'<span class="mvu-s-affection-value">\' + aff + \'</span>\'+\n        \'</div></div></div>\';\n    }\n    function init(){\n      var stat = getStatData();\n      var world = stat["世界"] || {};\n      var timeEl = document.getElementById("mvu-s-status-time");\n      var placeEl = document.getElementById("mvu-s-status-place");\n      if (timeEl && world["当前时间"] != null) timeEl.textContent = world["当前时间"];\n      if (placeEl && world["当前地点"] != null) placeEl.textContent = world["当前地点"];\n      var list = document.getElementById("mvu-s-role-list");\n      if (!list) return;\n      var html = "";\n      Object.keys(stat).forEach(function(k){\n        if (k === "世界") return;\n        var info = stat[k];\n        if (info && typeof info === "object") html += renderRole(k, info);\n      });\n      list.innerHTML = html || \'<div style="color:#999;font-size:12px;text-align:center;padding:12px">暂无角色数据</div>\';\n    }\n    $(errorCatched(init));\n    try {\n      eventOn(Mvu.events.VARIABLE_INITIALIZED, errorCatched(init));\n      eventOn(Mvu.events.VARIABLE_UPDATE_ENDED, errorCatched(init));\n    } catch(e) {}\n  <\\/script>\n</body>\n</html>';
