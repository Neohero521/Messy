$(() => {
  ...
  function showToast(msg, type) {
    ...
    try {
      if (typeof toastr !== 'undefined' && toastr && toastr[type]) toastr[type](msg);
      else if (typeof toastr !== 'undefined' && toastr) toastr.info(msg);
      else alert(msg);
  ...
  function createModalIframe() {
    return new Promise(function(resolve, reject) {
      try {
        var old = document.getElementById(SCRIPT_ID + '-modal');
        if (old) old.remove();
...
`;
  ...
  function closeModal() {
    try { var m = document.getElementById(SCRIPT_ID + '-modal'); if (m) m.remove(); } catch(e) {}
  }
      ...
      function showJsonModal(jsonStr) {
        ...
        doc.getElementById('downloadJson').addEventListener('click', function() {
          ...
          function tryParentBlob() {
            if (done) return;
            try {
              var pBlob = new Blob([content], {type: 'application/json;charset=utf-8'});
              var pUrl = (URL || webkitURL).createObjectURL(pBlob);
              ...
              pa.click();
              setTimeout(function() {
                try { document.body.removeChild(pa); } catch(_) {}
                try { (URL || webkitURL).revokeObjectURL(pUrl); } catch(_) {}
              }, 5000);
                done = true;
                showToast('下载已开始', 'success');
              }
