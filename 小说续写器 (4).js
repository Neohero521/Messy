  ...
  function _setScriptVars(obj) {
    ...
    try {
      ...
      if (pWin && typeof pWin.replaceVariables === 'function') {
        ...
        pWin.replaceVariables(obj || {}, { type: 'script' });
        ok = true;
      }
    } catch(_) {}
...
function getActivePresetParams() {
    ...
    if (settings.enableAutoParentPreset) {
        ...
        if (Object.keys(presetParams).length === 0) {
            ...
            const presetSettingsCandidates = [
                ...
                window.parent?.presetSettings,
                window.parent?.SillyTavern?.getContext?.()?.presetSettings
            ];
        ...
        if (Object.keys(presetParams).length === 0) {
            ...
            const oai = window.parent?.oai_settings;
            const pwr = window.parent?.power_user;
    ...
    const supplementalSources = [
        window.parent?.SillyTavern?.presetManager?.currentPreset?.data,
        context?.presetSettings,
        window.parent?.presetSettings,
        context?.generation_settings,
        context?.textCompletionSettings,
        window.parent?.generation_params
    ];
...
function cleanupNovelWriter() {
  ...
  function addNovelWriterFloatingButton() {
    try {
      ...
      btn.textContent = '📖';
      btn.style.cssText = 'position:fixed;bottom:80px;right:20px;z-index:2147483647;width:52px;height:52px;border-radius:50%;display:flex;align-items:center;justify-content:center;padding:0;background:linear-gradient(135deg,#f97316,#ea580c);color:#fff;border:2px solid #7c2d12;cursor:pointer;font-size:24px;line-height:1;box-shadow:0 6px 24px rgba(249,115,22,.5),2px 2px 0 #7c2d12;transition:transform .15s,box-shadow .15s;font-family:system-ui,-apple-system,"Segoe UI",sans-serif;';
      btn.onmouseover = function() { btn.style.transform = 'scale(1.08)'; btn.style.boxShadow = '0 8px 28px rgba(249,115,22,.65),2px 2px 0 #7c2d12'; };
      btn.onmouseout  = function() { btn.style.transform = 'scale(1)'; btn.style.boxShadow = '0 6px 24px rgba(249,115,22,.5),2px 2px 0 #7c2d12'; };
