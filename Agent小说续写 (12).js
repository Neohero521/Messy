...
(function() {
  ...
  const SCRIPT_VERSION = 'v5.6';  // 版本标识：标题栏可见 + console 输出，用于确认加载的是最新文件
...
@media (max-width: 720px) {
  .agent-root {
    ...
    font-size: 15px;
    /* 宽度兜底：iframe 缩放/容器场景下不依赖 100vw，避免超宽或缩窄 */
    width: 100%;
    height: 100%;
  }
  ...
  /* Bug6：安全区只加在状态面板（侧栏 footer 已自带），避免双重叠加 */
  .agent-status { padding-bottom: env(safe-area-inset-bottom, 0px); }
  /* 展开态 */
  ...
  function bindAllEvents() {
    ...
    (function bindSwipeTabs() {
      ...
      function finishSwipe() {
        ...
        el.classList.remove('swiping'); // 恢复过渡；内联 transform 仍占住当前手势位置
        el.style.touchAction = '';      // Bug10：还原触摸动作
        var goOther = false;
        ...
        // 双 rAF：先按手指离开位置绘制一帧（无跳变），再清内联 transform 触发 CSS 过渡归位
      ...
      el.addEventListener('touchmove', function (e) {
        ...
        if (lock !== 'horizontal') {
          ...
          // 水平意图明显强于垂直才接管，保证聊天/阅读/图谱的上下滚动不受影响
          if (Math.abs(rawDx) > Math.abs(rawDy) * 1.2) {
            lock = 'horizontal';
            el.classList.add('swiping');
            el.style.touchAction = 'pan-y';  // Bug10：锁定水平时收窄触摸动作，消除 iOS 一帧纵向回弹
