...
function cleanupNovelWriter() {
  ...
  function addNovelWriterFloatingButton() {
    try {
      ...
      btn.textContent = '📖';
      // 原版 float-ball 外观：浅色卡片底 + 深色描边 + 圆球（沿用原 CSS 变量默认值）
      btn.style.cssText = 'position:fixed;z-index:2147483647;width:' + FLOAT_BALL_SIZE + 'px;height:' + FLOAT_BALL_SIZE + 'px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:#f8f8f8;color:#222222;border:3px solid #222222;cursor:grab;font-size:28px;line-height:1;box-shadow:0 6px 20px rgba(0,0,0,.25);transition:box-shadow .2s ease,transform .2s ease;user-select:none;-webkit-user-select:none;touch-action:none;font-family:system-ui,-apple-system,"Segoe UI",sans-serif;';
      ...
      btn.addEventListener('mouseenter', function () { btn.style.boxShadow = '0 10px 30px rgba(0,0,0,.4)'; });
      btn.addEventListener('mouseleave', function () { btn.style.boxShadow = '0 6px 20px rgba(0,0,0,.25)'; });
