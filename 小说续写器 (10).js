...
function cleanupNovelWriter() {
  ...
  // 父页面常驻悬浮球（橙球样式，仿原 float-ball 外观；无动画，位置固定右下角）
  // 入口不依赖 iframe 创建成功：脚本启动立即挂载，永远有可点击入口
  function addNovelWriterFloatingButton() {
    try {
      ...
      btn.title = '小说续写器';
      btn.textContent = '📖';
      // ★ 简化样式：橙球外观，去掉动画，位置固定右侧
      btn.style.cssText = 'position:fixed;right:20px;bottom:80px;z-index:2147483647;width:56px;height:56px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#f97316,#ea580c);color:#fff;border:2px solid #7c2d12;cursor:pointer;font-size:26px;line-height:1;box-shadow:0 6px 24px rgba(249,115,22,.5),2px 2px 0 #7c2d12;font-family:system-ui,-apple-system,"Segoe UI",sans-serif;transition:box-shadow .15s ease;user-select:none;-webkit-user-select:none;';
      btn.onmouseenter = function () { btn.style.boxShadow = '0 10px 32px rgba(249,115,22,.7),2px 2px 0 #7c2d12'; };
      btn.onmouseleave = function () { btn.style.boxShadow = '0 6px 24px rgba(249,115,22,.5),2px 2px 0 #7c2d12'; };
