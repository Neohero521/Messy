...
(function() {
  ...
  function _buildMiniJQ(win, doc) {
    ...
    Q.fn = {
      ...
      on(evt, delegate, handler) {
        ...
        this.els.forEach(el => {
          el.addEventListener(evt, function (e) {
            if (delegate) { const t = e.target.closest && e.target.closest(delegate); if (t && el.contains(t)) handler.call(t, e); }
      ...
      data(k, v) { if (v === undefined) return this.els[0] ? this.els[0].dataset[k] : undefined; this.els.forEach(e => e.dataset[k] = v); return this; },
...
function cleanupNovelWriter() {
  ...
  function addNovelWriterFloatingButton() {
    try {
      ...
      btn.setAttribute('aria-label', '小说续写器');
      btn.title = '小说续写器 · 拖拽移动 · 单击打开';
      btn.textContent = '📖';
      btn.style.cssText = 'position:fixed;z-index:2147483647;width:' + FLOAT_BALL_SIZE + 'px;height:' + FLOAT_BALL_SIZE + 'px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#f97316,#ea580c);color:#fff;border:2px solid #7c2d12;cursor:grab;font-size:26px;line-height:1;box-shadow:0 6px 24px rgba(249,115,22,.5),2px 2px 0 #7c2d12;transition:box-shadow .2s ease;user-select:none;-webkit-user-select:none;touch-action:none;font-family:system-ui,-apple-system,"Segoe UI",sans-serif;';

      // 位置应用（默认右下角，钳制在视口内）
      function applyPos() {
        var w = pWin.innerWidth || doc.documentElement.clientWidth || 1280;
        var h = pWin.innerHeight || doc.documentElement.clientHeight || 800;
        if (st.x == null || st.y == null) { st.x = w - FLOAT_BALL_SIZE - 20; st.y = h - FLOAT_BALL_SIZE - 80; }
      ...
      btn.addEventListener('pointercancel', function () {
        drag = null; btn.style.cursor = 'grab';
      });
      btn.addEventListener('mouseenter', function () { btn.style.boxShadow = '0 10px 32px rgba(249,115,22,.7),2px 2px 0 #7c2d12'; });
      btn.addEventListener('mouseleave', function () { btn.style.boxShadow = '0 6px 24px rgba(249,115,22,.5),2px 2px 0 #7c2d12'; });
