...
@media (max-width: 720px) {
  ...
  /* ---------- 中央主区 / Tab （移动端体验优化：大号胶囊分段控件） ---------- */
  .agent-tabs {
    padding: 10px 12px 12px;
    gap: 8px; /* 每个Tab之间留出呼吸感 */
    background: var(--ag-bg-2); /* 顶栏背景与内容区区分 */
    border-bottom: 1px solid var(--ag-border);
    align-items: center;
  }
  .agent-tab {
    flex: 1; /* 均分宽度 */
    justify-content: center;
    min-height: 46px; /* 触控热区达标（44px+），不再"细" */
    padding: 12px 4px; /* 上下充足点击区域，左右交由flex控制 */
    gap: 6px;
    font-size: 14px;
    font-weight: 500;
    color: var(--ag-text-2); /* 未选中文字对比度提升 */
    border-radius: 12px; /* 胶囊圆角 */
    background: var(--ag-bg-3); /* 未选中时底色 */
    border: 1px solid var(--ag-border); /* 替换桌面的下划线 */
    margin-bottom: 0; /* 覆盖桌面端的 -1px */
    white-space: nowrap; /* 关键：彻底禁止文字换行变成竖排 */
  ...
  .agent-tab.active {
    ...
    border-color: var(--ag-accent);
    font-weight: 700;
    box-shadow: 0 2px 10px var(--ag-accent-dim); /* 选中态轻微上浮，层次更分明 */
  }
  .agent-tab:active {
    transform: scale(0.96); /* 按压回馈，提升触感 */
  }
  .agent-tab svg {
    width: 18px;
    height: 18px;
