...
`;
  ...
  function buildFrontendTabPrompt(cardData, messages) {
    ...
    let feSystemPrompt = '' +
      ...
      '- 正文美化用 extractContent(messageText): 用正则 /<story>([\\s\\S]*?)<\\/story>/ 提取正文（标签名与正则保持一致）；' +
      'renderPage(text) 按空行分段渲染，对话行（"「『开头）加 .dialogue 类，叙述行加 .narrative 类，max-width≤650px 居中\n' +
      '- 结构化用 parseData(messageText): 用正则 /<标签名>([\\s\\S]*?)<\\/标签名>/ 提取数据区；' +
      '解析方式按数据格式选：方式一 [字段名|值]（split(\'|\')）、方式二 键值对（line.split(\':\')），可两者兼容；' +
      'renderPage(data) 渲染界面（卡片/面板/表格/列表等自由设计，max-width≤600px 居中）\n' +
      '- handleClick(keyword, param): 结构化可选交互函数，调用 triggerSlash(\'/send 关键词：参数|/trigger\') 实现点击交互\n' +
      '- 样式自由设计：适配聊天区宽度与主题\n' +
      '- 入口: $(function(){ init(); });\n' +
    ...
    try {
      ...
      function switchTab(targetTab) {
        ...
        if (inputEl) {
          ...
          if (targetTab === 'card') {
            ...
            inputEl.placeholder = '描述你想要的前端界面（如"信纸风正文美化"或"做一个任务面板/论坛"），AI自动判断并生成正则+世界书条目...';
          } else {
        ...
        if (titleH1) {
          if (targetTab === 'card') {
            ...
            titleH1.innerHTML = svgIcon('layers', 18, 'topbar-ic') + ' 前端界面<span style="font-weight:400;font-size:.85em;color:var(--ink-soft)"> · 界面正则</span>';
          } else {
      ...
      function updateCtxBar() {
        ...
        if (__tab === 'card') {
          ...
          // 前端界面 Tab：统一阶段提示（正文美化 + 结构化数据面板）
          const _feB = hasFrontendBeautifyRegex();
          const _feS = getFrontendStructuredRegexes().length;
          stageIcon = 'layers';
          if (_feB && _feS > 0) stageName = '正文美化 + 数据面板 ×' + _feS + ' 已生成';
          else if (_feB) stageName = '正文美化正则已生成';
          else if (_feS > 0) stageName = '结构化数据面板 ×' + _feS + ' 已生成';
          else stageName = '前端界面 · 正则生成';
        } else {
        ...
        if (__tab === 'card') {
          ...
          // 前端界面 Tab：统一状态胶囊（正文美化 + 结构化面板 + 世界书条目）
          ...
          h += '<span class="ctx-chip ' + (_stOk ? 'ok' : 'todo') + '" title="' + (_stOk ? '已生成' : '未生成') + '">' + svgIcon(_stOk ? 'checkCircle' : 'circle', 11) + ' 数据面板 ×' + _feStRx.length + '</span>';
      ...
      function updateQuickActions() {
        ...
        if (currentTab === 'card') {
          ...
          // 前端界面 Tab：统一动作（正文美化 + 结构化数据面板，AI自动判断类型）
      ...
      function handleQuickAction(action) {
        ...
        const frontendOnlyActions = ['generate_frontend', 'preview_frontend', 'reset_frontend'];
        const tabJumpMap = [
        ...
        const frontendPrompts = {
          next: '我当前的前端界面设计进度该怎么推进？请分析：1) 是否已生成[界面]正文美化正则 2) 是否已生成结构化数据面板正则 3) 当前界面的样式/交互可以怎么优化 4) 推荐的下一步。用简洁列表呈现。',
          summary: '帮我梳理前端界面设计现状：1) [界面]正文美化正则的状态 2) 结构化数据面板（名称/标签/数据格式）的覆盖情况 3) 界面样式/交互的亮点与不足 4) 缺失什么、推荐的下一步。',
        ...
        // 选择当前Tab对应的Prompt字典（前端Tab统一一个字典，AI自动判断类型）
        let prompts;
        if (currentTab === 'card') prompts = cardPrompts;
        else if (currentTab === 'frontend') prompts = frontendPrompts;
      ...
      async function callAIChat() {
        ...
        try {
          ...
          // ===== 前端界面处理（统一提取：AI按描述自动输出正文美化或结构化面板）=====
          // ========== Tab 隔离：仅在前端界面 Tab 中执行，角色卡/MVU Tab完全跳过 ==========
          if (currentTab === 'frontend') {
            try {
              // ⚠️修复：不再按模板模式分发——AI根据用户输入自动判断类型。
