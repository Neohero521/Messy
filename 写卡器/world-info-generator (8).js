...
`;
  ...
  const SYS_PROMPT = 
    ...
    '■ 基础语法\n' +
    '- {{宏名}} 不区分大小写；参数用 {{宏名::参数}} 或 {{宏名 参数}}；宏可嵌套（{{getvar::{{char}}_mood}}）；空白字符会被忽略；显示字面花括号用 \\{\\{ 转义。\n' +
    '- 作用域语法：最后一个参数可放在开闭标签之间，如 {{setvar::背景}}多行内容{{/setvar}}；默认自动去首尾空白和统一缩进，加 # 标志保留全部空白（{{#setvar::背景}}...{{/setvar}}）。\n' +
    '■ 条件宏（让内容按状态显示）\n' +
    ...
    '- .变量名=本聊天局部变量、$变量名=跨聊天全局变量：{{.变量名}} 取值、{{.变量名=初值}} 赋值、{{.变量名+=5}} 加法、{{.计数++}} 递增、{{.变量名||默认值}} 假值回退、{{.名字??旅人}} 空值合并；比较运算 {{.数值>=80}}、{{.状态==战斗}} 返回 true/false，常配合 {{if}} 做条件分支。\n' +
    ...
    '· 变量（局部）：{{getvar::名}}/{{setvar::名::值}}/{{addvar::名::值}}/{{incvar::名}}/{{decvar::名}}/{{hasvar::名}}/{{deletevar::名}}；全局：{{getglobalvar::名}}/{{setglobalvar::名::值}}/{{addglobalvar::名::值}}/{{incglobalvar::名}}/{{decglobalvar::名}}/{{hasglobalvar::名}}/{{deleteglobalvar::名}}。\n' +
