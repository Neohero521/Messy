 ...
 *   - AI 基于两个示例 JSON 模板修改，输出 ```html 包裹的代码
 *   - 代码区标签页：正则配置 / HTML 代码 / 世界书源文件（B模式）
 *   - HTML 实时预览（mock 酒馆 API）
 *   - 一键复制 / 一键导入酒馆正则
 *   - 移动端三 tab 切换（聊天/代码/预览）
 *
 * 示例文件：
 *   - regex-[界面]正文美化(示例).json        → 模式A 基底模板
 *   - regex-[界面]想做的前端名称(示例).json   → 模式B 基底模板
 * ==========================================================================
 */
  ...
  (function exposeTavernApiLocally() {
    var names = [
      'eventOn', 'eventOff', 'eventTrigger', 'getButtonEvent',
      'replaceScriptButtons', 'appendInexistentScriptButtons',
...
:root{
  --bg:#f0f2f5;--surface:#ffffff;--surface-soft:#f8f9fa;--surface-sink:#eef0f3;
  --ink:#1a1a2e;--ink-soft:#495057;--muted:#868e96;
  --accent:#4f46e5;--accent-deep:#4338ca;--accent-soft:rgba(79,70,229,.08);
  --accent-border:rgba(79,70,229,.22);--accent-text:#4338ca;
  --user-bubble:#4f46e5;--user-bubble-text:#ffffff;
  --ai-bubble:#ffffff;--ai-bubble-text:#1a1a2e;
  --code-bg:#1e1e2e;--code-text:#cdd6f4;
  --sage:#16a34a;--terra:#dc2626;--amber:#ca8a04;
  --line:rgba(15,23,42,.08);--line-soft:rgba(15,23,42,.04);
  --radius:12px;--radius-sm:8px;
  --shadow-soft:0 2px 8px rgba(15,23,42,.04);--shadow-bubble:0 1px 2px rgba(15,23,42,.06);
  --font:'Segoe UI',system-ui,-apple-system,'PingFang SC','Microsoft YaHei UI',sans-serif;
...
.preview-header .pv-title{font-size:12px;font-weight:600;color:var(--ink-soft);margin-right:auto}
...
`;
  ...
  // ===== 模式A 示例模板（来自 regex-[界面]正文美化(示例).json 的 replaceString，原样保留）=====
  var EXAMPLE_A_HTML = `<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
    <title>正文美化</title>
...
<body>
  ...
  // ===== 模式B 示例模板（来自 regex-[界面]想做的前端名称(示例).json 的 replaceString，原样保留）=====
  var EXAMPLE_B_HTML = `<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
    <title>页面标题</title>
...
<body>
  ...
  var SYSTEM_PROMPT = [
    '# 角色定位',
    '你是 SillyTavern（酒馆助手）的「正则代码生成器」。用户用自然语言描述需求，你基于下方两个示例模板修改，输出 ```html 包裹的代码。',
    '',
    '# ⚠️ 绝对核心原则',
    '1. 必须基于下方示例模板修改，不允许从零重写',
    '2. 必须保留示例中的函数骨架（getMessageData / extractContent 或 parseData / renderPage / init / handleClick）',
    '3. 输出的 HTML 必须用 ```html 和 ``` 包裹（三反引号 + html 标记）',
    '4. 严禁只输出文字说明，每次必须输出完整代码',
    '',
    '# 两种模式',
    '## 模式A：正文美化模板',
    '适用：小说排版、对话气泡、信件、日记、便签等（AI 只需输出 <标签>正文</标签>）',
    '基底模板特征：getMessageData → extractContent（正则提取标签内容）→ renderPage（段落渲染）',
    '可修改：CSS 样式、标签名、段落处理逻辑、HTML 容器结构',
    '',
    '## 模式B：结构化数据美化模板',
    '适用：状态栏、属性面板、任务清单、背包界面、论坛帖子、排行榜等（AI 需按固定格式输出字段）',
    '基底模板特征：getMessageData → parseData（正则解析 [字段|值] 或键值对）→ renderPage（数据渲染）+ handleClick（按钮交互）',
    '可修改：CSS 样式、标签名、字段解析规则、UI 布局、按钮',
    '',
    '# 模式判断规则',
    '- 出现「字段数据/属性数值/项目列表/可点击按钮」→ 模式B',
    '- B 模式关键词：状态栏、属性面板、角色信息、任务面板、背包、装备、商店、论坛、排行榜、技能、数据展示',
    '- 只需排版文字/段落/对话 → 模式A',
    '- 拿不准默认模式B',
    '',
    '# 输出格式（严格遵守）',
    '第 1 行：[模式A] 或 [模式B] + 简短说明（1-2句）',
    '第 2 行起：脚本名称和查找正则',
    '  脚本名称: [界面]xxx',
    '  查找正则: <标签名>[\\s\\S]*?</标签名>',
    '然后输出 ```html 包裹的完整 HTML 代码',
    '模式B 还需输出 <WORLDBOOK_SOURCE> 包裹的世界书源文件',
    '',
    '# ==================== 模式A 基底模板（必须基于此修改） ====================',
    '脚本名称: [界面]正文美化',
    ...
    '# ==================== 修改规则 ====================',
    '## 模式A 修改范围',
    '- getMessageData()：原样保留，不改',
    '- extractContent()：仅改正则里的标签名（如 story → letter）',
    '- renderPage()：可改段落处理逻辑和 CSS class',
    '- init()：原样保留',
    '- $(function() { init(); })：原样保留',
    '- CSS 样式：自由设计（信纸/日记/对话气泡等风格）',
    '- HTML 容器：可改 class 名和结构，但保留 id="content"',
    '',
    '## 模式B 修改范围',
    '- getMessageData()：原样保留',
    '- parseData()：改标签名，增删字段解析行（如新增 [速度|x] 解析）',
    '- handleClick()：原样保留',
    '- renderPage()：自由设计 UI 布局（卡片/表格/进度条等）',
    '- init()：原样保留',
    '- $(function() { init(); })：原样保留',
    '- CSS 样式：自由设计',
    '- HTML 容器：可改结构，但保留 id="content"',
    '',
    '## 世界书源文件格式（仅模式B）',
    '<WORLDBOOK_SOURCE>',
    '<页面名称相关>',
    ...
    '- 脚本名称统一前缀：[界面]xxx',
    '- 每次输出完整代码，不要输出部分代码',
    '- 模式B 的 parseData 解析的字段必须和世界书源文件的 Format 字段一一对应'
  ].join('\n');
  ...
  // ===== 解析 AI 回复 =====
  // 提取 ```html ... ``` 包裹的代码（与示例 JSON 的 replaceString 格式完全一致）
  function parseAIResponse(text) {
    var result = {
      ...
      scriptName: '',
      findRegex: '',
      htmlCode: '',        // HTML 代码（不含 ```html 包裹）
      replaceString: '',   // 完整 replaceString（含 ```html\n...\n``` 包裹，与示例 JSON 一致）
      worldbookSource: '',
      tavernRegexObj: null
    };

    // 提取 ```html ... ``` 代码块
    var htmlMatch = text.match(/```html\s*\n([\s\S]*?)\n```/);
    if (htmlMatch) {
      result.htmlCode = htmlMatch[1].trim();
      // replaceString 格式与示例 JSON 完全一致：```html\n + 代码 + \n```
      result.replaceString = '```html\n' + result.htmlCode + '\n```';
    }

    // 提取脚本名称和查找正则
    var nameMatch = text.match(/脚本名称\s*[:：]\s*(.+)/);
    var findMatch = text.match(/查找正则\s*[:：]\s*(.+)/);
    ...
    // 提取说明文字（```html 之前的内容）
    if (htmlMatch) {
      result.explanation = text.substring(0, htmlMatch.index).trim();
    } else {
  ...
  async function openGenerator() {
    ...
    // ===== 生成正则配置文本（与示例 JSON 的字段对应）=====
    function buildRegexConfigText(parsed) {
      if (!parsed) return '';
      var lines = [];
      lines.push('脚本名称: ' + (parsed.scriptName || '[界面]自定义'));
      lines.push('查找正则: ' + parsed.findRegex);
      lines.push('');
      lines.push('placement: [2]  (AI输出)');
      lines.push('disabled: false');
      lines.push('markdownOnly: true  (仅格式显示)');
      lines.push('promptOnly: false');
      lines.push('runOnEdit: true  (在编辑时运行)');
      lines.push('substituteRegex: 0');
      lines.push('minDepth: null');
      lines.push('maxDepth: null');
      return lines.join('\n');
    }
    ...
    function updateCodeDisplay() {
      ...
      if (currentParsed.worldbookSource) { tabWorldbook.disabled = false; }
      else {
        tabWorldbook.disabled = true;
      ...
      if (activeCodeTab === 'regex') {
        content = buildRegexConfigText(currentParsed);
        title = '正则配置（与示例 JSON 字段对应）';
      } else if (activeCodeTab === 'html') {
        content = currentParsed.htmlCode || '(无 HTML 代码)';
        title = '前端 HTML 代码（```html 包裹的 replaceString 内容）';
    ...
    function renderPreview() {
      ...
      if (!testInput) {
        var tagMatch = (currentParsed.findRegex || '').match(/<([a-zA-Z_\u4e00-\u9fa5][a-zA-Z0-9_\u4e00-\u9fa5-]*)>/);
        if (tagMatch) {
      ...
      var mockScript = [
        ...
        'window.getChatMessages = function(id) { return [{ message: ' + JSON.stringify(testInput) + ' }]; };',
    ...
    async function sendMessage() {
      ...
      try {
        ...
        if (parsed.tavernRegexObj || parsed.htmlCode) {
    ...
    btnHelp.addEventListener('click', function () {
      alert('【正则代码生成器使用说明】\n\n💬 聊天\n  · 描述需求，AI 基于示例模板生成 ```html 包裹的代码\n  · 模式A：正文美化（小说/对话/信件）\n  · 模式B：结构化数据（状态栏/论坛/面板）\n\n📝 代码区（右上标签页切换）\n  · 正则配置：脚本名称/查找正则等字段\n  · HTML 代码：```html 包裹的 replaceString 内容\n  · 世界书源文件（模式B）：规范 AI 输出格式\n\n👁 预览区（右下）\n  · 输入测试内容（模拟 AI 输出的标签）\n  · 点击「渲染」查看实际效果\n\n📌 操作\n  · Ctrl+Enter 发送 · Enter 换行\n  · 点「导入酒馆」一键导入正则\n  · 点「复制」复制当前代码');
    });
    ...
    btnCopyCode.addEventListener('click', function () {
      ...
      var content = '';
      if (activeCodeTab === 'regex') content = buildRegexConfigText(currentParsed);
    ...
    btnImport.addEventListener('click', async function () {
      ...
      var tagCheck = obj.findRegex.match(/<([a-zA-Z_][a-zA-Z0-9_-]*)>/);
      ...
      try {
        ...
        if (typeof root.updateTavernRegexesWith === 'function') {
          await root.updateTavernRegexesWith(function (regexes) {
            var existingIdx = -1;
            regexes.forEach(function (r, i) { if (r.script_name === obj.scriptName) existingIdx = i; });
            if (existingIdx >= 0) regexes[existingIdx] = obj; else regexes.push(obj);
            return regexes;
          }, { type: 'global' });
          createToast(win, '✅ 正则「' + obj.scriptName + '」已导入！', 'success');
        } else {
          copyToClipboard(win, currentParsed.replaceString, function (ok) {
    ...
    addAssistantMessage('你好！我是正则代码生成助手。\n\n我会基于两个示例模板修改生成 ```html 包裹的代码：\n· 模式A：正文美化（getMessageData + extractContent + renderPage）\n· 模式B：结构化数据（getMessageData + parseData + renderPage + handleClick）\n\n生成后可在右侧查看代码和实时预览。\n\n例如：\n· "做一个小说正文美化，首行缩进，对话用引号"\n· "做一个角色状态栏，显示HP/MP/攻击力/防御力"');
  ...
  function mainInit() {
    ...
    initialized = true;
    console.log('[正则代码生成器] 🚀 初始化（版本：基于示例模板生成 ```html 代码）');
