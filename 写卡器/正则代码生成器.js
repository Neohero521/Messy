...
`;
  ...
  var SYSTEM_PROMPT = [
    ...
    '',
    '# ==================== 通用技术规范 ====================',
    '- HTML 中获取消息内容：var msg = getChatMessages(getCurrentMessageId())[0].message;',
    '- HTML 中触发按钮：triggerSlash(\'/send 关键词|/trigger\');',
    '- 正则表达式统一写法：<标签名>[\\s\\S]*?</标签名>',
    '- 标签名用英文语义化（story/status/task/inventory/letter/forum 等），不要中文',
    '- 脚本名称统一前缀：[界面]xxx',
    '- 每次都输出完整代码，不要输出部分代码或"只改了xx行"',
    '- 模式B 的 HTML 中必须同时包含 parseData 函数（解析 [字段|值] 或 键值对）和 handleClick 函数（按钮点击发送触发词）'
  ].join('\n');
