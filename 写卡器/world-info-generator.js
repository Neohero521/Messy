...
`;
  ...
  const SYS_PROMPT = 
    ...
    '- rename 旧名 → 新名 —— 重命名条目\n' +
    '\n' +
    '【条目元信息行（必须写在正文前，每行一个，AI可直接理解并逐项调整）】\n' +
    ...
    'prevent_recursion=false    （可选：防止递归）\n' +
    'exclude_recursion=false    （可选：排除递归）\n' +
    'delay_until_recursion=false（可选：延迟递归）\n' +
    'sticky=0 / cooldown=0 / delay=0 （可选，默认0）\n' +
    'enabled=true               （可选，false=禁用该条目）\n' +
    '\n' +
    '【条目生成示例（严格按此格式）】\n' +
    '::: upsert <重要角色>白娅\n' +
    'keys=白娅,白娅小姐,白娅公主\n' +
    ...
    '- 开场白1：::: set first_mes\n（开场白正文，结构：场景描写→动作驱动→内心独白→自然对话→结尾留钩）\n:::\n' +
