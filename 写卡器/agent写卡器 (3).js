...
<body>
  ...
  function buildPrompt(cardData, cardGenerated, messages, agentDirective) {
    ...
    const agentIdentity = '\n' +
      ...
      '2. 变量/数值追踪/好感度/金钱/物品/属性/阶段/境界/状态栏/MVU → 领域B：**一律生成MVU体系资产**（zod脚本→[InitVar]→更新规则→变量列表→输出格式→格式强调→占位提醒→状态栏HTML，按固定顺序，★默认且必须一次回复批量输出全部缺失条目——禁止逐条停顿等用户确认，除非用户明确要求"逐条生成/每条等确认"）。⚠️【铁律】变量追踪内容必须以MVU条目形式实现，禁止做成普通世界书条目、禁止写进description/开场白——变量系统只有走MVU体系才能在酒馆运行。⚠️【严格模板】第4/5/6条是固定内容模板必须原样输出，第2/3条严格依据第1条schema的字段名生成（路径字字对应），禁止自创格式。需求不明时先一次性问清（要追踪什么/怎么展示），用户说"直接做/你看着办"则按合理默认批量生成\n' +
    ...
    const _habitHint = (typeof UserProfile !== 'undefined') ? UserProfile.buildPromptHint() : '';
    const sysPrompt = agentIdentity + SYS_PROMPT + specBlock + (_habitHint ? ('\n\n' + _habitHint) : '');
    ...
    // ========== Agent 模式：理解阶段 / 确认阶段提示（按最后一条用户消息判断）==========
    // ⚠️ buildPrompt 在 IIFE 顶层作用域，访问不到 openEditor 内的 agentAutoMode，统一走 window 挂载
    let _agentStageBlock = '';
    const _agentModeOn = (typeof window !== 'undefined' && window.__getAgentAutoMode) ?
      !!window.__getAgentAutoMode() :
      (typeof window !== 'undefined' && !!window.__agentAutoMode);
    if (_agentModeOn && !agentDirective) {
      // 判断最后一条用户消息是否是"选项确认"
      let _isConfirmingMsg = false;
      for (let _mi = tabMessages.length - 1; _mi >= 0; _mi--) {
        if (tabMessages[_mi] && tabMessages[_mi].role === 'user') {
          const _uTxt = String(tabMessages[_mi].content || '').trim();
          if (/^我选择\s*[:：]/.test(_uTxt)) _isConfirmingMsg = true;
          break;
        }
      }
      if (_isConfirmingMsg) {
        _agentStageBlock =
          '\n\n' +
          '═══════════════════════════════════════════════════════════════════\n' +
          '✅【Agent模式 · 确认阶段（当前）】用户已确认理解与方向\n' +
          '═══════════════════════════════════════════════════════════════════\n' +
          '请**立即**输出 <agent_plan> 计划块并简短说明执行策略，写卡器会自动逐步执行。\n' +
          '· **禁止**再输出 <agent_understand> 或 <agent_options> 块\n' +
          '· **禁止**向用户再确认——直接给计划，计划之后按 agentLoop 逐步执行\n' +
          '· 步骤越少越好（典型 2-4 步），每步打包该范围内的全部产物\n' +
          '· 每步描述必须具体可执行（如"为白娅/林月各生成1条角色条目"）\n' +
          '· MVU 依赖顺序：前7条完成后才生成第8条状态栏\n\n' +
          '格式：\n' +
          '<agent_plan>\n' +
          '目标：一句话总目标\n' +
          '步骤：\n' +
        ...
        _agentStageBlock =
          ...
          '🎯【Agent模式 · 理解阶段（当前）】\n' +
          '═══════════════════════════════════════════════════════════════════\n' +
          '用户刚提出新的需求。你**必须**先理解并确认，**不要直接开始生成**。\n\n' +
          '【本轮必须输出】以下两个块（且只能输出这两个块 + 1-2句必要铺垫）：\n' +
          '1. <agent_understand>...</agent_understand>\n' +
          '   · 用 2-4 句话简洁复述你理解的用户意图（做什么 / 要哪些配套 / 覆盖什么范围）\n' +
          '2. <agent_options>...</agent_options>\n' +
          '   · 给出 2-3 个具体的方向选项，每行一个（不要写编号，写卡器会自动编号）\n' +
          '   · 选项之间必须是**有意义的差异**（范围大小 / 是否包含某子系统 / 不同风格取向），不是同义改写\n\n' +
          '【格式示例】\n' +
          '<agent_understand>\n' +
          '用户希望创作一个末世生存角色卡，主角是医生，需要 MVU 变量系统追踪健康/物资/道德值，以及一个暗黑风格的正文美化界面。\n' +
          '</agent_understand>\n' +
          '<agent_options>\n' +
          '完整创作：角色卡主体 + MVU变量系统 + 暗黑风格正文美化（推荐）\n' +
          '只做角色卡主体（世界观+开场白+3个核心条目），MVU和界面后续再说\n' +
          '先做角色卡 + MVU变量系统，暂不做前端界面\n' +
          '</agent_options>\n\n' +
          '【硬性要求】\n' +
          '· 本轮**只输出**这两个块 + 必要的1-2句铺垫文字\n' +
          '· **禁止**输出:::操作块、```html代码块、<agent_plan>、JSON\n' +
          '· 不要替用户做选择，静待用户点击选项或在输入框自定义输入\n' +
          '· 例外：若用户消息中已明确说"直接做/别问了/按你说的做/不用确认/你自己决定"——' +
          '  则**跳过理解阶段**，本轮直接输出 <agent_plan> 计划块\n';
      }
    }

    let fullPrompt = sysPrompt + stateInfo + existingInfo + qcBlock + jsonReminder + _agentStageBlock + '\n\n=== 对话历史（Agent单会话） ===\n';
  ...
  async function openEditor() {
    try {
      ...
      function revokeAIMessage(aiIdx) {
        ...
        try { UserProfile.recordRevoke(aiIdx); } catch (_) {}
      ...
      function revokeAfterUserMessage(userIdx) {
        ...
        progress = calcProgress();
        rerenderChatMessages();
        try { UserProfile.recordRevoke(userIdx); } catch (_) {}
      ...
      function regenerateAIMessage(aiIdx) {
        ...
        // 用户画像：这是一次重试（摩擦信号）
        try { UserProfile.recordRetry(aiIdx); } catch (_) {}
      ...
      function startCheckFlow() {
        ...
        if (!input) return;
        input.value =
          '请作为写卡Agent执行一次全面检查，并按检查结果输出待办计划：\n\n' +
          '【检查范围 · 逐项核对「当前创作进度总览」】\n' +
          '1. 角色卡主体\n' +
          '   · 名称 / 世界观描述（长度是否充分）/ 开场白（是否完整）/ 备选开场白\n' +
          '   · 世界书条目：数量是否合理、是否覆盖世界观核心概念、触发词是否完整（keys 空且非常驻=红线）、' +
          '     constant/position/selectiveLogic/depth/probability/order/match_whole_words 等字段配置是否符合规范、正文是否 YAML 中文格式且独立可读\n' +
          '2. MVU 变量系统（8步工作流，逐条核对）\n' +
          '   · 第1条 zod变量结构脚本 / 第2条 [InitVar]初始变量 / 第3条 [mvu_update]更新规则 / 第4条 变量列表 /\n' +
          '     第5条 输出格式 / 第6条 输出格式强调 / 第7条 <状态栏>占位符提醒 / 第8条 正则6 状态栏HTML\n' +
          '3. 前端界面\n' +
          '   · [界面]正文美化正则 / [界面]X 结构化面板正则 / 对应的规范AI输出世界书条目\n\n' +
          '【输出要求】\n' +
          '① 先用 3-8 条列表列出本次检查发现的所有问题（未完成项 + 可优化项），按优先级排列，' +
          '   每条必须具体指出位置（如"条目 ⟦白娅⟧ 的 keys 为空且 constant=false"），禁止写"内容不够丰富"这类空话\n' +
          '② 然后输出 <agent_plan> 计划块，覆盖上面列出的每一项待处理任务\n' +
          '③ ★步骤越少越好（典型 2-4 步），每步把该范围内的全部产物打包在同一次回复中输出' +
          '   （多个:::操作块 / 多个```html代码块可混排一次输出，节省调用次数优先）\n' +
          '④ 每步描述必须具体可执行（如"为白娅/林月各生成1条角色条目"），禁止模糊描述\n' +
          '⑤ 若检查后发现所有缺口已完成，则给出 1-2 步打磨优化计划（不空转）\n\n' +
          '写卡器会自动逐步执行你的计划，无需用户逐步确认。';
        handleSend();
      }
      ...
      function showUserProfilePanel() {
        ...
        doc.getElementById('upExportBtn').addEventListener('click', function() {
          try {
            const blob = new Blob([UserProfile.exportJson()], { type:'application/json' });
            const url = URL.createObjectURL(blob), a = doc.createElement('a');
            a.href = url; a.download = '写作习惯_' + new Date().toISOString().slice(0,10) + '.json';
  ...
  const _UP_GENRE_KEYWORDS = {
    '仙侠/修仙': ['修仙','仙侠','修真','宗门','飞升','灵石','丹药','金丹'],
    '玄幻/奇幻': ['玄幻','奇幻','魔法','斗气','异世界','勇者','魔王'],
    '都市/校园': ['都市','校园','学院','高中','大学','社团'],
    '末世/废土': ['末世','末日','废土','丧尸','避难所','幸存者'],
    '科幻/赛博': ['科幻','赛博','机甲','星际','飞船','人工智能'],
    '恋爱/养成': ['恋爱','养成','好感','告白','约会','攻略'],
    '经营/模拟': ['经营','模拟','店铺','农场','贸易'],
    '推理/悬疑': ['推理','悬疑','侦探','密室','凶手','案件'],
    '恐怖/惊悚': ['恐怖','惊悚','怨灵','怪谈','诡异'],
    '克苏鲁':   ['克苏鲁','触手','旧日','深渊','邪神'],
  };
  const _UP_SCALE_KEYWORDS = {
    dark: ['暗黑','黑暗','血腥','残酷','绝望','暴力','虐待'],
    nsfw: ['NSFW','18禁','成人','情欲','色情','露骨','涩涩'],
    safe: ['全年龄','治愈','日常','温馨','纯爱','轻小说'],
  };
  ...
  const UserProfile = (function() {
    ...
    function clear() { try { localStorage.removeItem(USER_PROFILE_KEY); } catch(_){} _cache = null; }
