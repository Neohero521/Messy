...
`;
  ...
  const SYS_PROMPT = 
    ...
    '③★comment（条目名）：按内容类型自由命名（<重要角色>XX/<地点场景>XX/<世界规则>XX 等）\n' +
    ...
    try {
      ...
      function handleQuickAction(action) {
        ...
        const cardPrompts = {
          ...
          opening: '请根据现有世界观设定生成开场白。开场白1用 ::: set first_mes（500-800字：场景描写→主角出场→冲突/悬念→结尾留钩，禁止占位符）；同时生成2条以上备选开场白用 ::: set alternate_greetings（多条用---分割，与开场白1不同的场景/视角/时机）。全部用:::操作块输出。',
