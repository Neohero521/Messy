        ...
        ops.forEach(function(op) {
          ...
          if (op.action === 'upsert' || op.action === 'update') {
            ...
            if (validFields.indexOf(fieldName) >= 0) {
              // ⚠️Tab隔离：角色卡Tab下拦截 MVU 内容写入顶层字段（AI可能把MVU内容塞进description等，绕过条目级拦截）
              if (currentTab === 'card' && _isMvuContentCheck(op.content)) {
                console.warn('[Tab隔离·角色卡Tab] :::set 拦截MVU内容写入字段:', fieldName);
                return;
              }
