...
/* 前端Tab模板模式切换条（正文美化 / 结构化数据） */
.fe-mode-bar{display:flex;align-items:center;gap:6px;padding:6px 12px;border-top:1px solid var(--line);flex-wrap:wrap}
.fe-mode-label{display:inline-flex;align-items:center;gap:4px;font-size:.75em;font-weight:600;color:var(--muted);margin-right:2px}
.fe-mode-btn{display:inline-flex;align-items:center;gap:5px;padding:5px 12px;border-radius:999px;border:1px solid var(--line);background:var(--surface);color:var(--muted);font-size:.78em;font-weight:500;cursor:pointer;transition:all .18s ease}
.fe-mode-btn:hover{color:var(--accent-text);border-color:var(--accent-border);background:var(--accent-soft)}
.fe-mode-btn.active{color:var(--accent-text);background:linear-gradient(135deg,var(--accent-soft),rgba(79,70,229,.14));border-color:var(--accent-border);font-weight:600;box-shadow:0 2px 8px rgba(79,70,229,.12)}
.fe-mode-btn.active svg{transform:scale(1.08)}
/* 状态栏8步进度step方块 */
...
`;
  ...
  function buildPrompt(cardData, cardGenerated, messages) {
    ...
    if (__tab !== 'card' && __tab !== 'mvu' && __tab !== 'frontend') __tab = 'card';
    ...
    if (__tab === 'frontend') {
      // ===== 前端界面 Tab：只发角色卡内容摘要 + 前端正则生成规范，完全不发角色卡生成逻辑 =====
      return buildFrontendTabPrompt(cardData, messages);
    }
    if (__tab === 'mvu') {
  ...
  // ===== 构建前端界面 Tab 提示词（正文美化正则生成，与角色卡/MVU Tab 完全隔离）=====
  function buildFrontendTabPrompt(cardData, messages) {
    const cd = cardData || {};
    // 0. 当前前端 Tab 模板模式（beautify=正文美化 / structured=结构化数据），由 openEditor 闭包同步到 window
    let __feMode = 'beautify';
    if (typeof window !== 'undefined' && window.__frontendMode) {
      __feMode = window.__frontendMode;
    }
    if (__feMode !== 'beautify' && __feMode !== 'structured') __feMode = 'beautify';
    ...
    if (cd.first_mes) ctxParts.push('开场白(完整' + (cd.first_mes || '').length + '字，不截断)：' + (cd.first_mes || ''));
    const entries = (cd.character_book || {}).entries || [];
    // 注入常规世界书条目摘要（只读上下文，用于设计界面风格参考，禁止修改）
    const nonMvuEntries = entries.filter(function(e) {
      const c = (e.comment || '').toLowerCase();
      if (c.indexOf('[initvar]') >= 0) return false;
      if (c.indexOf('变量更新规则') >= 0) return false;
    ...
    if (nonMvuEntries.length > 0) {
      ...
      nonMvuEntries.forEach(function(e, i) {
        nonMvuText += '  ' + (i + 1) + '. [' + (e.comment || '条目' + (i + 1)) + '] (摘要' + Math.min((e.content || '').length, 300) + '字): ' + String(e.content || '').slice(0, 300) + '\n';
      });
      ctxParts.push(nonMvuText);
    }
    // 当前已有的前端正则状态（正文美化 + 结构化面板，按模式展示相关部分）
    const regexScripts = (cd.extensions || {}).regex_scripts || [];
    const feRegexScripts = regexScripts.filter(function(s) {
      if (!s) return false;
      if (__feMode === 'structured') {
        return (s.id === 'frontend-beautify') || ((s.scriptName || '').indexOf('[界面]') >= 0);
      }
      return (s.id === 'frontend-beautify') || ((s.scriptName || '').indexOf('[界面]正文美化') >= 0);
    });
    ...
    // 2. 前端界面专属系统指令（按当前模板模式：beautify=正文美化 / structured=结构化数据）
    let feSystemPrompt = '';
    if (__feMode === 'structured') {
      feSystemPrompt = '' +
        '你是「前端界面设计师」——专门负责设计酒馆聊天界面的「结构化数据美化」正则（[界面]页面名称），' +
        '把AI按特定格式输出的结构化数据（状态栏、论坛、任务面板等，**非MVU变量美化**）渲染成自定义界面。\n\n' +
        '═══════════════════════════════════════════════════════════════════\n' +
        '🎯 你的专属职责（只有这些，别的都不管）\n' +
        '═══════════════════════════════════════════════════════════════════\n' +
        'A. 根据用户需求设计「[界面]页面名称」正则（页面名称按用途定：如[界面]任务面板、[界面]论坛、[界面]状态栏）\n' +
        'B. 设计数据格式（[字段名|值] 或 键值对）+ 完整HTML替换代码，样式贴合当前角色卡世界观\n' +
        'C. 设计「规范AI输出」的世界书条目（条目标题=此前端名称，内容含注意事项+触发条件Format示例）——写卡器会据你的输出自动生成，你只需保证数据格式清晰\n' +
        'D. 只负责结构化数据前端，绝不生成角色卡字段/MVU变量内容（zod脚本/[InitVar]/<状态栏>等一律禁止）\n\n' +
        '═══════════════════════════════════════════════════════════════════\n' +
        '📌 酒馆正则配置模板（必须严格遵循，这就是[界面]页面名称）\n' +
        '═══════════════════════════════════════════════════════════════════\n' +
        '正则名称: [界面]页面名称\n' +
        '查找正则表达式: <标签名>[\\s\\S]*?</标签名>\n' +
        ...
        '═══════════════════════════════════════════════════════════════════\n' +
        '🧩 前端界面HTML代码模板（必须基于此结构，样式可自由发挥）\n' +
        '═══════════════════════════════════════════════════════════════════\n' +
        '- getMessageData(): 用 getChatMessages(getCurrentMessageId()) 获取当前消息内容（酒馆标准API）\n' +
        '- parseData(messageText): 用正则 /<标签名>([\\s\\S]*?)<\\/标签名>/ 提取数据区；' +
        '解析方式按数据格式选：方式一 [字段名|值]（split(\'|\')）、方式二 键值对（line.split(\':\')），可两者兼容\n' +
        '- renderPage(data): 根据解析出的数据对象渲染界面（卡片/面板/表格/列表等自由设计）\n' +
        '- handleClick(keyword, param): 可选交互函数，调用 triggerSlash(\'/send 关键词：参数|/trigger\') 实现点击交互\n' +
        '- 样式自由设计：max-width≤600px 居中、适配聊天区宽度与主题\n' +
        '- 入口: $(function(){ init(); });\n' +
        '- 完整HTML文档：<!DOCTYPE html> + <style> + <body> + <script>，代码全部内联\n\n' +
        '═══════════════════════════════════════════════════════════════════\n' +
        '📜 结构化数据面板的输出格式（铁律，写卡器按此自动保存）\n' +
        '═══════════════════════════════════════════════════════════════════\n' +
        '回复时必须按以下格式组织（便于写卡器提取页面名称/标签名/HTML）：\n' +
        ...
        '⚠️ 前端 Tab 核心铁律（最高优先级）\n' +
        '═══════════════════════════════════════════════════════════════════\n' +
        '1. ✅唯一输出格式：【页面名称】+【标签名】+ 一个完整HTML代码块（```html 或纯```），写卡器自动提取保存\n' +
        ...
        '5. ⚠️不要修改已存在的[界面]正文美化正则（那是正文美化模式的功能，与本模式无关）\n' +
        '6. 修改已有面板时：直接输出新的【页面名称】【标签名】+完整HTML代码块，写卡器自动覆盖\n' +
        '7. 只处理用户「最新一条」消息的指令，不要重复处理旧指令\n';
    } else {
      feSystemPrompt = '' +
        '你是「前端界面设计师」——专门负责设计酒馆聊天界面的「正文美化」正则（[界面]正文美化），把AI消息中的正文优雅渲染成自定义界面。\n\n' +
        '═══════════════════════════════════════════════════════════════════\n' +
        '🎯 你的专属职责（只有这些，别的都不管）\n' +
        '═══════════════════════════════════════════════════════════════════\n' +
        'A. 根据用户需求设计/修改「[界面]正文美化」正则：正则查找 + 完整HTML替换代码\n' +
        'B. HTML界面样式自由设计（信纸/日记本/对话气泡/游戏面板等），贴合当前角色卡世界观风格\n' +
        'C. 只负责前端界面正则，绝不生成世界书条目/角色卡字段/MVU变量内容\n\n' +
        '═══════════════════════════════════════════════════════════════════\n' +
        '📌 酒馆正则配置模板（必须严格遵循，这就是[界面]正文美化）\n' +
        '═══════════════════════════════════════════════════════════════════\n' +
        '正则名称: [界面]正文美化\n' +
        '查找正则表达式: <story>[\\s\\S]*?</story>\n' +
        '  · <story> 标签可换成用户想要的任何标签名\n' +
        '  · 严禁使用 <think>、<thinking>、<content> 作为标签\n' +
        '替换为: 下方HTML代码（必须用```html代码块包裹）\n' +
        '勾选: AI输出 ✓、在编辑时运行 ✓、仅格式显示 ✓\n' +
        '（对应regex_scripts字段：placement=[2]、markdownOnly=true、promptOnly=false、runOnEdit=true）\n\n' +
        '═══════════════════════════════════════════════════════════════════\n' +
      ...
      '═══════════════════════════════════════════════════════════════════\n' +
      '- getMessageData(): 用 getChatMessages(getCurrentMessageId()) 获取当前消息内容（酒馆标准API）\n' +
      '- extractContent(messageText): 用正则 /<story>([\\s\\S]*?)<\\/story>/ 提取正文；标签名必须与正则里的保持一致\n' +
      '- renderPage(text): 按空行\\n\\s*\\n 分段渲染为段落；对话行（以"「『 开头）加 .dialogue 类，叙述行加 .narrative 类\n' +
      '- 样式自由设计：max-width≤650px 居中、line-height 1.9、适配聊天区宽度与主题；信纸/日记/对话气泡等皆可\n' +
      '- 入口: $(function(){ init(); });\n' +
      '- 完整HTML文档：<!DOCTYPE html> + <style> + <body> + <script>，代码全部内联\n\n' +
      '═══════════════════════════════════════════════════════════════════\n' +
      '⚠️ 前端 Tab 核心铁律（最高优先级）\n' +
      '═══════════════════════════════════════════════════════════════════\n' +
      '1. ✅唯一输出格式：一个完整的HTML文档代码块（```html 或纯```），写卡器自动提取保存为[界面]正文美化正则\n' +
      '2. ⚠️严禁把HTML代码放进JSON的entries数组、:::操作块、<statusblock>——HTML代码不属于世界书条目\n' +
      '3. ⚠️不要生成任何角色卡/世界书相关的JSON（name/description/entries）\n' +
      '4. ⚠️不要生成任何MVU变量内容（zod脚本/[InitVar]/变量列表/状态栏）\n' +
      '5. ⚠️不要修改已存在的[界面]正文美化正则的查找表达式，除非用户明确要求换标签\n' +
      '6. 修改已有正则时：直接输出新的完整HTML代码块，写卡器自动覆盖\n' +
      '7. 只处理用户「最新一条」消息的指令，不要重复处理旧指令\n';
    }
    ...
    fullPrompt += '助手: ';
    fullPrompt += '（请只针对上方>>>标记的最新指令回复。严格遵守前端 Tab 核心铁律：' +
      (__feMode === 'structured' ?
        '按【页面名称】+【标签名】+完整HTML代码块的格式输出，不生成任何角色卡/世界书/MVU内容。' :
        '只输出完整HTML代码块，不生成任何角色卡/世界书/MVU内容。') + '）';
  ...
  async function openEditor() {
    try {
      ...
      const chatSessions = {
        ...
        frontend: {
          messages: [], // 前端界面 Tab独立聊天历史
          mode: 'beautify' // 前端 Tab 模板模式：'beautify'=正文美化 | 'structured'=结构化数据
        }
      };
      ...
      // 前端 Tab 当前模板模式（beautify=正文美化 / structured=结构化数据），随 chatSessions.frontend.mode 持久化
      let frontendMode = (chatSessions.frontend.mode === 'structured') ? 'structured' : 'beautify';
      // 同步到 window：buildFrontendTabPrompt（模块级函数）通过 window.__frontendMode 读取当前模式
      if (typeof window !== 'undefined') {
        window.__frontendMode = frontendMode;
      }
      ...
      function getCurrentMessages() {
        if (activeTab === 'frontend') return chatSessions.frontend.messages;
      ...
      function switchTab(targetTab) {
        ...
        // ===== 3. 同步所有消息别名变量（cardMessages/mvuMessages/frontendMessages/messages），保证所有引用都指向最新数组 =====
        cardMessages = chatSessions.card.messages;
        mvuMessages = chatSessions.mvu.messages;
        frontendMessages = chatSessions.frontend.messages;
        if (targetTab === 'card') {
        ...
        // 前端Tab专属：模式切换条显示/隐藏 + 激活态
        const feModeBar = doc.getElementById('feModeBar');
        if (feModeBar) {
          feModeBar.style.display = (targetTab === 'frontend') ? 'flex' : 'none';
          const _feBtns = feModeBar.querySelectorAll('.fe-mode-btn');
          for (let _fi = 0; _fi < _feBtns.length; _fi++) {
            _feBtns[_fi].classList.toggle('active', _feBtns[_fi].getAttribute('data-fe-mode') === frontendMode);
          }
        }
        ...
        if (inputEl) {
          ...
          inputEl.value = '';
          if (targetTab === 'card') {
            inputEl.placeholder = '描述你想要的世界/角色设定，我来生成角色卡...';
          } else if (targetTab === 'frontend') {
            inputEl.placeholder = frontendMode === 'structured' ?
              '描述你想展示的结构化数据面板（如"做一个任务面板/论坛/状态栏"），我来生成正则+世界书条目...' :
              '描述你想要的前端界面效果（如"信纸风正文美化"），我来生成正文美化正则...';
          } else {
            inputEl.placeholder = '描述你想要的MVU变量系统或状态栏，如"做一个好感度+物品栏的状态栏"...';
          }
        ...
        if (titleH1) {
          if (targetTab === 'card') {
            ...
            titleH1.innerHTML = svgIcon('layers', 18, 'topbar-ic') + ' 前端界面<span style="font-weight:400;font-size:.85em;color:var(--ink-soft)"> · 正文美化正则</span>';
          } else {
        ...
        // ===== 8. 保存切换后的状态到 storage =====
        saveToStorage();
        showToast('已切换到：' + (targetTab === 'card' ? '角色卡生成 Tab' : (targetTab === 'frontend' ? '前端界面 Tab' : 'MVU变量状态栏 Tab')), 'info');
      }

      // 前端 Tab 模板模式切换：beautify=正文美化 / structured=结构化数据
      function switchFrontendMode(mode) {
        if (mode !== 'beautify' && mode !== 'structured') return;
        if (frontendMode === mode) return;
        frontendMode = mode;
        chatSessions.frontend.mode = mode;
        // 同步到 window：buildFrontendTabPrompt（模块级函数）读取最新模式
        if (typeof window !== 'undefined') {
          window.__frontendMode = mode;
        }
        // 更新模式按钮激活态
        const feModeBar = doc.getElementById('feModeBar');
        if (feModeBar) {
          const _feBtns = feModeBar.querySelectorAll('.fe-mode-btn');
          for (let _fi = 0; _fi < _feBtns.length; _fi++) {
            _feBtns[_fi].classList.toggle('active', _feBtns[_fi].getAttribute('data-fe-mode') === mode);
          }
        }
        // 更新输入框 placeholder
        const inputEl = doc.getElementById('chatInput');
        if (inputEl) {
          inputEl.value = '';
          inputEl.placeholder = mode === 'structured' ?
            '描述你想展示的结构化数据面板（如"做一个任务面板/论坛/状态栏"），我来生成正则+世界书条目...' :
            '描述你想要的前端界面效果（如"信纸风正文美化"），我来生成正文美化正则...';
          updateCharCount();
          updateSendBtnPulse();
        ...
        updateCtxBar();
        renderPreview();
        showToast('已切换到前端「' + (mode === 'structured' ? '结构化数据' : '正文美化') + '」模板', 'info');
      }
      ...
      function renderChatUI() {
        doc.body.innerHTML =
          ...
          '<button class="tab-btn" data-tab="frontend"><span class="tab-icon">' + svgIcon('layers', 14) + '</span>前端</button>' +
          '</div>' +
          '</div>' +
          '<div class="topbar-right">' +
          ...
          // ===== 前端 Tab 模板模式切换条（正文美化 / 结构化数据），仅前端Tab显示 =====
          '<div class="fe-mode-bar" id="feModeBar" style="display:none">' +
          '<span class="fe-mode-label">' + svgIcon('layout', 12) + ' 模板类型</span>' +
          '<button class="fe-mode-btn active" data-fe-mode="beautify" title="把AI正文渲染成信纸/日记/气泡等自定义界面">' + svgIcon('document', 12) + ' 正文美化</button>' +
          '<button class="fe-mode-btn" data-fe-mode="structured" title="把AI输出的特定格式数据（状态栏/论坛/任务面板等）渲染成界面，并生成规范AI输出的世界书条目">' + svgIcon('table', 12) + ' 结构化数据</button>' +
          '</div>' +
      ...
      function renderWorkspaceMenuItems() {
        ...
        if (currentTab === 'card') {
          ...
          items += '<div class="ws-dropdown-item" data-action="switch-tab" data-tab="frontend">' + svgIcon('layers', 15) + ' 切到 前端界面</div>';
      ...
      function bindEvents() {
        ...
        // 前端Tab模式切换条绑定
        const _feModeBarEl = doc.getElementById('feModeBar');
        if (_feModeBarEl) {
          const _feBtns = _feModeBarEl.querySelectorAll('.fe-mode-btn');
          for (let _fi = 0; _fi < _feBtns.length; _fi++) {
            _feBtns[_fi].addEventListener('click', function() {
              switchFrontendMode(this.getAttribute('data-fe-mode'));
            });
          }
        }
        // 键盘事件：
        ...
        try {
          ...
          if (state.cardData) {
            ...
            if (state.chatSessions && typeof state.chatSessions === 'object') {
              ...
              chatSessions.frontend = Object.assign({
                  messages: []
                },
                state.chatSessions.frontend || {}
              );
            } else {
            ...
            mvuMessages = chatSessions.mvu.messages;
            frontendMessages = chatSessions.frontend.messages;
            // 恢复前端 Tab 模板模式（beautify/structured），并同步到 window 供 buildFrontendTabPrompt 读取
            frontendMode = (chatSessions.frontend.mode === 'structured') ? 'structured' : 'beautify';
            if (typeof window !== 'undefined') {
              window.__frontendMode = frontendMode;
            }
            ...
            if (activeTab !== 'card' && activeTab !== 'mvu' && activeTab !== 'frontend') activeTab = 'card';
            ...
            if (activeTab === 'card') {
              messages = chatSessions.card.messages;
            } else if (activeTab === 'mvu') {
              messages = chatSessions.mvu.messages;
            }
            // frontend Tab：messages 全局兼容别名仍指向角色卡消息（该别名仅供旧代码引用角色卡用）
      ...
      function continueFromSave() {
        if (loadFromStorage()) {
          ...
          if (inputEl) {
            if (activeTab === 'card') {
              inputEl.placeholder = '描述你想要的世界书条目（任何设定…），我来生成世界书JSON...';
            } else if (activeTab === 'frontend') {
              inputEl.placeholder = frontendMode === 'structured' ?
                '描述你想展示的结构化数据面板（如"做一个任务面板/论坛/状态栏"），我来生成正则+世界书条目...' :
                '描述你想要的前端界面效果（如"信纸风正文美化"），我来生成正文美化正则...';
          ...
          // 恢复前端Tab模式切换条的显隐与激活态
          const _feModeBarR = doc.getElementById('feModeBar');
          if (_feModeBarR) {
            _feModeBarR.style.display = (activeTab === 'frontend') ? 'flex' : 'none';
            const _feBtnsR = _feModeBarR.querySelectorAll('.fe-mode-btn');
            for (let _fir = 0; _fir < _feBtnsR.length; _fir++) {
              _feBtnsR[_fir].classList.toggle('active', _feBtnsR[_fir].getAttribute('data-fe-mode') === frontendMode);
            }
          }
          const titleH1 = doc.querySelector('.topbar h1');
          if (titleH1) {
            if (activeTab === 'card') {
              titleH1.innerHTML = svgIcon('bolt', 18, 'topbar-ic') + ' 时之写卡器<span style="font-weight:400;font-size:.85em;color:var(--ink-soft)"> · 角色卡生成</span>';
            } else if (activeTab === 'frontend') {
              titleH1.innerHTML = svgIcon('layers', 18, 'topbar-ic') + ' 前端界面<span style="font-weight:400;font-size:.85em;color:var(--ink-soft)"> · ' + (frontendMode === 'structured' ? '结构化数据面板' : '正文美化正则') + '</span>';
            } else {
              titleH1.innerHTML = svgIcon('sliders', 18, 'topbar-ic') + ' MVU变量系统<span style="font-weight:400;font-size:.85em;color:var(--ink-soft)"> · 变量与状态栏</span>';
            }
          }
          ...
          showToast('已恢复上次创作进度（当前：' + (activeTab === 'card' ? '角色卡生成 Tab' : (activeTab === 'frontend' ? '前端界面 Tab' : 'MVU变量状态栏 Tab')) + '）', 'success');
        } else {
      ...
      function updateCtxBar() {
        ...
        } else if (__tab === 'frontend') {
          // 前端界面 Tab：按模板模式显示阶段提示
          if (frontendMode === 'structured') {
            stageIcon = 'table';
            const _stRxCnt = getFrontendStructuredRegexes().length;
            stageName = _stRxCnt > 0 ? '结构化数据面板 ×' + _stRxCnt + ' 已生成' : '前端界面 · 结构化数据';
          } else {
            stageIcon = 'layers';
            stageName = hasFrontendBeautifyRegex() ? '正文美化正则已生成' : '前端界面 · 正文美化';
          }
        } else {
        ...
        if (__tab === 'card') {
          ...
          if (frontendMode === 'structured') {
            // 结构化数据模式：面板数 + 世界书条目数胶囊
            const _feStRx = getFrontendStructuredRegexes();
            const _feStWi = (cardData.character_book && cardData.character_book.entries || []).filter(function(e) {
              return (e.comment || '').indexOf('[前端数据面板]') >= 0;
            }).length;
            const _stOk = _feStRx.length > 0;
            h += '<span class="ctx-chip ' + (_stOk ? 'ok' : 'todo') + '" title="' + (_stOk ? '已生成' : '未生成') + '">' + svgIcon(_stOk ? 'checkCircle' : 'circle', 11) + ' 结构化数据面板 ×' + _feStRx.length + '</span>';
            h += '<span class="ctx-chip ' + (_feStWi > 0 ? 'ok' : 'todo') + '" title="规范AI输出的世界书条目">' + svgIcon('book', 11) + ' 世界书条目 ×' + _feStWi + '</span>';
            if (_feStRx.length > 0) {
              const _firstTag = String((_feStRx[0].findRegex || '').replace(/^\/|\/$/g, ''));
              h += '<span class="ctx-chip ok" title="查找表达式">' + svgIcon('search', 11) + ' ' + escHtml(_firstTag) + '</span>';
              h += '<span class="ctx-chip ok" title="输出位置：AI输出">' + svgIcon('eye', 11) + ' AI输出 · 编辑时运行 · 仅格式显示</span>';
            }
          } else {
            // 正文美化模式：正文美化正则状态胶囊（生成状态 + 标签名 + 输出位置）
            const feRx = getFrontendBeautifyRegex();
            const feOk = !!feRx;
            h += '<span class="ctx-chip ' + (feOk ? 'ok' : 'todo') + '" title="' + (feOk ? '已生成' : '未生成') + '">' + svgIcon(feOk ? 'checkCircle' : 'circle', 11) + ' ' + (feOk ? '正文美化正则已生成' : '正文美化正则未生成') + '</span>';
            if (feRx) {
              h += '<span class="ctx-chip ok" title="查找表达式">' + svgIcon('search', 11) + ' ' + escHtml(String((feRx.findRegex || '').replace(/^\/|\/$/g, ''))) + '</span>';
              h += '<span class="ctx-chip ok" title="输出位置：AI输出">' + svgIcon('eye', 11) + ' AI输出 · 编辑时运行 · 仅格式显示</span>';
            }
          }
        } else {
      ...
      function updateQuickActions() {
        ...
        if (currentTab === 'card') {
          ...
          // 前端界面 Tab：按模板模式显示不同动作
          if (frontendMode === 'structured') {
            // ===== 结构化数据模式：生成[界面]页面名称正则 + 世界书条目 =====
            const _feStRx = getFrontendStructuredRegexes();
            const _feStWiCount = (cardData.character_book && cardData.character_book.entries || []).filter(function(e) {
              return (e.comment || '').indexOf('[前端数据面板]') >= 0;
            }).length;
            if (_feStRx.length === 0) {
              actions.push({
                action: 'generate_frontend_structured',
                icon: 'table',
                label: '生成结构化数据面板',
                title: '生成「[界面]页面名称」正则 + 界面HTML + 规范AI输出的世界书条目（状态栏/论坛/任务面板等）',
                hl: true
              });
            } else {
              actions.push({
                action: 'preview_frontend_structured',
                icon: 'eye',
                label: '预览数据面板',
                title: '预览结构化正则（' + _feStRx.length + '个面板 · 世界书条目' + _feStWiCount + '条）',
                hl: true
              });
              actions.push({
                action: 'generate_frontend_structured',
              ...
              actions.push({
                action: 'reset_frontend_structured',
                icon: 'trash',
                label: '清除数据面板'
              });
            }
            ...
            if (!feRx) {
              actions.push({
                ...
                icon: 'layers',
                label: '生成正文美化正则',
                title: '按正则模板生成「[界面]正文美化」HTML正则（AI输出+编辑时运行+仅格式显示）',
                hl: true
              });
            } else {
              actions.push({
      ...
      function bindToolbarButtons() {
        ...
        if (clearChatBtn) {
          clearChatBtn.addEventListener('click', function() {
            ...
            const curMsgs = getCurrentMessages();
            const tabName = currentTab === 'card' ? '角色卡生成' : (currentTab === 'frontend' ? '前端界面' : 'MVU变量状态栏');
            if (curMsgs.length === 0) {
      ...
      function handleQuickAction(action) {
        ...
        // 在A Tab中点击了B Tab专属动作 → 自动切到B Tab再执行（三Tab通用跳转器）
        ...
        const frontendOnlyActions = ['generate_frontend', 'preview_frontend', 'reset_frontend', 'generate_frontend_structured', 'preview_frontend_structured', 'reset_frontend_structured'];
        const tabJumpMap = [
          { tab: 'frontend', acts: frontendOnlyActions, label: '前端界面' },
          { tab: 'mvu', acts: mvuOnlyActions, label: 'MVU变量状态栏' },
          { tab: 'card', acts: cardOnlyActions, label: '角色卡生成' }
        ];
        for (let _tj = 0; _tj < tabJumpMap.length; _tj++) {
          const _tjItem = tabJumpMap[_tj];
          if (currentTab !== _tjItem.tab && _tjItem.acts.indexOf(action) >= 0) {
            // ⚠️修复：生成中 switchTab 被锁静默失败，但旧逻辑仍 300ms 后原样重放 → 无限 toast 循环。
            // 改为：切换失败（Tab 未变）时中断重放链并明确提示，不重试
            switchTab(_tjItem.tab);
            if (currentTab !== _tjItem.tab) {
              showToast('AI正在处理中，无法切换到' + _tjItem.label + ' Tab，请稍后再试「' + action + '」', 'warning');
              return;
            }
            showToast('「' + action + '」是' + _tjItem.label + '专属功能，已切换到' + _tjItem.label + ' Tab', 'info');
            // 切换后在目标Tab执行同一个动作
            setTimeout(function() {
              handleQuickAction(action);
            }, 300);
        ...
        if (action === 'generate_frontend') {
          if (isGenerating) {
            showToast('AI正在处理中，请稍候再点「生成正文美化正则」', 'warning');
            return;
          }
          if (input) {
            input.value = '请根据现有角色卡的风格，生成「正文美化」前端界面正则。输出一个完整的HTML文档代码块（```html），包含 getMessageData() + extractContent() + renderPage() 标准模式，写卡器会自动保存为[界面]正文美化正则。';
            handleSend();
          }
          return;
        }
        if (action === 'preview_frontend') {
          // 预览当前[界面]正文美化正则的HTML（从replaceString提取并展示）
          const feRx = getFrontendBeautifyRegex();
          if (!feRx || !feRx.replaceString) {
            showToast('还没有生成正文美化正则，先点「生成正文美化正则」', 'warning');
            return;
          }
          const htmlCode = String(feRx.replaceString).replace(/^```\s*\n?/, '').replace(/\n?```$/, '');
          addAssistantMsg('🎨 当前[界面]正文美化正则预览（HTML代码）：\n```html\n' + htmlCode + '\n```');
          return;
        }
        if (action === 'reset_frontend') {
          if (!confirm('确定清除已生成的「[界面]正文美化」正则吗？\n\n✅ 仅删除正文美化正则，不影响角色卡/世界书/MVU内容\n✅ 可随时重新生成')) return;
          cardData.extensions = cardData.extensions || {};
          ...
          cardData.extensions.regex_scripts = _rxFe.filter(function(r) {
            const isFe = r && (r.id === 'frontend-beautify' || (r.scriptName || '').indexOf('[界面]正文美化') >= 0);
            if (isFe) console.warn('[frontend] 清除正文美化正则:', r.scriptName);
            return !isFe;
          });
          saveToStorage();
          renderPreview();
          updateQuickActions();
          updateCtxBar();
          showToast('✅ 已清除正文美化正则，可重新生成', 'success');
          return;
        }

        // ========== 前端界面 Tab · 结构化数据面板专属动作 ==========
        if (action === 'generate_frontend_structured') {
          if (isGenerating) {
            showToast('AI正在处理中，请稍候再点「生成结构化数据面板」', 'warning');
            return;
          }
          if (input) {
            if (input.value && input.value.trim()) {
              ...
              // 无输入 → 填入引导指令
              input.value = '请根据现有角色卡的风格，设计一个「结构化数据面板」前端（如任务面板/论坛/状态栏等，展示AI按特定格式输出的数据）。\n' +
                '按格式输出：【页面名称】+【标签名】+ ```html 完整HTML代码（getMessageData + parseData + renderPage + handleClick），写卡器会自动保存为[界面]页面名称正则并生成规范AI输出的世界书条目。';
              handleSend();
            }
          }
          return;
        }
        if (action === 'preview_frontend_structured') {
          // 预览全部结构化数据面板（正则列表 + HTML源码 + 世界书条目）
          const _feStRx = getFrontendStructuredRegexes();
          if (_feStRx.length === 0) {
            showToast('还没有生成结构化数据面板，先点「生成结构化数据面板」', 'warning');
            return;
          }
          let previewText = '🎛️ 当前结构化数据面板（' + _feStRx.length + '个）：\n';
          _feStRx.forEach(function(r, i) {
            previewText += '\n──── 面板 ' + (i + 1) + ' ────\n';
            previewText += '名称: ' + (r.scriptName || '') + '\n';
            previewText += '查找: ' + (r.findRegex || '') + '\n';
            const htmlCode = String(r.replaceString || '').replace(/^```\s*\n?/, '').replace(/\n?```$/, '');
            previewText += 'HTML源码（' + htmlCode.length + '字）：\n```html\n' + htmlCode.slice(0, 3000) + (htmlCode.length > 3000 ? '\n…（截断）' : '') + '\n```';
          });
          addAssistantMsg(previewText);
          return;
        }
        if (action === 'reset_frontend_structured') {
          const _feStRx = getFrontendStructuredRegexes();
          if (_feStRx.length === 0) {
            showToast('还没有生成结构化数据面板', 'info');
            return;
          }
          if (!confirm('确定清除全部「结构化数据面板」吗？\n\n✅ 将删除 ' + _feStRx.length + ' 个[界面]正则 + 对应的世界书条目\n✅ 不影响正文美化/MVU/角色卡内容\n✅ 可随时重新生成')) return;
          removeStructuredFrontendAll();
          saveToStorage();
          renderPreview();
          updateQuickActions();
          updateCtxBar();
          showToast('✅ 已清除全部结构化数据面板（正则 + 世界书条目），可重新生成', 'success');
          return;
        }
        ...
        const frontendPrompts = {
          next: '我当前的前端界面设计进度该怎么推进？请分析：1) [界面]正文美化正则是否已生成 2) 当前界面的样式/交互可以怎么优化 3) 推荐的下一步。用简洁列表呈现。',
          summary: '帮我梳理前端界面设计现状：1) 当前[界面]正文美化正则的状态 2) 界面样式/交互的亮点与不足 3) 缺失什么、推荐的下一步。',
          generate_frontend: '请根据现有角色卡风格设计前端界面「正文美化」正则。输出一个完整的HTML文档代码块（```html），包含 getMessageData() + extractContent() + renderPage() 标准模式，写卡器会自动保存为[界面]正文美化正则。'
        };
        const structuredFrontendPrompts = {
          next: '我当前的结构化数据面板进度该怎么推进？请分析：1) 已生成哪些[界面]面板正则 2) 对应世界书条目的规范是否到位 3) 推荐的下一步。用简洁列表呈现。',
          summary: '帮我梳理结构化数据面板现状：1) 已生成的[界面]面板（名称/标签/数据格式）2) 世界书条目覆盖情况 3) 缺失什么、推荐的下一步。'
        };
        // 选择当前Tab对应的Prompt字典（前端Tab再按模板模式细分）
        let prompts;
        if (currentTab === 'card') prompts = cardPrompts;
        else if (currentTab === 'frontend') prompts = (frontendMode === 'structured') ? structuredFrontendPrompts : frontendPrompts;
        else prompts = mvuPrompts;
      ...
      function tryExtractFrontendRegexHtml(aiText) {
        ...
        // 复用统一的保存函数
        return saveFrontendRegexToCard(frontendHtml);
      }
      ...
      async function callAIChat() {
        ...
        try {
          ...
          // ===== 前端界面处理（按模板模式提取AI回复并保存正则）=====
          // ========== Tab 隔离：仅在前端界面 Tab 中执行，角色卡/MVU Tab完全跳过 ==========
          if (currentTab === 'frontend') {
            try {
              if (frontendMode === 'structured') {
                // 结构化数据模式：[界面]页面名称 正则 + 世界书条目
                ...
                if (_feSt) {
                  ...
                  const _hasFence = /```/.test(aiResponse || '');
                  if (_hasFence) {
                    showToast('⚠️ AI回复中有代码块，但未识别为结构化面板HTML（未保存）。\n请确认AI按【页面名称】+【标签名】+```html 格式输出', 'warning', 7000);
                  }
                }
              } else {
                // 正文美化模式：[界面]正文美化 正则
          ...
          } // End of: if (currentTab === 'frontend') - 前端界面处理仅在前端Tab
      ...
      function _renderPreviewImpl() {
        ...
        if (__tab === 'frontend') {
          if (frontendMode === 'structured') {
            // ========== 结构化数据模式预览：[界面]面板列表 + 世界书条目 ==========
            const _feStRx = getFrontendStructuredRegexes();
            const _feStWi = (cardData.character_book && cardData.character_book.entries || []).filter(function(e) {
              return (e.comment || '').indexOf('[前端数据面板]') >= 0;
            });
            if (_feStRx.length === 0) {
              h += '<div class="pv-section"><h3><span class="sec-left"><span class="dot empty"></span>' + svgIcon('table', 14) + ' 结构化数据面板</span><span class="pv-toggle"></span></h3><div class="pv-empty">尚未生成面板，点「生成结构化数据面板」开始（状态栏/论坛/任务面板等）</div></div>';
            } else {
            ...
            // 规范AI输出的世界书条目
            if (_feStWi.length > 0) {
              h += '<div class="pv-section"><h3><span class="sec-left"><span class="dot full"></span>' + svgIcon('book', 14) + ' 规范AI输出的世界书条目（' + _feStWi.length + '条）</span><span class="pv-toggle"></span></h3><div class="pv-content">' + escHtml(_feStWi.map(function(e, i) {
                return (i + 1) + '. [' + (e.name || e.comment || '条目') + '] 触发词: ' + ((e.keys || []).join('、') || '无') + '\n' + String(e.content || '').slice(0, 300) + (String(e.content || '').length > 300 ? '\n…' : '');
              }).join('\n\n')) + '</div></div>';
            } else {
              h += '<div class="pv-section"><h3><span class="sec-left"><span class="dot empty"></span>' + svgIcon('book', 14) + ' 规范AI输出的世界书条目</span><span class="pv-toggle"></span></h3><div class="pv-empty">尚未生成，生成面板时自动创建</div></div>';
            }
          } else {
            // ========== 正文美化模式预览：[界面]正文美化 正则状态 + HTML 摘要 ==========
            const feRx = getFrontendBeautifyRegex();
            if (feRx) {
              h += '<div class="pv-section"><h3><span class="sec-left"><span class="dot full"></span>' + svgIcon('layers', 14) + ' [界面]正文美化</span><span class="pv-toggle"></span></h3><div class="pv-content">' + escHtml('名称: ' + (feRx.scriptName || '') + '\n查找: ' + (feRx.findRegex || '')) + '</div></div>';
              const feHtmlCode = String(feRx.replaceString || '').replace(/^```\s*\n?/, '').replace(/\n?```$/, '');
              ...
              h += '<div class="pv-section"><h3><span class="sec-left"><span class="dot empty"></span>' + svgIcon('layers', 14) + ' [界面]正文美化</span><span class="pv-toggle"></span></h3><div class="pv-empty">尚未生成正文美化正则，点「生成正文美化正则」开始</div></div>';
            }
          }
          // 关联角色卡信息（只读，紧凑）
          h += '<div class="pv-section"><h3><span class="sec-left"><span class="dot ' + (cardData.name ? 'full' : 'empty') + '"></span>' + svgIcon('mask', 14) + ' 关联角色卡</span><span class="pv-toggle"></span></h3><div class="pv-content">' + escHtml(cardData.name || '(未命名)') + (cardData.description ? ' · 描述' + cardData.description.length + '字' : '') + '</div></div>';
