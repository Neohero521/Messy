...
.ctx-chip.opt{color:var(--accent-text);background:linear-gradient(135deg,var(--accent-soft),rgba(79,70,229,.12));border:1px solid var(--accent-border);max-width:300px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
/* 状态栏8步进度step方块 */
...
/* ===== Agent 对话式写卡：AI 选项按钮（:::options → 可点击推进） ===== */
.agent-options{display:flex;flex-direction:column;gap:6px;margin-top:8px;padding:0 4px 2px 0}
.agent-opt{display:flex;flex-direction:column;align-items:flex-start;gap:2px;
  padding:10px 14px;background:var(--surface);border:1px solid var(--accent-border);
  border-radius:var(--radius);cursor:pointer;text-align:left;transition:all .15s;
  font-family:inherit;color:inherit;width:100%}
.agent-opt:hover{background:var(--accent-soft);border-color:var(--accent);transform:translateX(2px)}
.agent-opt:active{transform:translateX(2px) scale(.99)}
.agent-opt:disabled{opacity:.5;cursor:not-allowed;transform:none}
.agent-opt-label{font-size:.78em;color:var(--accent-deep);font-weight:600}
.agent-opt-text{font-size:.82em;color:var(--ink-soft)}
/* 快捷动作区顶部的 Agent 建议提示位 */
.qa-agent-hint{display:block;flex-basis:100%;width:100%;font-size:.72em;color:var(--ink-soft);padding:2px 2px 4px;
...
`;
  ...
  function _releaseEditorGlobals() {
    try {
      ...
      const keys = ['__cardData', '__tab_activeTab', '__getActiveTab', '__getCurrentTab',
        ...
        '__setChatSessionsFrontendMessages',
  ...
  const SYS_PROMPT = 
    ...
    '- 开场白1：::: set first_mes\n（开场白正文，字数自然生成不设限，结构：场景描写→动作驱动→内心独白→自然对话→结尾留钩；开场白决定角色的交流风格）\n:::\n' +
  ...
  function mergePartial(partial, cd, options) {
    ...
    // ===================== Agent 架构：不再按 Tab 拦截 =====================
    // 角色卡 / 世界书 / MVU / 前端 全部在单一 Agent 会话中生成，AI 有权产出任何内容。
    // 原「角色卡 Tab 拦截所有 MVU 条目」「MVU Tab 只允许白名单字段」两块拦截及配套
    // _activeTab 计算块 / MVU 关键词库 / 灰色模式 已删除。
    // ====== 统一去重：写入前对已有 MVU 数据做去重清理（不再依赖 Tab，防止 AI 反复生成堆叠） ======
  ...
  function buildMissingMvuHint(missing) {
    ...
    return '⚠️ 前7条未齐全（第8条=状态栏HTML，必须前7条完成后才生成）。\n' +
      ...
      '请在 MVU Tab 按以下8条固定顺序生成，可一次生成多条，无需逐条等待：\n' +
      ...
      '⚠️ 第8条必须前7条完成后才生成；其余条目可一次生成多条，无需每步停下等待。';
  }
  ...
  const MVU_SEQUENTIAL_RULE =
    '【生成节奏】\n' +
    '· 可按8条固定顺序一次生成多条内容（脚本/条目/正则），无需逐条停下等待"继续"。\n' +
    '· 前7条全部完成后，才生成第8条（状态栏HTML）。\n\n';
  ...
  // ============================================================================
  // Agent 对话式写卡：统一 Prompt 层（三 Tab 提示词合一 + 全局状态感知 + 选项协议）
  // ============================================================================
  const AGENT_CORE_RULES = `
你是「时之写卡器 Agent」——一个统筹角色卡全生命周期的智能助手。
你的能力覆盖：角色卡字段生成、世界书条目、MVU 变量系统、前端界面正则。
用户只会和你对话，不需要切换任何 Tab；你负责感知全局状态并主动推进。

【行为准则】
1. 每轮先感知「当前状态摘要」（=== 当前状态 === 区块），判断现在处于哪个阶段、缺什么。
2. 用最少的一步推进最有价值的缺口，不要贪多（一个模块 / 一组条目即可）。
3. 每轮回复结尾必须给出 3 个下一步选项（:::options 块，见输出协议）。
4. 用户可能不点选项而自由输入，此时忽略选项直接响应。
5. 用户明确拒绝某方向（如"不要MVU"）时，后续不再主动推荐该方向。
6. 生成前先确认关键信息（世界观方向/内容尺度/核心角色），不要凭空捏造。

【阶段模型】（用于判断下一步）
· 阶段1 立项：确认世界观方向、内容尺度
· 阶段2 骨架：name + description + 核心世界观条目（3-5条）
· 阶段3 血肉：角色条目 / 势力条目 / 事件条目（按需扩展）
· 阶段4 开场：first_mes + alternate_greetings
· 阶段5 动态（可选）：MVU 变量系统（用户要求或角色卡需要数值追踪时）
· 阶段6 界面（可选）：前端正则（用户要求或需要美化展示时）
· 阶段7 收尾：检查完整性 → 写入酒馆

每个阶段结束时，选项应指向下一阶段或本阶段的深化。
`;

  const AGENT_OUTPUT_PROTOCOL = `
【输出协议】（每轮回复严格按此结构组织）
1. 自然语言说明（1-3句：你在做什么 / 为什么）
2. :::操作块（如有实际修改，沿用现有协议）—— 详见【角色卡技能】中的操作块语法
3. :::options 选项块（固定 3 个下一步选项）

:::options 块格式：
::: options
1. [动作标签] 一句话描述这个选项会做什么
2. [动作标签] 一句话描述
3. [动作标签] 一句话描述
:::

选项约束：
· 必须基于「当前状态摘要」的缺口给出，不能给已经完成的事
· 三个选项覆盖不同方向（如：深化现有 / 开辟新模块 / 收尾导出）
· 动作用方括号标注（如 [深化世界观]、[生成角色]、[配置MVU]、[配置界面]）
· 描述要具体，不要"继续优化"这种空话
· 用户可能会忽略选项自由输入，不要假设用户一定点选
`;

  // 原 SYS_PROMPT 精华（精简至约 1/3）：角色卡与世界书条目生成规则
  const AGENT_SKILL_CARD = `
【角色卡技能】（世界书条目生成规则，完整规范精简版）
一、ST 世界书结构：角色卡 character_book.entries 是对象数组，每条含 40 个元素；只写需要自定义的，用默认值的不写（不臆造 ST 官方不存在的字段名，严格对齐 world-info.js）。
二、生成/修改条目时按「条目元素逐项决策清单」把 40 个元素全部过一遍：
   · keys（触发词）：可中文/英文/宏；常用 {{char}}/{{user}}/{{time}}/{{date}}/{{random}}/{{pick}}/{{roll}}/{{idleDuration}} 等酒馆宏
   · constant（常驻）：true 则始终注入、不需要触发词；false 必须配 keys 否则永不触发
   · position：0=对话最前，100=对话最后（ST 官方约定）
   · selective / selectiveLogic / order：组合控制条件触发（AND/OR/NOT/AND_NOT）
   · depth：递归深度限制，防无限递归
   · probability：触发概率（0-100）
   · match_whole_words / role / group / triggers：精确匹配 / 角色限制 / 分组 / 附加触发词
三、正文内容用 YAML 中文格式（key: value，列表用 - 前缀），不用 markdown 表格。
四、增量修改用 :::操作块 协议：
   \`\`\`
   ::: upsert 唯一的条目标识
   comment: 条目名称
   ...
   五种动作：
   · upsert：新增 / 整体覆盖条目
   · update：只改指定字段（如 ::: update 标识 + 只给要改的字段）
   · delete：删除条目（::: delete 标识）
   · set：修改角色卡顶层字段（如 ::: set first_mes / name / description / alternate_greetings / personality / scenario / creator_notes）
   · rename：重命名条目标识（::: rename 旧标识 → 新标识）
   条目标识用 comment 或 key（key 用双引号包裹）。
五、多开局：first_mes + alternate_greetings（多条用 --- 分割），酒馆聊天界面可切换，{{charFirstMessage::N}} 可取第 N 条。
六、质量红线：
   · 常驻条目（constant=true）不要配 keys，避免重复触发；触发条目必须给 keys
   · 正文禁止占位符/未定文案，禁止"假装执行"（不写"（这里写…）"这类话）
   · 生成前先确认关键信息（世界观方向/内容尺度/核心角色），不要凭空捏造
`;

  // MVU 规则摘要（渐进式披露：需要时注入 MVU_VAR_SPEC 全文）
  const AGENT_SKILL_MVU_BRIEF = `
【MVU 技能】（摘要）MVU = 变量系统 + 状态栏，固定 8 条工作流，按顺序生成：
${MVU_8STEPS_SHORT}
· 前 7 条是变量系统核心（脚本 / 初始变量 / 更新规则 / 变量列表 / 输出格式 / 输出强调 / 占位提醒），第 8 条是状态栏 HTML（正则 id=mvu-status-bar，注入 {{statusBar}} 宏渲染）
· 变量需在"zod 4 schema"脚本中定义 schema；初始值用 [InitVar] 条目（YAML 中文格式）；更新规则用 [mvu_update] 条目
· 当用户需要配置变量系统（好感度/物品栏/时间/体力等数值追踪）或状态栏时，我会注入完整规范（MVU_VAR_SPEC）后按 8 条工作流生成
`;

  // 前端界面规则摘要（渐进式披露：需要时注入 FE_FULL_SPEC 全文）
  const AGENT_SKILL_FRONTEND_BRIEF = `
【前端界面技能】（摘要）用 HTML 正则美化角色卡界面展示：
· 类型1 正文美化：正则 id=frontend-beautify，HTML 用 getMessageData() + extractContent() + renderPage()，正则配置 placement=[2]（AI输出）、markdownOnly、runOnEdit、promptOnly=false，注入 {{frontendBeautify}} 宏渲染
· 类型2 结构化数据面板：正则 id=frontend-structured-<名称>，输出【页面名称】+【标签名】+完整 HTML（getMessageData + parseData + renderPage + handleClick），写卡器自动保存正则并生成配套世界书条目
· 当用户需要界面（信纸/日记/气泡美化、任务面板/论坛/状态栏面板等）时，我会注入完整规范（FE_FULL_SPEC）
`;

  // 前端界面完整规范（渐进式披露：仅当 activeSkill === 'frontend' 时注入；原 buildFrontendTabPrompt 内容精华）
  const FE_FULL_SPEC = `
【前端界面完整规范】
一、两种类型（AI 按用户需求自动判断）：
   1. 正文美化（信纸/日记/气泡/赛博等风格化正文展示）
   2. 结构化数据面板（任务面板/论坛/属性面板/状态栏等，可交互）
二、正文美化规范：
   · 正则 id: frontend-beautify，scriptName 以 [界面] 开头，enabled: true
   · HTML 结构必须完整：getMessageData() 取消息 → extractContent() 提取正文 → renderPage() 渲染
   · 正则配置：placement=[2]（AI输出）、markdownOnly=true、promptOnly=false、runOnEdit=true；findRegex 用宏标签 {{frontendBeautify}} 或消息包裹标记
   · replaceString 需包含渲染容器（如 <div class="fe-root">…{{frontendBeautify}}…</div>）
   · 样式写 <style> 内联，优先用 var(--*) CSS 变量适配明暗主题，不写死色值
三、结构化数据面板规范：
   · 输出格式（严格顺序）：
     【页面名称】<面板名>
     【标签名】<标签>（可用 {{stat_data}} 取变量，或 {{frontendStructured::<面板名>}} 注入）
     \`\`\`html
     完整 HTML（getMessageData() + parseData() 解析数据 + renderPage() 渲染 + handleClick() 交互）
     \`\`\`
   · 写卡器会自动保存为 正则 id=frontend-structured-<名称> 并生成配套世界书条目（comment 以 [前端数据面板] 开头）
   · 需要绑定变量时优先复用 MVU 变量（{{stat_data.变量名}}）
四、铁律：
   · 不生成角色卡/世界书/MVU 内容（除非用户同时要求）
   · HTML 必须是完整可渲染的（完整 <style>+<script> 结构），不造假数据
   · 面板数据必须真实取自 parseData/getMessageData
   · 不写死用户的颜色主题，用 CSS 变量
`;

  const AGENT_ANCHOR = '助手: （请只针对上方>>>标记的最新指令回复，不要重复处理已回答过的旧指令。严格遵循输出协议：自然语言说明 → :::操作块 → :::options 选项块。）';

  // ===== 对话历史（Agent 版）：剥离操作块/JSON/选项块/状态栏，只留自然语言 =====
  function buildAgentConversationHistory(messages) {
    if (!Array.isArray(messages)) messages = [];
    let fullPrompt = '';
    const historyMsgs = messages.slice(-40);
    for (let idx = 0; idx < historyMsgs.length; idx++) {
      const m = historyMsgs[idx];
      if (!m || !m.role) continue;
      const isLast = (idx === historyMsgs.length - 1);
  ...
  // ===== 阶段判定：让 AI 明确知道现在处于写卡的哪个阶段 =====
  function judgeStage(cd) {
    try {
      const entries = (cd && cd.character_book && cd.character_book.entries) || [];
      const mvuEntries = entries.filter(function(e) { return isMVUEntry(e.comment || ''); });
      const worldCount = entries.length - mvuEntries.length;
      const rx = (cd && cd.extensions && cd.extensions.regex_scripts) || [];
      const feRx = rx.filter(function(r) { return (r.scriptName || '').indexOf('[界面]') >= 0; });
      const hasName = !!(cd && cd.name);
      const hasDesc = !!(cd && cd.description && cd.description.length > 50);
      const hasFirst = !!(cd && cd.first_mes && cd.first_mes.length > 50);
      const mk = function(stage, name, short, long) {
        return { stage: stage, name: name, short: short, long: long };
      };
      if (!hasName && !hasDesc) return mk(1, '立项', '阶段1 · 立项', '阶段1 立项：确认世界观方向、内容尺度与核心角色');
      if (worldCount < 3) return mk(2, '骨架', '阶段2 · 骨架', '阶段2 骨架：name + description + 核心世界观条目（3-5条）');
      if (!hasFirst) return mk(3, '血肉', '阶段3 · 血肉', '阶段3 血肉：角色/势力/事件条目（按需扩展，next=开场白）');
      // first_mes 已生成
      if (mvuEntries.length > 0) {
        const chk = checkMvu8Entries(cd);
        if (!chk.all7Done || !chk.has8) return mk(5, '动态', '阶段5 · 动态(MVU)', '阶段5 动态：MVU 变量系统进行中（' + chk.doneCount + '/7' + (chk.has8 ? ' + 状态栏' : '，状态栏未生成') + '）');
        if (feRx.length > 0) return mk(6, '界面', '阶段6 · 界面', '阶段6 界面：前端正则进行中');
        return mk(7, '收尾', '阶段7 · 收尾', '阶段7 收尾：动态系统已完整，可检查完整性并写入酒馆');
      }
      if (feRx.length > 0) return mk(6, '界面', '阶段6 · 界面', '阶段6 界面：前端正则进行中（正文美化 / 数据面板）');
      return mk(7, '收尾', '阶段7 · 收尾', '阶段7 收尾：检查完整性（可选：按需补 MVU 动态系统 / 前端界面）→ 写入酒馆');
    } catch (e) {
      return { stage: 1, name: '立项', short: '阶段1 · 立项', long: '阶段1 立项：确认世界观方向、内容尺度与核心角色' };
    }
  }

  // ===== 全局状态摘要：让 AI 有"眼睛" =====
  function buildGlobalStateSummary(cd) {
    try {
      const entries = (cd && cd.character_book && cd.character_book.entries) || [];
      const mvuEntries = entries.filter(function(e) { return isMVUEntry(e.comment || ''); });
      const scripts = (cd && cd.extensions && cd.extensions.tavern_helper && cd.extensions.tavern_helper.scripts) || [];
      const rx = (cd && cd.extensions && cd.extensions.regex_scripts) || [];
      const feRx = rx.filter(function(r) { return (r.scriptName || '').indexOf('[界面]') >= 0; });
      const st = judgeStage(cd);
      const lines = [];
      lines.push('=== 当前状态（角色卡 / 世界书 / MVU / 前端 全局） ===');
      lines.push('【角色卡】name:' + (cd.name || '空') + ' | desc:' + ((cd.description || '').length) + '字 | first_mes:' + ((cd.first_mes || '').length) + '字 | 备选开场白:' + ((cd.alternate_greetings || []).length) + '条');
      lines.push('【世界书】共 ' + entries.length + ' 条（常驻 ' + entries.filter(function(e) { return e.constant === true; }).length + ' / 触发 ' + entries.filter(function(e) { return !e.constant; }).length + '）');
      const first20 = entries.slice(0, 20).map(function(e) { return '· ' + (e.comment || '(无comment)'); }).join('\n  ');
      if (first20) lines.push('  条目清单（前20条）：\n  ' + first20 + (entries.length > 20 ? '\n  …' : ''));
      lines.push('【MVU】条目 ' + mvuEntries.length + ' / 7' + ' | 脚本 ' + scripts.length + ' 个 | 状态栏正则 ' + (rx.some(function(r) { return r.id === 'mvu-status-bar'; }) ? '有' : '无'));
      lines.push('【前端】正文美化 ' + (rx.some(function(r) { return r.id === 'frontend-beautify'; }) ? '有' : '无') + ' | 数据面板 ' + rx.filter(function(r) { return (r.id || '').indexOf('frontend-structured-') === 0; }).length + ' 个');
      lines.push('【阶段判定】' + st.long + '  ← 明确告诉 AI 现在处于哪个阶段，下一步应推进什么');
      return lines.join('\n');
    } catch (e) {
      return '=== 当前状态 ===（摘要生成失败，请根据用户对话推断进度）';
    }
  }

  // ===== 统一 Agent Prompt：核心规则 + 输出协议 + 技能库 + 全局状态 + 历史 =====
  function buildAgentPrompt(cardData, messages) {
    const parts = [
      AGENT_CORE_RULES,
      AGENT_OUTPUT_PROTOCOL,
      AGENT_SKILL_CARD,
      AGENT_SKILL_MVU_BRIEF,
      AGENT_SKILL_FRONTEND_BRIEF,
      buildGlobalStateSummary(cardData)
    ];
    // 渐进式披露（省 token 关键）：仅当对应技能被激活时才注入完整规范
    const active = (cardData && cardData._agent && cardData._agent.activeSkill) || null;
    if (active === 'mvu') parts.push(MVU_VAR_SPEC);
    if (active === 'frontend') parts.push(FE_FULL_SPEC);
    parts.push(buildAgentConversationHistory(messages));
    parts.push(AGENT_ANCHOR);
    return parts.join('\n\n');
  }
  ...
  async function openEditor() {
    try {
      ...
      let cardData = {
        ...
        // Agent 架构运行时字段（不进导出卡）：记录当前激活的技能（mvu/frontend），驱动渐进式披露
        _agent: {
          activeSkill: null
        }
      };

      // ========== Agent 架构：单一会话统一写卡（角色卡 / 世界书 / MVU / 前端 全部在同一对话里） ==========
      // activeTab 保留声明（恒为 'card'）以兼容旧引用；三 Tab 切换器已删除，switchTab 为空实现
      let activeTab = 'card';
      // 暴露到 window：让 mergePartial / checkMvu8Entries 等顶层作用域函数也能正确取到 cardData
      if (typeof window !== 'undefined') {
        ...
        /* 灰色模式开关：保留声明以兼容旧引用（Agent 架构下 AI 可自由生成任何内容，不再需要拦截） */
      ...
      // ===== 单一 Agent 会话：不再按 Tab 拆三套消息 =====
      const chatSessions = {
        agent: {
          messages: [], // 唯一对话历史（角色卡 / 世界书 / MVU / 前端 全在这一条会话里）
          mode: 'normal'
        }
      };
      // 会话数组别名：全部指向单一 Agent 会话数组（旧代码对 card/mvu/frontend 别名的引用自动收敛）
      let cardMessages = chatSessions.agent.messages;
      let mvuMessages = chatSessions.agent.messages;
      let frontendMessages = chatSessions.agent.messages;
      // ★ 向后兼容别名：旧代码各处仍直接引用 messages 变量（importCardData/loadFromStorage等）
      // let 与原 var 同作用域语义一致（仅无前向提升，本函数声明前无 messages 引用，已核查）
      let messages = chatSessions.agent.messages;

      // 当前会话访问器：Agent 架构下恒返回唯一会话数组
      function getCurrentMessages() {
        return chatSessions.agent.messages;
      }

      function setCurrentMessages(arr) {
        chatSessions.agent.messages = arr;
        cardMessages = arr;
        mvuMessages = arr;
      ...
      // ===== Tab 切换函数：Agent 架构下已删除 Tab，切换无意义 =====
      // ⚠️保留函数体入口（空实现 + console.warn），避免历史调用点（工作区菜单/快捷动作等）报错
      function switchTab(targetTab) {
        console.warn('[agent] switchTab(' + targetTab + ') 已忽略：三 Tab 工作流已合并为单一 Agent 对话');
      }
      ...
      function renderWelcome() {
        ...
        doc.getElementById('startBtn').addEventListener('click', function() {
          ...
          renderChatUI();
          addAssistantMsg('你好！我是你的写卡 Agent 🎭\n\n我会基于 SillyTavern 原生机制，在同一对话里帮你完成：角色卡字段 / 世界书条目 / MVU变量系统 / 前端界面。每轮我都会给出「做了什么 + 三个下一步选项」。\n\n**先从两件事开始：**\n\n**① 内容尺度**：这张卡是什么尺度？\n   • 全年龄向：纯洁的青春 / 友情 / 冒险（默认，不含成人内容）\n   • 暗黑向：残酷 / 血腥 / 绝望，不含色情\n   • NSFW（18禁）：成人内容，不回避不省略\n   • 暗黑+NSFW 叠加：两套标准同时全开\n\n**② 核心方向**：想做什么样的世界？\n   直接说构想即可（如"修仙宗门""末世生存""日式校园恋爱"），我会据此生成世界书条目。\n\n请先选尺度和告诉我方向，我们就开始创作！', [
            { label: '全年龄向', text: '我想要全年龄向的角色卡，不含任何成人内容。我的方向是：' },
            { label: '暗黑向',   text: '我想要暗黑向的角色卡，包含残酷/血腥/绝望剧情，不含色情。我的方向是：' },
            { label: 'NSFW',    text: '我想要 NSFW（18禁）角色卡，包含成人内容。我的方向是：' }
          ]);
        });
      ...
      function renderChatUI() {
        doc.body.innerHTML =
          ...
          '<div class="topbar-left">' +
          '<h1>' + svgIcon('bolt', 18, 'topbar-ic') + ' 时之写卡器<span style="font-weight:400;font-size:.85em;color:var(--ink-soft)"> · Agent</span></h1>' +
          '</div>' +
          '<div class="topbar-right">' +
          ...
          '<textarea class="chat-input" id="chatInput" placeholder="描述你想做什么（角色卡 / 世界书 / MVU变量 / 前端界面…），AI 感知全局状态并给出下一步选项..." rows="1"></textarea>' +
      ...
      function renderWorkspaceMenuItems() {
        ...
        items += '<div class="ws-font-expand collapsed" id="wsFontExpand">' +
          ...
          '</div>' +
          '</div>';
        // Agent 架构：Tab 已删除，改为展示全局进度入口
        items += '<div class="ws-dropdown-divider"></div>';
        items += '<div class="ws-dropdown-section">Agent 模式</div>';
        items += '<div class="ws-dropdown-item" data-action="qa-summary" data-noop="1">' + svgIcon('bolt', 15) + ' 单 Agent 对话<span class="ws-dropdown-desc">角色卡 · 世界书 · MVU · 前端 全在同一会话</span></div>';
        // 工具（进度总览/开场白/权重/分组）
        items += '<div class="ws-dropdown-divider"></div>';
      ...
      function bindEvents() {
        ...
        // ========== Agent 架构：Tab 切换按钮已删除（单一 Agent 对话，无 Tab 可切） ==========
      }
      ...
      function importCardData(data) {
        ...
        // ========== Agent 架构：导入角色卡时重置会话（回到全新起始状态） ==========
        chatSessions.agent.messages = [];
        cardMessages = chatSessions.agent.messages;
        mvuMessages = chatSessions.agent.messages;
        // 同步到全局 messages 别名（向后兼容）
        messages = [];
        // Agent 架构：activeTab 恒为 'card'（三 Tab 已删除）
        // 导入新角色，技能激活状态重置（避免沿用上一张卡的 MVU/前端上下文）
        if (cardData && !cardData._agent) cardData._agent = { activeSkill: null };
        else if (cardData && cardData._agent) cardData._agent.activeSkill = null;
        ...
        const greeting = '你好！已成功导入角色卡「' + (cardData.name || '未命名') + '」🎭\n\n' +
          ...
          '• 想删除条目？说"删掉XXX条目"\n\n' +
          '（MVU变量系统 / 状态栏 / 前端界面也都在同一对话里直接说即可，AI 感知全局状态）\n\n' +
          '请告诉我你想做什么！';
      ...
      function resetAllWorkForNewCard() {
        ...
        // 2. 聊天记录：单一 Agent 会话清空 + 所有别名同步（兼容旧代码对 messages/cardMessages/mvuMessages/frontendMessages 的直接引用）
        chatSessions.agent = {
          messages: [],
          mode: 'normal'
        };
        cardMessages = chatSessions.agent.messages;
        mvuMessages = chatSessions.agent.messages;
        frontendMessages = chatSessions.agent.messages;
        // 全局 messages 兼容别名
        messages = chatSessions.agent.messages;

        // 3. Agent 架构：activeTab 恒为 'card'（三 Tab 已删除，保留字段仅为兼容旧引用）
        activeTab = 'card';
        currentTab = 'card';
        if (typeof window !== 'undefined') {
          window.__tab_activeTab = activeTab;
        }
        // Agent 运行时字段重置（新卡从零开始，无激活技能）
        cardData._agent = { activeSkill: null };
      ...
      function saveToStorage() {
        try {
          // 同步别名引用（chatSessions 是唯一真源；Agent 架构下全部指向单一会话数组）
          cardMessages = chatSessions.agent.messages;
          mvuMessages = chatSessions.agent.messages;
          frontendMessages = chatSessions.agent.messages;
          messages = chatSessions.agent.messages;
      ...
      function loadFromStorage() {
        try {
          ...
          if (state.cardData) {
            ...
            // ========== Agent 架构：加载唯一会话；旧版三 Tab 消息自动合并迁移（不丢任何历史） ==========
            if (state.chatSessions && typeof state.chatSessions === 'object') {
              if (state.chatSessions.agent && Array.isArray(state.chatSessions.agent.messages)) {
                // 新版：单一 Agent 会话
                chatSessions.agent = {
                  messages: state.chatSessions.agent.messages,
                  mode: 'normal'
                };
              } else {
                // 旧版三 Tab：按 角色卡 → MVU → 前端 顺序合并为一条 Agent 会话
                const _legacyC = (state.chatSessions.card && state.chatSessions.card.messages) || [];
                const _legacyM = (state.chatSessions.mvu && state.chatSessions.mvu.messages) || [];
                const _legacyF = (state.chatSessions.frontend && state.chatSessions.frontend.messages) || [];
                chatSessions.agent = {
                  messages: _legacyC.concat(_legacyM, _legacyF),
                  mode: 'normal'
                };
              }
            } else {
              ...
              if (state.cardMessages && Array.isArray(state.cardMessages)) {
                ...
                // 最早的单会话版本：原来的 messages 全部迁移
                migratedCardMsgs = state.messages.slice();
              }
              const _legacyM2 = (state.mvuMessages && Array.isArray(state.mvuMessages)) ? state.mvuMessages : [];
              chatSessions.agent = {
                messages: migratedCardMsgs.concat(_legacyM2),
                mode: 'normal'
              };
            }
            // 同步别名引用（全部指向单一 Agent 会话数组）
            cardMessages = chatSessions.agent.messages;
            mvuMessages = chatSessions.agent.messages;
            frontendMessages = chatSessions.agent.messages;
            messages = chatSessions.agent.messages;

            // Agent 架构：activeTab 恒为 'card'（三 Tab 已删除，保留字段仅为兼容旧引用）
            activeTab = 'card';
            currentTab = 'card'; // 兼容别名
            if (typeof window !== 'undefined') {
              window.__tab_activeTab = activeTab;
            }
            // Agent 运行时字段兜底
            if (!cardData._agent || typeof cardData._agent !== 'object') cardData._agent = { activeSkill: null };
            else if (!cardData._agent.activeSkill) cardData._agent.activeSkill = null;
      ...
      function continueFromSave() {
        if (loadFromStorage()) {
          ...
          // 更新输入框占位符（Agent 模式：固定统一引导）
          const inputEl = doc.getElementById('chatInput');
          if (inputEl) {
            inputEl.placeholder = '描述你想做什么（角色卡 / 世界书 / MVU变量 / 前端界面…），AI 感知全局状态并给出下一步选项...';
          }
          // 顶栏标题固定：时之写卡器 · Agent（不再随 Tab 变化）
          const titleH1 = doc.querySelector('.topbar h1');
          if (titleH1) {
            titleH1.innerHTML = svgIcon('bolt', 18, 'topbar-ic') + ' 时之写卡器<span style="font-weight:400;font-size:.85em;color:var(--ink-soft)"> · Agent</span>';
          }
          updateProgress();
          updateQuickActions();
          updateCtxBar();
          ...
          showToast('已恢复上次创作进度（Agent 单一对话模式）', 'success');
        } else {
      ...
      function updateCtxBar() {
        ...
        const p = progress || 0;
        // ===== 阶段提示：全局阶段判定（不再按 Tab 分支） =====
        const st = judgeStage(cardData);
        stage.innerHTML = svgIcon('bolt', 13) + ' <strong>' + escHtml(st.short) + '</strong> <span style="font-size:.72em;color:var(--ink-soft);margin-left:4px">' + p + '%</span>';
        // ===== 操作区：全局状态胶囊 + AI 三选项提示位 =====
        let h = '';
        const entries = (cardData.character_book || {}).entries || [];
        const mvuCount = entries.filter(function(e) {
          return isMVUEntry(e.comment || '');
        }).length;
        const ctxChk = checkMvu8Entries(cardData);
        h += '<span class="ctx-chip" title="世界书条目总数（含MVU变量条目）">' + svgIcon('book', 11) + ' 条目 ' + entries.length + '</span>';
        h += '<span class="ctx-chip ' + (mvuCount > 0 ? 'ok' : 'todo') + '" title="MVU 8步进度（前7条 + 状态栏）">' + svgIcon('sliders', 11) + ' MVU ' + ctxChk.doneCount + '/7' + (ctxChk.has8 ? ' + 栏' : '') + '</span>';
        const feB = getFrontendBeautifyRegex();
        const feSt = getFrontendStructuredRegexes();
        const feCount = (feB ? 1 : 0) + feSt.length;
        h += '<span class="ctx-chip ' + (feCount > 0 ? 'ok' : 'todo') + '" title="前端界面正则（正文美化 + 数据面板）">' + svgIcon('layers', 11) + ' 界面 ' + feCount + '</span>';
        // 三选项提示位：取最新一条助手消息的选项块（若无则提示自由输入）
        const msgs = getCurrentMessages();
        let optHint = 'AI 每轮会给出下一步选项，也可自由输入';
        for (let ci = msgs.length - 1; ci >= 0; ci--) {
          if (msgs[ci] && msgs[ci].role === 'assistant' && Array.isArray(msgs[ci].options) && msgs[ci].options.length > 0) {
            optHint = 'AI 建议：' + msgs[ci].options.map(function(o) {
              return o.label;
            }).join(' / ');
            break;
          }
        }
        h += '<span class="ctx-chip opt" title="AI 建议的下一步（点选聊天区选项按钮即可执行）">' + svgIcon('sparkle', 11) + ' ' + escHtml(optHint) + '</span>';
        actions.innerHTML = h;
        // 模块按钮点击（保留：快捷操作仍可用）
      ...
      function updateQuickActions() {
        ...
        if (!qa) return;
        // ========== Agent 架构：统一全局按钮组（不再按 Tab 分支，也不再有"预设动作"） ==========
        // 三个动态下一步选项由 AI 每轮输出（:::options 块 → 聊天区按钮），前端不再猜
        const actions = [
          { action: 'continue', label: '继续', icon: 'play' },
          { action: 'redo', label: '重做', icon: 'refreshCycle' },
          { action: 'summary', label: '查看进度', icon: 'gauge' },
          { action: 'save', label: '写入酒馆', icon: 'save' },
          { action: 'clear', label: '清空', icon: 'trash' }
        ];
        let h = '';
        actions.forEach(function(a) {
          h += '<button class="quick-btn" data-action="' + a.action + '">' + svgIcon(a.icon, 14) + ' ' + a.label + '</button>';
        });
        // 2 mini（保留 id 兼容旧绑定）：写入酒馆 / 清空
        ...
        // Agent 建议提示位（顶部）：展示最新一条助手消息的选项标签
        const msgs = getCurrentMessages();
        let hintText = '让 AI 感知全局状态，每轮给出「做了什么 + 三个下一步选项」';
        for (let hi = msgs.length - 1; hi >= 0; hi--) {
          const m = msgs[hi];
          if (m && m.role === 'assistant' && Array.isArray(m.options) && m.options.length > 0) {
            hintText = 'AI 建议：' + m.options.map(function(o) {
              return o.label;
            }).join(' / ');
            break;
          }
        }
        h = '<div class="qa-agent-hint" title="点选聊天区中的选项按钮即可执行对应步骤">' + escHtml(hintText) + '</div>' + h;
        qa.innerHTML = h;
      ...
      function bindToolbarButtons() {
        ...
        if (clearChatBtn) {
          clearChatBtn.addEventListener('click', function() {
            ...
            // Agent 架构：清空唯一 Agent 会话的聊天记录（角色卡内容不受影响）
            const curMsgs = getCurrentMessages();
            if (curMsgs.length === 0) {
              showToast('对话记录已经是空的', 'info');
              return;
            }
      ...
      function handleQuickAction(action) {
        ...
        // ========== Agent 架构：全局动作（无 Tab 概念，不再有跨 Tab 跳转） ==========
        if (action === 'save') {
          if (isGenerating) {
            showToast('AI正在处理中，请稍候再写入酒馆', 'warning');
            return;
          }
          saveCharacter();
          return;
        ...
        if (action === 'clear') {
          ...
          const msgsClear = getCurrentMessages();
          msgsClear.length = 0;
          const chatC2 = doc.getElementById('chatMessages');
          if (chatC2) chatC2.innerHTML = '';
          saveToStorage();
          updateQuickActions();
          updateCtxBar();
          renderPreview();
        ...
        if (action === 'generate_entry') {
          // Agent 架构：让AI自由生成世界书条目（用户描述什么就生成什么）
          if (input) {
        ...
        const agentPrompts = {
          next: '下一步我该做什么？请先感知当前全局状态（角色卡 / 世界书 / MVU变量系统 / 前端界面 的完成度与缺口），给出2-3条具体可执行的建议，并说明每条建议会推进哪个方向。用简洁列表呈现。',
          summary: '帮我梳理当前写卡进度：1) 已完成的核心设定 2) 世界书条目现状（含MVU变量条目）3) MVU变量系统/状态栏与前端界面的覆盖情况 4) 还缺什么 5) 推荐的下一步。用简洁列表呈现。',
          opening: '请根据现有世界观设定生成开场白。开场白1用 ::: set first_mes（场景描写→主角出场→冲突/悬念→结尾留钩，字数自然生成不设限；用 {{char}}/{{user}} 代角色与玩家名，可结合 {{time}}/{{date}}/{{random}}/{{pick}}/{{idleDuration}}/{{roll}} 等酒馆宏增强真实感，但禁止占位符/未定文案）；同时生成2条以上备选开场白用 ::: set alternate_greetings（多条用---分割，与开场白1不同的场景/视角/时机；酒馆聊天界面可切换，{{charFirstMessage::N}} 可取第N条）。全部用:::操作块输出。',
          ...
          init_var: '请帮我设计MVU变量系统：先收集我的变量需求（角色/世界观/场景/需要追踪什么状态），' +
            '然后按8条固定顺序生成，可一次生成多条，无需逐条等待。\n' +
            '现在从【第1条：变量结构脚本(zod 4 schema)】开始。',
          var_update_rule: '请检查当前MVU系统已有的条目，按8条固定顺序从缺失的第一条开始补，可一次补多条。\n' +
            '前7条全部完成后才生成第8条状态栏。',
        ...
        // Agent 架构：统一字典（next/summary 为全局视角；具体功能动作保留专属提示）
      ...
      function addAssistantMsg(content, options) {
        // 队列模式：不立即显示，收集到队列
        if (_aiChatQueueMode) {
        ...
        // Agent 架构：可选 options（预设选项按钮，用于欢迎语等静态消息）
        if (Array.isArray(options) && options.length > 0) _msgObj.options = options;
        curMsgs.push(_msgObj);
      ...
      // ===== Agent 技能激活（渐进式披露）：按用户消息/选项标签激活 MVU 或 前端 技能全文 =====
      function setActiveSkillFromText(text) {
        try {
          if (!cardData._agent || typeof cardData._agent !== 'object') cardData._agent = { activeSkill: null };
          const t = String(text || '').toLowerCase();
          // 用户明确拒绝某方向 → 停用对应技能，后续不再主动推荐
          if (/(不要|取消|停用|禁用|删除|去掉|撤掉).*(mvu|变量|状态栏|好感度|物品栏)/.test(t)) {
            if (cardData._agent.activeSkill === 'mvu') cardData._agent.activeSkill = null;
          }
          if (/(不要|取消|停用|禁用|删除|去掉|撤掉).*(前端|界面|美化|面板)/.test(t)) {
            if (cardData._agent.activeSkill === 'frontend') cardData._agent.activeSkill = null;
          }
          const wantsMvu = /\[配置mvu\]|\[配置变量\]|\[生成mvu\]|mvu|变量系统|好感度|物品栏|状态追踪|数值追踪|时间追踪|initvar|血条|体力|变量追踪|状态栏预览/.test(t);
          const wantsFe = /\[配置界面\]|\[生成界面\]|前端|界面正则|正文美化|数据面板|结构化面板|【页面名称】|【标签名】/.test(t);
          if (wantsMvu && !wantsFe) cardData._agent.activeSkill = 'mvu';
          else if (wantsFe && !wantsMvu) cardData._agent.activeSkill = 'frontend';
        } catch (e) { logWarn('setActiveSkill', e); }
      }

      // ===== Agent 选项块解析（:::options → [{label, text}]，最多3个） =====
      function parseOptionsBlock(rawText) {
        if (typeof rawText !== 'string') return [];
        const m = rawText.match(/:::\s*options\s*\n([\s\S]*?)\n\s*:::/i);
        if (!m) return [];
        return m[1].split('\n')
          .map(function(l) {
          ...
          .filter(Boolean)
          .map(function(mm) {
            return { label: mm[1].trim(), text: mm[2].trim() };
          })
          .slice(0, 3);
      }

      // ===== Agent 选项按钮渲染：把选项块渲染为可点击按钮，点选即以用户消息发出并触发 AI =====
      function renderAgentOptions(div, options, msgIdx) {
        try {
          const optHtml = options.map(function(o, i) {
            return '<button class="agent-opt" data-opt-idx="' + i + '" data-msg-idx="' + msgIdx + '" title="点击执行该选项">' +
              '<span class="agent-opt-label">' + escHtml(o.label) + '</span>' +
              '<span class="agent-opt-text">' + escHtml(o.text) + '</span>' +
              '</button>';
          }).join('');
          const wrap = doc.createElement('div');
          wrap.className = 'agent-options';
          wrap.innerHTML = optHtml;
          div.appendChild(wrap);
          wrap.querySelectorAll('.agent-opt').forEach(function(btn) {
            btn.addEventListener('click', function() {
              const _optIdx = +btn.getAttribute('data-opt-idx');
              const _mIdx = +btn.getAttribute('data-msg-idx');
              try {
                const _msgs = getCurrentMessages();
                const _msg = _msgs[_mIdx];
                if (!_msg || !Array.isArray(_msg.options) || !_msg.options[_optIdx]) return;
                if (isGenerating) {
                  showToast('AI正在处理中，请稍候', 'warning');
                  return;
                }
                const opt = _msg.options[_optIdx];
                // 技能激活（选项标签如 [配置MVU] / [配置界面]）
                setActiveSkillFromText(opt.label + ' ' + opt.text);
                addUserMsg('[' + opt.label + '] ' + opt.text);
                callAIChat().catch(function(err) {
                  showToast('调用失败：' + (err && err.message ? err.message : ''), 'error');
                });
              } catch (_optErr) { logWarn('renderAgentOptions', _optErr); }
            });
          });
        } catch (e) { logWarn('renderAgentOptions', e); }
      }

      // ===== Agent 统一后处理管线：并行运行 状态栏 / 前端界面 / 选项块 提取器 =====
      // 特征识别（不再按 Tab 分支）：各提取器自带特征黑名单互不误伤
      function processAgentReply(aiText, cardData) {
        const report = { ops: 0, json: 0, statusbar: 0, frontend: 0, structured: 0, pageName: '', options: [] };
        try {
          // 1. 状态栏 HTML（特征识别：MVU 核心 API 特征）
          if (tryExtractStatusBarHtml(aiText)) report.statusbar = 1;
        } catch (e) { logWarn('statusbar', e); }
        try {
          // 2. 前端界面（特征识别：结构化面板带【页面名称】/【标签名】标记；否则按正文美化提取）
          const _feLooksStructured = (aiText || '').indexOf('【页面名称】') >= 0 || (aiText || '').indexOf('【标签名】') >= 0;
          if (_feLooksStructured) {
            const _feSt = tryExtractStructuredFrontendHtml(aiText);
            if (_feSt) {
              report.structured = 1;
              report.pageName = _feSt.pageName || '';
            }
          } else {
            if (tryExtractFrontendRegexHtml(aiText)) report.frontend = 1;
          }
        } catch (e) { logWarn('frontend', e); }
        try {
          // 3. 选项块（:::options → 可点击推进）
          report.options = parseOptionsBlock(aiText);
        } catch (e) { logWarn('options', e); }
        return report;
      }
      ...
      function appendMsg(role, content, explicitIdx) {
        ...
        // ===== Agent 选项块：若该条助手消息带 options，渲染可点击的下一步按钮 =====
        if (role === 'assistant') {
          try {
            const _msgObj = getCurrentMessages()[msgIdx];
            if (_msgObj && Array.isArray(_msgObj.options) && _msgObj.options.length > 0) {
              renderAgentOptions(div, _msgObj.options, msgIdx);
            }
          } catch (_optLookupErr) { logWarn('appendMsg.options', _optLookupErr); }
        }
        scrollChat();
      }
      ...
      // ====================================================================
      // Agent 架构：单一快照表（三 Tab 已合并，快照统一存到 agent 键）
      ...
      async function handleSend() {
        ...
        // ===== Agent 技能激活检测（渐进式披露）：按用户消息激活 MVU / 前端 技能全文 =====
        setActiveSkillFromText(text);
        if (isGenCmd && progress >= 30) {
          // Agent 架构：无 Tab 概念，生成指令直接走 doGenerate（写卡器装配 + 写入酒馆）
          addUserMsg(text);
          await doGenerate();
          return;
        }
      ...
      function applyOps(ops, cd) {
        ...
        ops.forEach(function(op) {
          // ===== Agent 架构：不再按 Tab 拦截 MVU 条目（AI 有权生成任何内容，MVU 条目可在同一对话直接生成） =====
          ...
          if (op.action === 'upsert' || op.action === 'update') {
            ...
            if (validFields.indexOf(fieldName) >= 0) {
              // Agent 架构：不再拦截 MVU 内容写入顶层字段（AI 有权生成任何内容；固定资产保护/去重仍生效）
      ...
      async function callAIChat() {
        ...
        try {
          ...
          _aiChatNotesQueue = [];
          // ========== Agent 架构：使用唯一 Agent 会话数组 ==========
          const curTabMessages = getCurrentMessages();
          // ===== Agent 统一 Prompt：核心规则 + 输出协议 + 技能库 + 全局状态摘要 + 历史 =====
          let prompt = buildAgentPrompt(cardData, curTabMessages);
          ...
          // 否则回退到旧JSON路径（兼容）
          // ===== Agent 统一报告：记录本轮回合各提取器的命中情况（ops/json/状态栏/前端/选项块） =====
          const _unifiedReport = { ops: 0, json: 0, statusbar: 0, frontend: 0, structured: 0, pageName: '', options: [] };
          let parsed = null;
          if (hasOpBlocks(aiResponse)) {
            ...
            if (ops.length > 0) {
              ...
              if (opResult.modified) {
                ...
                progress = calcProgress();
                // Agent 架构：AI 可在同一对话生成任何内容 → 操作块应用后无条件检查固定资产注入（有 InitVar 才注入）
          ...
          if (parsed) {
            _unifiedReport.json = 1;
            // Agent 架构：不再按 Tab 过滤 MVU 内容（AI 有权生成任何内容，MVU 条目可在同一对话直接生成）
            ...
            if (hasData) {
              ...
              if (actuallyModified) {
                ...
                progress = calcProgress();
                // ===== Agent 架构：合并后确保固定资产（bundle.js + 正则1-5）始终存在 =====
                // ⚠️仅自动注入 bundle.js 和正则1-5；变量结构脚本/WTC/<状态栏>占位符提醒/正则6 由 AI 按 9.1.6 工作流一条一条生成
                // 当AI生成了InitVar条目后，触发一次补注入（确保固定资产不丢，不依赖 Tab）
          ...
          // ===== 统一后处理管线：状态栏 / 前端界面 / 选项块（特征识别，不再按 Tab 分支） =====
          // processAgentReply 并行运行各提取器（各自带特征黑名单互不误伤），并把选项块解析结果挂到消息上
          const _feReport = processAgentReply(aiResponse, cardData);
          _unifiedReport.statusbar = _feReport.statusbar;
          _unifiedReport.frontend = _feReport.frontend;
          _unifiedReport.structured = _feReport.structured;
          _unifiedReport.pageName = _feReport.pageName;
          _unifiedReport.options = _feReport.options; // ← Agent 选项块：漏了这行会导致选项按钮/ctx提示/快捷hint 全部失效
          if (_feReport.statusbar) {
          ...
          if (_feReport.structured) {
            showToast('✅ 已保存「[界面]' + (_feReport.pageName || '面板') + '」结构化正则 + 规范AI输出的世界书条目', 'success');
            saveToStorage();
            renderPreview();
            updateQuickActions();
            updateCtxBar();
          } else if (_feReport.frontend) {
            ...
            const _hasFence2 = /```/.test(aiResponse || '');
            if (_hasFence2) {
              showToast('⚠️ AI回复中有代码块，但未识别为前端界面HTML（未保存）。\n详情见浏览器Console的 [frontend] 日志', 'warning', 7000);
            }
          }
          // 检测AI发出的 <preview_statusbar> 命令（特征识别，不依赖 Tab）
          ...
          // Agent 架构：从可见 content 中剥除 :::options 块（已单独渲染为按钮），避免用户看到重复选项
          let _aiMsgContent = (rawContent && rawContent.trim().length > 0) ? rawContent : '（已应用修改）';
          if (_unifiedReport.options && _unifiedReport.options.length > 0) {
            _aiMsgContent = _aiMsgContent
              .replace(/:::\s*options\s*\n[\s\S]*?\n\s*:::/gi, '')
              .replace(/\n{3,}/g, '\n\n')
              .trim() || '（已应用修改）';
          }
          // Agent 架构：消息携带 选项块（可点击推进）+ meta（撤回/重放用）
          curTabMessages.push({
            role: 'assistant',
            content: _aiMsgContent,
            options: _unifiedReport.options,
            meta: { ops: _unifiedReport.ops, json: _unifiedReport.json, statusbar: _unifiedReport.statusbar, structured: _unifiedReport.structured, frontend: _unifiedReport.frontend }
          });
      ...
      // ===== 完整生成（Agent 架构：无 Tab 限制，直接装配角色卡并写入酒馆） =====
      async function doGenerate() {
        ...
        try {
          ...
          // Agent 架构：不再剥离 MVU 内容（AI 有权生成任何内容，后续操作块/JSON 统一走合并管线）
          // ⚠️修复：拼接 Agent 核心规则 + 输出协议，确保一键生成的回复也带 :::options 三选项
          const filteredSysForGenerate = AGENT_CORE_RULES + '\n\n' + AGENT_OUTPUT_PROTOCOL + '\n\n' + SYS_PROMPT;
          let genPrompt = filteredSysForGenerate +
            '\n\n=== 生成指令 ===\n' +
            ...
            '\n=== 输出要求 ===\n' +
            '1. 主要修改用 :::操作块协议（set顶层字段 + upsert条目），严禁输出任何JSON代码块。\n' +
            '2. ★★★ 回复末尾必须追加 :::options 块，给出 3 个下一步选项，格式严格如下（不要省略花括号里的内容）：\n' +
            '::: options\n' +
            '1. [动作标签] 一句话描述\n' +
            '2. [动作标签] 一句话描述\n' +
            '3. [动作标签] 一句话描述\n' +
            ':::\n' +
            '3. 三个选项覆盖不同方向（深化现有 / 开辟新模块 / 收尾导出），不要给重复或空泛的选项。';
          ...
          if (parsed) {
            // Agent 架构：不再做 MVU 内容过滤（AI 有权生成任何内容）
            try {
      ...
      function getModuleProgress() {
        ...
        // Agent 架构：全局统计（不再按 Tab 过滤 MVU 条目，MVU 条目也计入世界书进度）
      ...
      function calcProgress() {
        ...
        let entries = (cardData.character_book || {}).entries || [];
        // Agent 架构：全局统计（不再按 Tab 过滤 MVU 条目）
        if (entries.length >= 4) {
      ...
      function _renderPreviewImpl() {
        ...
        // ========== Agent 架构全局预览：角色卡 + 世界书 + MVU + 前端 一个面板看全 ==========
        ...
        const allEntries = (cardData.character_book && cardData.character_book.entries) || [];
        // Agent 架构：不过滤 MVU 条目（全部显示，AI 与用户都能看到全局状态）
        ...
        // ===== Agent 架构：全局预览附挂 MVU / 前端 总览（不用切换 Tab，一个面板看全） =====
        h += buildStatusBarPreviewSection();
        {
          // 前端界面总览（紧凑：正文美化 + 数据面板 + 规范条目）
          const _pvFeB = getFrontendBeautifyRegex();
          const _pvFeStRx = getFrontendStructuredRegexes();
          const _pvFeWi = (cardData.character_book && cardData.character_book.entries || []).filter(function(e) {
            return (e.comment || '').indexOf('[前端数据面板]') >= 0 || (e.comment || '').indexOf('[前端正文美化]') >= 0;
          });
          const _pvFeTotal = (_pvFeB ? 1 : 0) + _pvFeStRx.length;
          h += '<div class="pv-section"><h3><span class="sec-left"><span class="dot ' + (_pvFeTotal > 0 ? 'full' : 'empty') + '"></span>' + svgIcon('layers', 14) + ' 前端界面总览</span><span class="sec-right">' + (_pvFeTotal > 0 ? (_pvFeB ? '正文美化 · ' : '') + _pvFeStRx.length + '个面板' : '未生成') + '</span><span class="pv-toggle"></span></h3>';
          if (_pvFeTotal > 0) {
            h += '<div class="pv-sub">';
            if (_pvFeB) h += '<div style="margin:2px 0"><span class="pv-tag ok">正文美化</span><code>' + escHtml(String(_pvFeB.findRegex || '').replace(/^\/|\/$/g, '')) + '</code></div>';
            if (_pvFeStRx.length > 0) h += '<div style="margin:2px 0"><span class="pv-tag ok">数据面板 ×' + _pvFeStRx.length + '</span><code>' + escHtml(_pvFeStRx.map(function(r, i) { return (i + 1) + '.' + (r.scriptName || '(未命名)'); }).join(' ')) + '</code></div>';
            h += '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:6px">' +
              '<button class="pv-mini-btn" data-pv-action="preview-frontend">' + svgIcon('eye', 13) + ' 预览界面</button>' +
              '<button class="pv-mini-btn" data-pv-action="reset-frontend">' + svgIcon('trash', 13) + ' 清除界面正则</button>' +
              '</div></div>';
          } else {
            h += '<div class="pv-empty">尚未生成前端界面（正文美化 / 数据面板），在对话里直接说要什么界面即可</div>';
          }
          h += '</div>';
        }
