...
`;
  ...
  const SYS_PROMPT = 
    ...
    '\n' +
    '【宏规范（对齐 SillyTavern 官方宏引擎，用于开场白/世界书正文/对话示例）】\n' +
    '- 宏语法：{{宏名}} 不区分大小写；带参数用 {{宏名::参数}} 或 {{宏名 参数}}（空格或双冒号均可）；宏可嵌套（如 {{getvar::{{char}}_mood}}）；想显示字面花括号用 \\{\\{ 转义。\n' +
    '- 名称与参与者：{{user}}（当前用户/人设名）、{{char}}（当前角色名）、{{group}}（群组成员列表）、{{charIfNotGroup}}（非群组时的角色名）。\n' +
    '- 角色卡字段：{{description}}（角色描述）、{{personality}}（性格）、{{scenario}}（场景）、{{charFirstMessage}}（第一条消息/问候语）。\n' +
    '- 时间日期：{{time}}（当前时间）、{{date}}（当前日期）、{{weekday}}（星期几）、{{isodate}}（YYYY-MM-DD）、{{datetimeformat::格式}}（自定义格式，如 YYYY-MM-DD HH:mm:ss）。\n' +
    '- 随机与骰子：{{random::a::b::c}}（随机选一个，每次重随机）、{{pick::a::b::c}}（稳定随机）、{{roll::XdY+Z}}（骰子，如 {{roll::1d20}}、{{roll::2d6+3}}）。\n' +
    '- 变量：{{getvar::变量名}}/{{setvar::变量名::值}}（MVU 局部变量）、{{getglobalvar::}}/{{setglobalvar::}}（全局变量）。\n' +
    '- 实用：{{newline}}（换行）、{{space}}（空格）、{{idleDuration}}（距上次用户消息的时长，如 4 hours）、{{lastMessage}}（最后一条消息）、{{input}}（当前输入框内容）。\n' +
    '- 使用原则：开场白/世界书正文/对话示例中，遇到"角色名/用户名/时间/随机事件/骰子"等动态内容时优先用对应宏，让内容随对话实时变化；不得编造官方不存在的宏名。\n' +
    '- 对话示例（mes_example）：每个示例前用 <START> 标签分隔；{{user}} 代表用户发言、{{char}} 代表角色发言，禁止直接写死角色/用户名。\n' +
    '- 触发词（keys）写法：普通词直接写；需要灵活匹配时用 JS 正则并以 / 为定界符（如 /(?:{{char}}|他) (?:看向|望着) (?:天空|月亮)/i），合法正则会被酒馆自动识别为 regex 匹配，可配合 use_regex 使用。\n' +
    '- 世界书顶层可选元数据：description（世界书说明）、scan_depth（全局扫描深度）、token_budget（条目token预算）、recursive_scanning（是否允许递归激活），有明确设定时可输出到 character_book 顶层。\n' ;
