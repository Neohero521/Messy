...
`;
  ...
  function getDisplayGroup(e) {
    ...
    if (['引导机制', '互动选项', '状态栏'].indexOf(prefixKey) >= 0) return '动态系统';
    if (['故事发展', '文化与习俗', '历史事件'].indexOf(prefixKey) >= 0) return '叙事';
    return '触发体系';
  }
  ...
  const SYS_PROMPT = 
    ...
    '=== 输出协议（二选一，自由选择） ===\n' +
    '\n' +
    '【方式一 · 完整JSON代码块（用户要求"生成/导出世界书"时使用）】\n' +
    '直接输出一个```json代码块，内容为完整世界书JSON，例如：\n' +
    '{\n' +
    '  "entries": {\n' +
    '    "0": { "uid": 0, "key": "触发词A,触发词B", "comment": "条目名", "content": "条目内容", "constant": false, "selective": false, "order": 100, "position": 0, "disable": false, "probability": 100, "useProbability": false, "matchWholeWords": false }\n' +
    '  }\n' +
    '}\n' +
    '只输出代码块，代码块外可加1-2句简短说明。\n' +
    '\n' +
    '【方式二 · :::操作块协议（对话中增量增删改，最常用）】\n' +
    '- ::: upsert 条目名\\n完整条目内容\\n:::   → 新增或覆盖（覆盖时必须包含完整旧内容+改动部分，严禁只写变动字段）\n' +
    '- ::: delete 条目名\\n:::   → 删除条目\n' +
    '- ::: set 顶层字段名\\n值\\n:::   → 修改世界/角色卡顶层字段（name/description等）\n' +
    '- ::: rename 旧名 → 新名\\n:::   → 重命名条目\n' +
    '多条操作块之间空一行，可混合增删改同时输出。\n' +
    '\n' +
    '=== 对话引导原则 ===\n' +
    '- 像朋友聊天一样自然，不要像填表单\n' +
    '- 用户说"我要XX / 给我加XX / 帮我写XX" → 直接生成对应世界书条目\n' +
    '- 每次生成条目时，用户未提及的字段用默认值，不强行编造\n' +
    '- 主动给出建议和灵感，可以建议补充触发词、常驻设置、位置层级等\n' +
    '- 修改已有条目时必须包含完整旧内容+改动部分（upsert整体覆盖，只写变动部分会丢失原信息）\n' +
    '- 用户说"重做/换掉/重写" → 先 delete 旧条目再 upsert 新条目，组合操作一次完成\n' +
    '- 用户说"补充/细化/丰富" → 新增条目（可用 ⟦条目名·维度⟧ 形式），不覆盖原条目\n' +
    '- 中文内容 matchWholeWords 必须 false\n' +
  ...
  function findMatchingEntry(newEntry, existingArr) {
    ...
    // 第2优先级：同规范前缀下只有1条现有条目 + 后缀有相关性（AI改了comment后缀但前缀一致，如<世界规则>魔法→<世界规则>魔力）
  ...
  function normalizeWorldInfoJSON(partial) {
    ...
    lists.forEach(function(entryList) {
      entryList.forEach(function(ne) {
        ...
        // key/keysecondary（逗号分隔字符串）→ keys/secondary_keys（数组）
        if (ne.key !== undefined && ne.keys === undefined) {
          ne.keys = String(ne.key).split(/[,，、\n]/).map(function(s) { return s.trim(); }).filter(function(s) { return s; });
          delete ne.key;
        }
        if (ne.keysecondary !== undefined && ne.secondary_keys === undefined) {
          ne.secondary_keys = String(ne.keysecondary).split(/[,，、\n]/).map(function(s) { return s.trim(); }).filter(function(s) { return s; });
  ...
  function mergePartial(partial, cd, options) {
    ...
    if (_activeTab === 'card') {
      ...
      // 拦截的：明确属于角色卡Tab常驻体系/世界观体系的专有模板条目（世界元数据/核心玩法/重要角色等）
      const CARD_ONLY_TEMPLATES_RE = /^<(世界元数据|当前局势|核心玩法|世界规则|重要角色|势力与组织|物品|地点场景|故事发展|文化与习俗|历史事件|引导机制|互动选项|统一输出格式|角色边界|禁止项|自定义条目|观察锚点)>/i;
      const filterMvuOnlyEntries = function(arr, srcName) {
        ...
        arr = arr.filter(function(e) {
          ...
          // 判定2：命中角色卡Tab专属模板条目（<世界元数据>、<核心玩法>等）→ 拦截
  ...
  function buildPrompt(cardData, cardGenerated, messages) {
    ...
    // jsonReminder：角色卡Tab支持两种输出——完整世界书JSON代码块 或 :::操作块协议
    let jsonReminder = '';
    // 角色卡Tab：允许输出```json世界书代码块（自由生成世界书条目），也保留:::操作块协议（增量增删改）
    jsonReminder = '\n\n⚠️【输出格式提醒 - 每次回复必须遵守（角色卡Tab）】\n' +
      '1. 两种输出方式任选：\n' +
      '   A. ```json代码块：输出完整世界书条目（顶层 entries 为对象，键为字符串uid "0"/"1"...；每条含 uid/key/comment/content/constant/selective/selectiveLogic/order/position/disable/probability/useProbability/depth/matchWholeWords 等字段，字段名必须与上方规则一致，不得臆造）\n' +
      '   B. :::操作块协议：::: 动作 条目名\\n内容\\n:::（upsert增改 / update只改 / delete删 / set顶层字段 / rename重命名）\n' +
      '2. 用户要求"直接生成世界书/给我JSON/生成条目代码"时，优先输出```json代码块\n' +
      '3. 对话中增量修改推荐使用:::操作块协议\n' +
      '4. 状态栏完全不在此Tab处理——如果用户要做MVU/变量/状态栏，请引导切换到MVU Tab\n' +
      '5. 先输出1-2句自然语言说明，再输出代码块，代码块后不再解释\n' +
      ...
      '10. ⚠️【upsert覆盖必填完整】当要改已有条目：先读上方「当前角色卡已有内容」拿完整旧content，:::upsert或JSON覆盖时输出完整旧内容+改动部分，严禁只输出变化字段（会导致原信息清空）\n' +
      ...
      '13. ★★★【自由生成】不要受任何固定体系/标签前缀束缚！用户要什么就生成什么条目，条目标签自由命名（如"世界规则""重要角色""地点场景""自定义"等），字段配置按上方ST世界书JSON规则设置。';
  ...
  function assignAndWrapTagIds(entries) {
    ...
    const classified = entries.map(function(e, idx) {
      ...
      // 2. 世界观组：
      if (/^(世界观|世界元数据|当前局势|核心玩法|世界规则|重要角色|势力与组织|物品|地点场景|故事发展|文化与习俗|历史事件|引导机制|互动选项|状态栏|<世界元数据>)/.test(c) ||
    ...
    try {
      ...
      function renderChatUI() {
        doc.body.innerHTML =
          ...
          '<textarea class="chat-input" id="chatInput" placeholder="描述你想要的世界书条目（角色/地点/规则/历史/机制…），我来生成世界书JSON..." rows="1"></textarea>' +
      ...
      function buildWorkspaceTree() {
        ...
        const tagOrder = ['核心玩法', '统一输出格式', '状态变量输出', '引导机制', '未分类'];
        ...
        if (loadFromStorage()) {
          ...
          if (inputEl) {
            inputEl.placeholder = activeTab === 'card' ?
              '描述你想要的世界书条目（角色/地点/规则/历史/机制…），我来生成世界书JSON...' :
              '描述你想要的MVU变量系统或状态栏，如"做一个好感度+物品栏的状态栏"...';
          }
          const titleH1 = doc.querySelector('.topbar h1');
          if (titleH1) {
      ...
      function handleQuickAction(action) {
        ...
        if (action === 'generate_entry') {
          ...
          if (input) {
            input.value = '请帮我生成世界书条目。请先告诉我你想在世界书里添加什么内容（角色/地点/物品/规则/历史/机制等），或者我直接根据当前进度给你建议。可以直接输出符合ST世界书JSON规范的条目（```json代码块）或用:::操作块。';
            handleSend();
          }
          return;
        }
        ...
        const cardPrompts = {
          ...
          generate_entry: '请帮我自由生成世界书条目（不限制标签前缀，用户要什么就生成什么）。用:::upsert操作块或```json代码块输出，字段配置遵循ST世界书JSON规则。'
        };
        const mvuPrompts = {
      ...
      function parseModProgress(reply) {
        // 已移除八体系模块标记机制：世界书条目进度由 getModuleProgress 从 entries 实时计算
        return null;
      }
