...
`;
  ...
  // MVU 官方运行时接口参照（MagVarUpdate 官方 API，生成代码必须贴合）—— 源自官方类型定义
  const MVU_OFFICIAL_API_RULES =
    '【MVU 官方运行时接口（MagVarUpdate / MagicalAstrogy/MagVarUpdate，生成代码必须贴合官方）】\n' +
    '· 变量数据统一存放于 stat_data（MvuData.stat_data）；脚本中读取用 _.get(data, \'stat_data.路径\')，写入用 _.set(data, \'stat_data.路径\', 值)，删除用 _.unset(data, \'stat_data.路径\')。\n' +
    '· 楼层变量更新命令（AI 在回复正文中输出，Mvu.parseMessage 自动解析执行）：_.set(\'角色.好感度\', 30); // 修改原因；命令路径不带 stat_data 前缀；支持 _.get/_.update/_.unset 等 lodash 形式，可附 // 注释说明修改原因。\n' +
    '· 官方事件（eventOn 监听变量更新生命周期）：Mvu.events.VARIABLE_INITIALIZED（新开聊天初始化）、VARIABLE_UPDATE_STARTED（一轮更新开始）、COMMAND_PARSED（命令解析后可修正）、VARIABLE_UPDATE_ENDED（更新结束后可修正，如把数值钳制在范围内）。\n' +
    '· 官方函数：await waitGlobalInitialized(\'Mvu\') 等待可用；Mvu.getMvuData({type:\'message\'|\'chat\'|\'character\'|\'global\'}) 读取；Mvu.parseMessage(消息, 旧数据) 解析更新命令；Mvu.replaceMvuData(新数据, 选项) 写回。\n';
