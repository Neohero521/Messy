  ...
  function _setScriptVars(obj) {
    ...
    try {
      ...
      if (pWin && typeof pWin.replaceVariables === 'function') {
        // 酒馆助手 API：replaceVariables(variables, options) —— 第一个参数是变量对象
        // 先读现有变量，合并写入 → 保留酒馆中其他脚本/其他 key 的数据，避免互相清空
        var merged = obj || {};
        if (typeof pWin.getVariables === 'function') {
          try {
            var existing = pWin.getVariables({ type: 'script' }) || {};
            merged = Object.assign({}, existing, obj || {});
          } catch(_) {}
        }
    ...
    // 双写兜底：无论脚本变量是否写入成功，都同步写 localStorage，
    // 保证刷新网页后设置/图谱进度不丢失；读取已有数据合并，避免只写不读覆盖其他 key
    try {
      var raw = localStorage.getItem(SCRIPT_ID + ':vars');
      var stored = raw ? JSON.parse(raw) : {};
      var next = Object.assign({}, stored, obj || {});
...
function _loadSettingsCache() {
    // 优先读取脚本变量，失败/为空时自动回退到 localStorage 兜底，刷新后数据不丢
...
function cleanupNovelWriter() {
  ...
  var _retryCount = 0;
  function tryInit() {
    // 悬浮图标常驻：无需任何按键，刷新/加载后自动显示在页面上
    addNovelWriterFloatingButton();
    // 额外注册脚本按钮（环境支持时提供，不影响悬浮图标）
  ...
  function cleanupScriptArtifacts() {
    ...
    try {
      var doc = _pDoc();
      ['-btn', '-modal', '-float-style', '-float-menu'].forEach(function (suffix) {
        var el = doc.getElementById(SCRIPT_ID + suffix);
        if (el) el.remove();
      });
    } catch(_) {}
  }
  ...
  function scriptEntryPoint() {
    ...
    // 卸载前：先同步强制落盘（覆盖防抖 300ms 期间就刷新的极端情况），再清理 DOM
    try {
