...
@media (max-width: 720px) {
  ...
  .agent-status { position: absolute; top: 60px; right: 0; bottom: 0; z-index: 50; box-shadow: var(--ag-shadow); }
  .agent-status.collapsed { transform: translateX(100%); width: 280px; }
}
...
小数据适配：若前文内容较少，严格遵循现有文本的叙事范式、对话模式、剧情节奏，不做风格跳脱的续写，不无限新增设定与人物`;
  ...
  function filterGraphByTimeline(mergedGraph, baseChapterId) {
    ...
    if (filteredGraph.世界观设定库?.['全本所有隐藏设定/伏笔汇总']) {
  ...
  async function generateNovelWrite(selectedChapterId, wordCount, chapterCount, extraDirectives) {
    ...
    try {
      ...
      for (let chapterRound = 0; chapterRound < chapterCount; chapterRound++) {
        ...
        let continueContent = await generateRawWithBreakLimit({ systemPrompt, prompt: userPrompt, ...presetInjected });
        if (stopGenerateFlag) { agentTaskEnd(taskId, '已停止', 'warn'); return; }
        ...
        if (settings.enableWriteQualityGate !== false) {
          ...
          if (gateIssue) {
            ...
            while (gateAttempt < 2 && !stopGenerateFlag) {
              ...
              const retryContent = String(await generateRawWithBreakLimit({ systemPrompt, prompt: fixPrompt, ...presetInjected }) || '').trim();
              let retryIssue = '';
  ...
  function recordTaskHistory(text) {
    ...
    while (mem.taskHistory.length > AGENT_CFG.MAX_MEMORY_ITEMS) mem.taskHistory.shift();
    saveAgentMemory(mem);
  }
  ...
  function rememberPreference(text) {
    ...
    while (mem.preferences.length > AGENT_CFG.MAX_MEMORY_ITEMS) mem.preferences.shift();
    saveAgentMemory(mem);
  }
  function rememberFact(text) {
    const mem = getAgentMemory();
    mem.facts.push({ text: String(text), time: Date.now() });
    while (mem.facts.length > AGENT_CFG.MAX_MEMORY_ITEMS) mem.facts.shift();
    saveAgentMemory(mem);
  }
  ...
  const TOOLS = {
    ...
    list_chapters: {
      ...
      params: { range: '可选 all（默认）/ recent / missing_graph' },
      run: async (args) => {
    ...
    generate_graph: {
      ...
      params: { scope: '可选 missing（默认，只生成缺失的）/ all / selected；chapterIds: 可选章节 id 数组' },
      run: async (args) => {
    ...
    precheck: {
      ...
      params: { chapterId: 'number 基准章节 id（必填）' },
      run: async (args) => {
    ...
    write_chapters: {
      desc: '续写章节：从基准章节之后继续写，自动做前置校验、回填续写章节图谱。',
      params: { chapterId: 'number 基准章节 id（必填）', wordCount: 'number 每章目标字数，默认 2000', chapterCount: 'number 续写章数 1-20，默认 1', directives: 'string 可选，用户补充的写作要求（风格/剧情方向等）' },
      run: async (args) => {
        const id = parseInt((args && args.chapterId), 10);
        const ch = currentParsedChapters.find(c => c.id === id);
    ...
    open_view: {
      ...
      params: { view: 'reader / graph / chat', chapterId: '可选，阅读指定章节' },
      run: async (args) => {
    ...
    save_memory: {
      ...
      params: { text: 'string 要记住的内容（必填）', type: '可选 preference / fact，默认 preference' },
      run: async (args) => {
    ...
    delete_continue_chapter: {
      ...
      params: { chapterId: 'number 续写章节 id（必填）' },
      danger: true,
      requireConfirm: true,
      run: async (args) => {
        const id = parseInt((args && args.chapterId), 10);
    ...
    update_chapter_content: {
      ...
      params: { chapterId: 'number 章节 id（必填）', content: 'string 新内容（必填）' },
      run: async (args) => {
        const id = parseInt((args && args.chapterId), 10);
    ...
    rename_chapter: {
      desc: '修改章节标题。',
      params: { chapterId: 'number 章节 id（必填）', title: 'string 新标题（必填）' },
      run: async (args) => {
        const id = parseInt((args && args.chapterId), 10);
    ...
    set_settings: {
      desc: '修改 Agent 设置并立即生效：enablePrecheck / enableWriteQualityGate / writeQualityMinChars / rateLimitEnabled / rateLimitMaxCalls / rateLimitWindow / rateLimitUnit / agentMaxSteps / enableTavernPresetInject / enableAutoParentPreset。',
      params: { key: 'string 设置键（必填）', value: 'number / boolean / string 新值（必填）' },
      run: async (args) => {
        const key = String((args && args.key) || '').trim();
        const ALLOWED = new Set(['enablePrecheck', 'enableWriteQualityGate', 'writeQualityMinChars', 'rateLimitEnabled', 'rateLimitMaxCalls', 'rateLimitWindow', 'rateLimitUnit', 'agentMaxSteps', 'enableTavernPresetInject', 'enableAutoParentPreset']);
        ...
        let value = raw;
        if (key === 'enablePrecheck' || key === 'enableWriteQualityGate' || key === 'rateLimitEnabled' || key === 'enableTavernPresetInject' || key === 'enableAutoParentPreset') {
    ...
    recall_task: {
      ...
      params: { count: '可选 number，读取最近 N 条，默认 5' },
      run: async (args) => {
    ...
    forget_memory: {
      ...
      params: { index: '可选 number 记忆序号（preferences 从 0 开始，facts 紧接其后，任务历史单独 T 前缀展示）' },
      run: async (args) => {
        const mem = getAgentMemory();
  ...
  const AGENT_STEP_SCHEMA = {
    ...
    value: {
      ...
      properties: {
        ...
        tool: { type: 'string', description: '要调用的工具名（任务未完成时输出）' },
  ...
  function buildAgentStaticPrompt() {
    ...
    const toolLines = Object.keys(TOOLS).map(name => {
      const t = TOOLS[name];
      const params = Object.keys(t.params || {}).length ? '  参数：' + JSON.stringify(t.params) : '';
...
4. 续写前如果图谱不完整（覆盖率低或无合并图谱），应先生成/合并图谱再续写；用户要求快速续写时可直接续写。
5. 用户提到的风格、字数、剧情要求必须纳入 write_chapters 的 directives 参数。
6. 某步失败时，根据错误信息调整策略或换工具，不要原样重试超过 2 次；确实无法完成就直接用 final 说明。
7. 默认续写字数参考长期记忆中的偏好，没有则 2000 字。
8. final 用中文回答用户：说明做了什么、结果如何、建议下一步，简洁清晰。
9. 涉及清空数据等破坏性操作时，先向用户确认再执行。`;
...
${extraRules.length ? extraRules.join('\n') : '无特殊限制。'}`;
  ...
  async function runAgent(userInput) {
    ...
    try {
      for (let step = 0; step < maxSteps; step++) {
        ...
        if (step === 0 && Array.isArray(res.plan) && res.plan.length) {
        ...
        if (stepIdx >= 0) agentState.currentPlan[stepIdx].status = 'running';
        renderStatusPanel();

        const t0 = Date.now();
        let obs;
        try {
          const toolResult = await TOOLS[toolName].run(res.args || {});
        ...
        obs.ms = Date.now() - t0;
        agentRenderToolCall(toolName, res.args, obs, step);
        agentState.toolLog.push({ tool: toolName, args: res.args || {}, ok: obs.ok, ms: obs.ms, time: Date.now() });
        ...
        const obsContent = obs.ok
          ? (typeof obs.result === 'string' ? obs.result : JSON.stringify(obs.result))
          : `工具执行失败：${obs.error}。请调整参数或策略后重试，或改用其他工具；连续失败请直接用 final 向用户说明。`;
        messages.push({ role: 'user', content: '[工具 ' + toolName + ' 返回]\n' + String(obsContent).slice(0, 6000) });
      }
  ...
  // 需求四/A：危险工具确认 —— 生成确认卡并挂起 Promise，等用户点击确认/取消
  function requestAgentConfirm(toolName, args) {
    return new Promise(resolve => {
      ...
      agentState.pendingConfirm = { id, resolve };
    });
  }
  ...
  function renderStatusPanel() {
    ...
    const memHtml = memItems.length
      ? memItems.slice(-8).map(m => `<div class="mem-item ${m.fact ? 'mem-fact' : ''}">${escapeHtml(m.text)}</div>`).join('')
  ...
  function renderChatMessage(msg) {
    ...
    if (msg.kind !== 'chapter_preview' && msg.kind !== 'chapter_full' && msg.kind !== 'confirm') {
      const bt = el.querySelector('.bubble-text');
      if (bt) bt.textContent = msg.progress?.text || msg.text;
    }
    $flow.get(0).appendChild(el);
    // 自动滚到底
  ...
  function restoreCheckpoint() {
    ...
    setTimeout(() => {
      const keep = confirm(`检测到上次续写任务未完成（基准第 ${baseChapterId} 章，已完成 ${progress || 0}/${chapterCount} 章）。是否继续完成上次任务？`);
      extension_settings[extensionName].agentCheckpoint = null;
      if (keep) {
        // 先移除该任务已生成的续写章节，避免重复
  ...
  function bindAllEvents() {
    ...
    $('#settings-save-btn').off('click').on('click', function () {
      ...
      saveSettingsDebounced();
      closeSettingsModal();
      agentNotify('✅ 设置已保存（质检与限流开关即时生效）', 'success');
    });
    ...
    $('#chapter-list').off('contextmenu').on('contextmenu', '.chapter-item', function (e) {
      ...
      $menu.css({ left: Math.max(0, Math.min(e.clientX, maxX)) + 'px', top: Math.max(0, Math.min(e.clientY, maxY)) + 'px' });
    });
