...
(function() {
  ...
  const SCRIPT_VERSION = 'v5.8';  // 版本标识：标题栏可见 + console 输出，用于确认加载的是最新文件
...
.agent-version { opacity: 0.55; font-size: 10px; letter-spacing: 0.3px; }
...
/* Agent 状态面板结构（全局：桌面/移动端都需要背景、flex、可滚动；宽度/折叠在媒体查询里覆盖） */
.agent-status {
  background: var(--ag-bg-2);
...
.agent-tab {
  ...
  font-family: inherit;
  white-space: nowrap; /* 全局强制：任何断点/容器宽度下 Tab 文字不换行竖排 */
}
...
/* 三面板轨道包裹层（桌面端纵向单显；移动端媒体查询内改为横向滑动轨） */
.agent-panels {
  flex: 1;
  display: flex;
  flex-direction: column;
...
/* Bug9：滚动容器到边界不外泄成整页拖拽（含 iOS 惯性滚动） */
.chat-flow,
.reader-content,
.chapter-list,
.graph-editor,
...
@media (max-width: 720px) {
  .agent-root {
    ...
    --ag-header-h: 56px;  /* P0：抽屉顶部偏移基准，JS 启动时写入实际 header 高度 */
    font-size: 15px;
    /* P0：不要覆盖 width/height，保留基础规则的 100vw/100vh/100dvh，否则 iframe 内高度塌缩 */
    max-width: 100vw;
    max-height: 100vh;
    max-height: 100dvh;
  ...
  .agent-header {
    position: relative;
    z-index: 56;  /* P0：遮罩 z=55，header 要在遮罩之上可点，抽屉 z=60 */
    padding: 8px 10px;
  ...
  .sidebar-resizer { display: none !important; }  /* Bug5：窄屏/混合触摸场景下不残留拖拽柄 */
  ...
  /* ---------- 中央主区：三面板横向轨道 + 紧凑胶囊 Tab 条（参照移动端竖版对话/预览交互） ---------- */
  /* 说明：对话/阅读/图谱三个面板横向并排各占 100%，靠 transform 平移切换；
     顶部保留紧凑胶囊 Tab 条（当前视图指示 + 点击直达），左右滑动跟手切换 */
  .agent-tabs {
    display: flex;
    gap: 6px;
    padding: 8px 12px;
  ...
  .agent-tab:active { transform: scale(.96); }
  .agent-tab svg { width: 16px; height: 16px; flex-shrink: 0; }
  ...
  .agent-main[data-tab-idx="1"] .agent-panels { transform: translateX(-100%); }
  .agent-main[data-tab-idx="2"] .agent-panels { transform: translateX(-200%); }
  /* Bug2：JS 把 swiping 类加在 .agent-panels 自身，选择器必须匹配；跟手时去过渡并启用 GPU 合成层 */
  .agent-panels.swiping { transition: none; will-change: transform; }
  ...
  .confirm-btns .card-btn { flex: 1; display: inline-flex; align-items: center; justify-content: center; }
  ...
  /* 文字按钮（摘要/人物）取消 38px 定宽，自适应内容并保证最小可点宽度 */
  #reader-summary-btn, #reader-characters-btn { width: auto; min-width: 56px; min-height: 38px; padding: 0 12px; font-size: 13px; }
  ...
  .graph-editor { font-size: 16px; line-height: 1.65; padding: 12px; border-radius: 10px; }
  ...
  .modal, .modal-content {
    ...
    max-height: 86vh; max-height: 86dvh; border-radius: var(--ag-radius);
  }
...
/* ==================== Agent 状态面板（右侧） ==================== */
/* 桌面端专属：这些规则必须包在 min-width 里，否则会覆盖移动端媒体查询里的
   position:absolute / 抽屉宽度（CSS 层叠：同特异性后写胜出），导致移动端侧栏没脱离 flex 流、占着宽度把主区挤到右半屏 */
...
/* ==================== 需求三B：侧栏拖拽 / ⌘K 命令面板 ==================== */
@media (min-width: 721px) {
.agent-sidebar { position: relative; }  /* 仅桌面：resizer 定位参照；移动端必须 absolute 抽屉，见 max-width 媒体查询 */
}
.sidebar-resizer {
...
/* Bug1：抽屉遮罩覆盖整屏，包含 header / footer（fixed 相对视口，不再 relative 到 .agent-body） */
@media (max-width: 720px) {
...
/* Bug10：轨道显式全宽，避免滑动时 1px 漂移；基础态 pan-y，水平滑动时交还内部纵向滚动 */
@media (max-width: 720px) {
  .agent-panels { width: 100%; touch-action: pan-y; }
}

/* Bug11：Tab 双击不缩放 + 消除高亮 */
@media (max-width: 720px) {
  .agent-tab {
...
/* Bug5：对话气泡长文本（长单词/URL/Base64）不溢出 */
.chat-msg { max-width: 100%; min-width: 0; }
.chat-bubble {
...
/* P3：矮屏（<=560px）更多菜单向上弹，避免被输入栏截断 */
@media (max-width: 720px) and (max-height: 560px) {
  .agent-more-menu { top: auto; bottom: calc(100% + 8px); }
}

/* Bug12：阅读器滚动补偿，避免正文被 header / footer 压住 */
@media (max-width: 720px) {
  .reader-content {
...
如果 2 次尝试都无法解决，请直接用 final 汇报失败原因和建议，不要继续无效重试。`;
  ...
  function switchTab(tabName) {
    const prevTab = extension_settings[extensionName].activeTab;
    ...
    // 只在真正切换时持久化，避免滑动过程重复写 localStorage
    if (prevTab !== tabName) saveSettingsDebounced();
    // 移动端：三面板轨道按索引平移到位（桌面端无对应 CSS 规则，不受影响）
    try {
      const idxMap = { chat: 0, reader: 1, graph: 2 };
      const $main = $('.agent-main');
      if ($main.length) $main.attr('data-tab-idx', String(idxMap[tabName] || 0));
    } catch (_) {}
  }
  ...
  function bindAllEvents() {
    ...
    let _lpTimer = null, _lpFired = false, _lpResetTimer = null;
    $('#chapter-list')
      ...
      .off('touchmove touchend touchcancel').on('touchmove touchend touchcancel', function () {
        clearTimeout(_lpTimer);
        // Bug3：长按触发过后延时重置标志，即使没点章节也不会吞掉下一次点击
        if (_lpFired) {
          clearTimeout(_lpResetTimer);
          _lpResetTimer = setTimeout(function () { _lpFired = false; }, 350);
        }
      });
    ...
    // Bug7：图谱编辑器聚焦时滚入视口，避免被软键盘遮挡
    $('#graph-preview').off('focus').on('focus', function () {
      if (!isMobileLayout()) return;
      const el = this;
      setTimeout(function () {
    ...
    // ========== 输入栏 ==========
    // 移动端：聚焦自动滚到底；键盘弹起压缩快捷栏。
    // Bug6 关键修复：mini-jQuery 的 on() 原样传事件名给 addEventListener，'focus.agkb' 这种
    // 带命名空间的写法浏览器根本不识别，导致旧监听器从未触发。必须用纯事件名 'focus'/'blur'。
    $('#agent-input').off('focus').on('focus', function () {
      ...
      const scrollIn = function () {
        try { inputEl.scrollIntoView({ block: 'end', behavior: 'smooth' }); } catch (_) {
          try { inputEl.scrollIntoView(false); } catch (_) {}
        }
      ...
      if (vv && vv.addEventListener) {
        const onVV = function () { vv.removeEventListener('resize', onVV); scrollIn(); };
        vv.addEventListener('resize', onVV);
        setTimeout(onVV, 320);  // 保底
      } else {
        setTimeout(scrollIn, 320);
      }
    });
    $('#agent-input').off('blur').on('blur', function () {
    ...
    function sendUserInput() {
      ...
      handleCommand(text);
      // 可选优化：移动端发送后收起键盘，避免输入栏被顶住
      if (isMobileLayout()) { try { $('#agent-input').blur(); } catch (_) {} }
    }

    // ========== 移动端：三面板轨道左右滑动切换（跟手 + 边缘阻尼 + 阈值 + 双 rAF 归位） ==========
    // 参照移动端竖版对话/预览交互：整轨 transform 跟随手指，松手按阈值切换或回弹
    (function bindSwipeTabs() {
      var $wrap = $('#agent-panels');
      if (!$wrap || !$wrap.length) return;
      var el = $wrap.get(0);
      var win = (el.ownerDocument && el.ownerDocument.defaultView) || window;
      var TAB_ORDER = ['chat', 'reader', 'graph'];
      var SWIPE_THRESHOLD = 55;   // 超过该位移才切换
      var EDGE_RATIO = 0.28;      // 首页/末页继续拖动时的阻尼系数
      var startX = 0, startY = 0, lock = null, dx = 0, fromIdx = 0;

      // 起点是否落在可横向滚动的容器内（快捷按钮横滚条 / 宽表格 / 代码块），若是则不拦截手势
      function inHScroller(node) {
        var n = node;
        while (n && n !== el && n.nodeType === 1) {
          var ovx = win.getComputedStyle(n).overflowX;
          if ((ovx === 'auto' || ovx === 'scroll') && n.scrollWidth - n.clientWidth > 4) return true;
          n = n.parentNode;
        }
        return false;
      }
      ...
      function applyTransform() {
        ...
        el.style.transform = 'translateX(' + (-fromIdx * w + dx) + 'px)';
      }
      function finishSwipe() {
        if (lock !== 'horizontal') { lock = null; return; }
        var goOther = false;
        if (dx < -SWIPE_THRESHOLD && fromIdx < TAB_ORDER.length - 1) goOther = true;
        if (dx > SWIPE_THRESHOLD && fromIdx > 0) goOther = true;
        var targetIdx = goOther ? (dx < 0 ? fromIdx + 1 : fromIdx - 1) : fromIdx;
        // Bug2：同一帧先更新 data-tab-idx，再移除 swiping、清空内联 transform。
        // CSS 过渡从「手指离开位置」直接滑到「目标 panel」，不会先回弹到旧位置。
        if (targetIdx !== fromIdx) switchTab(TAB_ORDER[targetIdx]);
      ...
      el.addEventListener('touchstart', function (e) {
        if (!isMobileLayout()) { lock = 'skip'; return; }
        var t = e.touches[0], node = e.target;
      ...
      el.addEventListener('touchmove', function (e) {
        ...
        var t = e.touches[0];
        var rawDx = t.clientX - startX, rawDy = t.clientY - startY;
        if (lock !== 'horizontal') {
          // P1：起手 8px 内不判定，避免误触
          if (Math.abs(rawDx) < 8 && Math.abs(rawDy) < 8) return;
          // 水平意图必须"明显"强于垂直（1.5 倍 + 水平绝对位移 >12px），斜滑不误判，保证上下滚动顺畅
          if (Math.abs(rawDx) > Math.abs(rawDy) * 1.5 && Math.abs(rawDx) > 12) {
            lock = 'horizontal';
            el.classList.add('swiping');
            el.style.touchAction = 'pan-y';
          } else { lock = 'vertical'; return; }  // 垂直：不 preventDefault，让内部容器自然滚动
        }
        dx = rawDx;
        // 首页向右 / 末页向左：边缘阻尼，拖不动但不突兀
        if ((fromIdx === 0 && dx > 0) || (fromIdx === TAB_ORDER.length - 1 && dx < 0)) dx = rawDx * EDGE_RATIO;
        applyTransform();
    ...
    // Bug1：抽屉展开时，点击 header / footer / 主区非抽屉区域先收起，避免误触
    $(doc).off('click').on('click', function (e) {
      if (!isMobileLayout()) return;
      const open = !$('#agent-sidebar').hasClass('collapsed') || !$('#agent-status-panel').hasClass('collapsed');
      if (!open) return;
      const t = e.target;
      if (!t || !t.closest) return;
      if (t.closest('.agent-sidebar') || t.closest('.agent-status') || t.closest('.agent-more-menu')) return;
      closeDrawers();
    });

    // P0：写入实际 header 高度到 CSS 变量（抽屉 top 偏移基准，兼容刘海屏）
    function syncHeaderHeightVar() {
      if (!isMobileLayout()) return;
      try {
        const headerEl = $('.agent-header').get(0);
        if (!headerEl) return;
        const h = Math.round(headerEl.getBoundingClientRect().height);
        const rootEl = getDoc().querySelector('.agent-root');
        if (rootEl) rootEl.style.setProperty('--ag-header-h', h + 'px');
      } catch (_) {}
    }
    syncHeaderHeightVar();
    let _hdrTimer = null;
    try {
      (window.visualViewport || window).addEventListener('resize', function () {
        ...
        _hdrTimer = setTimeout(syncHeaderHeightVar, 120);
      });
    } catch (_) {}

    // P1：软键盘统一补偿（visualViewport）：阅读器底部留键盘高度、对话流滚到底
    (function bindKeyboardViewport() {
      ...
      const apply = function () {
        ...
        if (Math.abs(kbH - lastKbH) < 8) return;
        lastKbH = kbH;
        const rootEl = getDoc().querySelector('.agent-root');
        if (!rootEl) return;
        if (kbH > 80) {
          rootEl.classList.add('kb-open');
          const rc = getDoc().querySelector('.reader-content');
          if (rc) rc.style.paddingBottom = (kbH + 24) + 'px';
          const cf = getDoc().querySelector('.chat-flow');
          if (cf) cf.scrollTop = cf.scrollHeight;
        } else {
          rootEl.classList.remove('kb-open');
          const rc = getDoc().querySelector('.reader-content');
          if (rc) rc.style.paddingBottom = '';
        }
      };
  ...
  function scriptEntryPoint() {
    ...
    try {
      console.log('[小说续写Agent] ' + SCRIPT_VERSION + ' 已加载（iframe 内，viewport=' + (window.innerWidth || '?') + 'x' + (window.innerHeight || '?') + '，matchMedia(mobile)=' + (isMobileLayout ? isMobileLayout() : '?') + '）');
      try {
        const $ver = $('#agent-version');
        if ($ver && $ver.length) $ver.text(SCRIPT_VERSION);
      } catch (_) {}
    } catch (_) {}
