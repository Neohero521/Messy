...
(function() {
  ...
  // #11 兼容性降级：旧版浏览器缺少 Object.hasOwn / structuredClone 时提供 polyfill，
  // 避免加载阶段报错。JSON 序列化对纯数据对象（设置/图谱）足够。
  if (typeof Object.hasOwn !== 'function') {
    ...
    } catch (_) { /* 只读 Object 环境（罕见）跳过，后续调用点已有 try 包裹 */ }
  }
    ...
    try {
      ...
      if (raw) {
        try {
          ...
          if (typeof stored !== 'object' || stored === null || Array.isArray(stored)) throw new Error('not-an-object');
        } catch (parseErr) {
          // #15 修复：解析失败不静默跳过——清除坏数据并重建，保证后续写入真正生效
          try { console.warn('[小说续写器] localStorage 变量数据损坏，已清除重建:', parseErr && parseErr.message); } catch (_) {}
          try { localStorage.removeItem(LOCAL_VARS_KEY); } catch (_) {}
          stored = {};
        }
      }
  ...
  const AI_CALL_TIMEOUT_MS = CONFIG.AI_CALL_TIMEOUT_MS; // 单次 AI 调用上限 120 秒，超时后进入重试/报错
  ...
  // ---- 兼容别名：业务逻辑中大量使用的 getDoc()/jQuery/$/toastr ----
  var getDoc = _pDoc;

  // $ / jQuery / toastr：优先转发到 iframe 内（DOM 操作隔离在 iframe 里）
  // （旧版 _resolveDollar/_toastrMake 惰性取值已删除：自赋值模式会覆盖 $ 丢失 iframe 分支）
  var $ = function () {
...
const SAVE_DEBOUNCE_MS = CONFIG.SAVE_DEBOUNCE_MS;
...
function escapeHtml(text) {
    ...
    // #12/#23 修复：字符串替换实现，不依赖 getDoc()（iframe 未就绪也可用），长列表渲染性能更佳
...
const MAX_RETRY_TIMES = CONFIG.MAX_RETRY_TIMES;
...
const MAX_API_CALLS_PER_MINUTE = CONFIG.MAX_API_CALLS_PER_MINUTE;
const API_RATE_LIMIT_WINDOW_MS = CONFIG.RATE_LIMIT_WINDOW_MS;
...
const defaultSettings = {
    ...
    precheckReportText: "",
    floatBallState: {
            position: { x: 16, y: 16 }, // #10：默认位置惰性计算（模块加载阶段不访问 window.parent）
            isPanelOpen: false,
...
async function rateLimitCheck() {
    ...
    // #9 修复：循环「检查-等待-再检查」，等待期间其他调用过期/被清理会被感知，避免突破限流上限
    const interval = 100;
    let notified = false;
    while (true) {
        const checkNow = Date.now();
        apiCallTimestamps = apiCallTimestamps.filter(timestamp => checkNow - timestamp < windowMs);
        if (apiCallTimestamps.length < maxCalls) break;

        const earliestCallTime = Math.min(...apiCallTimestamps);
        const waitTime = earliestCallTime + windowMs - checkNow;
        if (waitTime <= 0) break;
        ...
        await new Promise(resolve => setTimeout(resolve, Math.min(interval, waitTime)));
    }
...
const FloatBall = {
    ...
    restoreState() {
        ...
        const maxX = window.parent.innerWidth - this.ball.offsetWidth;
        const maxY = window.parent.innerHeight - this.ball.offsetHeight;
        // #10 修复：无保存位置/位置无效时惰性计算默认位置（模块加载阶段不访问 window.parent，避免跨域/未就绪抛错）
        const savedX = (state && state.position && typeof state.position.x === 'number') ? state.position.x : window.parent.innerWidth - this.ball.offsetWidth - 26;
        const savedY = (state && state.position && typeof state.position.y === 'number') ? state.position.y : Math.max(0, Math.floor(window.parent.innerHeight / 2));
        const safeX = Math.max(0, Math.min(savedX, maxX));
        const safeY = Math.max(0, Math.min(savedY, maxY));
...
async function loadSettings() {
    ...
    // 章节正则同步到新书架解析入口（有保存值时才覆盖默认值）
    if (settings.chapterRegex) {
        $("#bookshelf-chapter-regex-input").val(settings.chapterRegex);
    }
...
// 同步书架计数、排序选择器、排序方向、视图模式等头部状态（元素缺失时为无害空操作）
function syncBookshelfHeaderState() {
    const s = extension_settings[extensionName] || {};
    $('#bookshelf-count-display').text(`共 ${bookshelf.length} 本小说`);
    $('#bookshelf-sort-select').val(s.bookshelfSortBy || 'updatedAt');
    $('#bookshelf-sort-order-icon').text(s.bookshelfSortOrder === 'asc' ? '⬆️' : '⬇️');
    $('#bookshelf-view-icon').text(s.bookshelfViewMode === 'grid' ? '📑' : '📋');
}
// rAF 合并：同一帧内多次调用 renderBookshelf 只执行最后一次，
// 减少批量删除/导出/标签筛选时的重复重渲染（书架 DOM 较重）
if (CONFIG.RENDER_RAF_ENABLED) {
...
async function updateGraphWithContinueContent(continueChapter, continueId) {
    ...
    try {
        ...
        const graphMap = extension_settings[extensionName].chapterGraphMap || {};
        graphMap[`continue:${continueId}`] = graphData;
        extension_settings[extensionName].chapterGraphMap = graphMap;
        saveSettingsDebounced();
        return graphData;
    } catch (error) {
...
async function generateContinueWrite(targetChainId) {
    ...
    // 用 findIndex 定位目标章节在链中的真实位置（链上 id 可能因删除而不连续），取目标章节及其前一章作为上下文
    const targetIndex = continueWriteChain.findIndex(item => item.id === targetChainId);
    const safeTargetIndex = targetIndex >= 0 ? targetIndex : Math.max(0, targetChainId - 1);
    const targetBeforeChapters = continueWriteChain.slice(Math.max(0, safeTargetIndex - 1), safeTargetIndex + 1);
    targetBeforeChapters.forEach((chapter, index) => {
        const chapterNum = Math.max(0, safeTargetIndex - 1) + index + 1;
...
async function openNovelWriter(opts) {
    ...
    try { await loadSettings(); } catch(e) { console.error('[小说续写器] loadSettings 失败:', e); }
        ...
        // 旧版「文件上传/解析/按字数分章」UI 入口已下线（新流程见 bookshelf-select-file-btn / bookshelf-parse-and-save-btn）
;
    ...
    // 旧版「发送模板/发送延迟」输入框 UI 已下线（sendTemplate/sendDelay 使用默认值）
    ...
    // 质量检查开关统一由 setupToggleSwitch 绑定（基于配置值取反，避免浏览器自动切换 checkbox 导致双重翻转）
    ...
    // ========== 书架事件监听器 ==========
        // 旧版「保存到书架/清除当前小说」按钮已下线（新流程：解析添加即保存到书架）
    ...
    // 书架计数/排序/视图同步已合并进 renderBookshelf 本体（syncBookshelfHeaderState）
    ...
    try {
      if (!window.__nwResizeBound) {
        ...
        // （#8/#25：悬浮球重建会先经 _removeFloatBallListeners 清理旧监听，重复调用幂等、不累积监听）
...
function cleanupNovelWriter() {
  ...
  // #8 修复：悬浮球全局监听注册表，重建悬浮球前先移除旧监听，避免 doc/pWin 监听累积
  let _floatBallListeners = [];
  function _removeFloatBallListeners() {
    for (const item of _floatBallListeners) {
      try { item.target.removeEventListener(item.type, item.fn); } catch (_) {}
    }
    _floatBallListeners = [];
  }
  function _addFloatBallListener(target, type, fn) {
    try { target.addEventListener(type, fn); } catch (_) { return; }
    _floatBallListeners.push({ target: target, type: type, fn: fn });
  }
  function addNovelWriterFloatingButton() {
    try {
      ...
      var pWin = (typeof window !== 'undefined' && window.parent) ? window.parent : window;
      _removeFloatBallListeners(); // #8：先移除上一次注册的全局监听
      ...
      // 点击菜单外关闭
      _addFloatBallListener(doc, 'click', function (e) {
        if (menu.style.display === 'flex' && !menu.contains(e.target) && !btn.contains(e.target)) hideMenu();
      });

      // 视口变化时钳制位置
      var _resizeTimer = null;
      _addFloatBallListener(pWin, 'resize', function () {
      ...
      addCleanup(function () {
        try { _removeFloatBallListeners(); } catch (_) {}
